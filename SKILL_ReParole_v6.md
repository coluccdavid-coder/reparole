...
## Kabyle (langue partielle) — état précis (v6.113)

**v6.113** : premier retour du contact kabylophone intégré — écran
"Avez-vous un bilan ?" traduit pour la première fois (confidentialité
d'un fichier médical, tenu en français depuis la v6.2 faute de
relecture native), plusieurs clés d'interface, et 2/4 questions de
"Vos ressentis". Nouveau `docs/kabyle-glossary.md` (vocabulaire,
boutons, style) à réutiliser pour la suite.

**En attente** : `docs/kabyle-symptoms-request.md` (mis à jour) — 2
lots restants : (1) les 2 dernières questions de "Vos ressentis"
(`SYMPTOM_QUESTIONS_KAB` pas encore créé, il faut les 4 questions
ensemble), (2) les 6 phrases du mini-test d'accueil pour complétion/
compréhension (`ASSESS_ITEMS` — la dénomination est déjà faite,
`ASSESS_ITEMS_KAB.denomination`). Volontairement pas fait par
moi-même à l'origine (phrases complètes à conjuguer, kabyle
structurellement trop différent de l'arabe dialectal) — un brouillon
partiel a depuis été ajouté au fichier de demande, à la demande de
l'utilisateur, explicitement marqué "à corriger, pas à relire". Une
fois la version finale reçue : compléter `SYMPTOM_QUESTIONS_KAB` +
`completion`/`comprehension` dans `ASSESS_ITEMS_KAB`, tout dans
`js/assessment.js`.
