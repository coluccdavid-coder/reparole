...
## Dialectes maghrébins (algérien, marocain, tunisien) — état précis (v6.103)

**v6.103** : contenu d'exercices ajouté pour l'**algérien (dz)
uniquement** — dénomination (92 mots) et complétion (24 phrases),
fournis par l'utilisateur (5 fichiers Excel), intégrés dans
`js/exercises-dz.js`. Il manque encore la compréhension (18
questions, voir `docs/dz-parity-request.md`). Marocain (ma) et
tunisien (tn) n'ont toujours aucun contenu d'exercices.

**Changement de cap (v6.102)** : sur demande explicite de l'utilisateur
(trop d'écrans visibles restaient en français), on ne se limite plus
au socle minimal de 22 clés — objectif désormais affiché : **parité
d'interface complète comme le japonais (443 clés)**. Vu le volume
(~420 clés restantes × 3 dialectes), le travail avance **par lots**
plutôt qu'en une seule livraison. **Lot 1 fait (46 clés/langue)** :
écran de connexion + tableau de bord de base (socle d'origine) +
espace aidant, rappels, boutons d'accessibilité, 3 titres/descriptions
d'exercices (photos, dénomination orale, répétition). **~400 clés
restantes par dialecte** — reprendre lot par lot (voir `js/i18n.js`,
bloc `fr` pour la liste de référence complète, comparer aux blocs
`dz`/`ma`/`tn`).

`js/i18n.js`, blocs `dz`/`ma`/`tn` dans `I18N_STRINGS`. Toujours en
langues **partielles** (`PARTIAL_LANGS`), pas complètes — le statut
"partiel" ne porte plus sur le volume de l'interface (en cours
d'extension) mais sur l'absence de relecture native et l'absence de
contenu d'exercices (voir plus bas).
