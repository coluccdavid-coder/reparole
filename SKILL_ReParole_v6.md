...
ne l'est** (14 langues dont kabyle et 3 dialectes maghrébins, sourcées
avec des degrés de rigueur variables). Ce
...
langues restantes, contenu kabyle/darija plus riche) plutôt que
...
**14 langues** : fr (complète, dénomination 35 mots/niveau) ;
en/es/it/pt/de/ar/tr/pl/**ja** (interface complète + tous les
exercices — ja depuis la v6.91, mais voir la note ja plus bas : 3
exercices vocaux manquants, découverts en v6.150). kab, dz, ma, tn
(**partielles** — interface à 538/538 comme les langues complètes
depuis la v6.145, mais toujours signalées "en cours de traduction" :
contenu d'exercices présent partout mais pas relu par des
locuteurs·rices natif·ves pour dz/ma/tn (brouillons de ma part, v6.142
et v6.147), aucun exercice vocal pour aucune des 4 (aucune voix
navigateur ne les prend en charge). v6.151 : sango retiré de l'app,
demandé par l'utilisateur — n'a jamais dépassé 22 mots de dénomination
ni eu d'interface traduite. Voir "Kabyle", "Japonais" et "Dialectes
maghrébins" ci-dessous pour l'état précis de chacune.
...
## Kabyle (langue partielle) — état précis (v6.120)

**🎉 v6.120 : interface 100% complète (448/448 clés).** Tout le
questionnaire d'accueil, tout le dashboard, tout l'espace
orthophoniste, l'espace aidant, le compagnon "Ami" (46/46 phrases,
`COMPANION_PHRASES.kab`), le mini-test (dénomination/complétion/
compréhension, `ASSESS_ITEMS_KAB`, 3/3 chacun) et les 4 questions de
"Vos ressentis" (`SYMPTOM_QUESTIONS_KAB`) sont traduits et relus par
le même contact kabylophone, sur une dizaine d'envois échelonnés
(v6.104 → v6.120). Voir `docs/kabyle-glossary.md` pour le vocabulaire
de référence et l'historique complet du travail.

**Reste dans les limites documentées** :
- **kab reste dans `PARTIAL_LANGS`**, volontairement — malgré
  l'interface complète, ce n'est pas un statut "langue complète"
  comme le japonais : pas de `speechLocale` (aucune voix navigateur ne
  prend en charge le kabyle), et bandeau d'avertissement toujours
  affiché pour signaler que le contenu, bien que relu par un∙e natif∙ve,
  reste un premier passage sans validation clinique formelle
  (garde-fou n°8).
- Écran de dépôt de bilan (lecture de fichier PDF/texte) : ~9 clés
  encore en français (`reading_in_progress`, `read_file_failed`,
  etc. — voir `ASSESS_STRINGS.kab` dans `js/assessment.js`), chemin
  secondaire peu emprunté, pas encore couvert.
- Footer légal (`js/footer.js`) : traductions kabyles reçues et
  conservées dans `docs/kabyle-glossary.md`, mais pas câblées — ce
  fichier est codé en dur en français pour **toutes** les langues,
  chantier séparé qui dépasse le seul kabyle.
- 3 conflits de vocabulaire (cheval/vélo/voiture) **résolus** en
  v6.116 — l'utilisateur a tranché en faveur de l'existant
  (Agmar/Avilu/Takeṛṛust), rien à changer côté code. Reste juste à
  améliorer 2 distracteurs d'exercice (AƐEWDIW → AGMAR, → TAMUTURT)
  quand une prochaine livraison sera faite — annoté dans le code,
  pas fait pour éviter une livraison non demandée.

**Découverte utile en cours de route (v6.115), corrigée pour TOUTES
les langues** : la tendance de progression (`js/app.js`) et le
message du graphique (`js/charts.js`) étaient codés en dur en
français, jamais localisés pour personne — nouvelles clés
`chart_not_enough_data`/`trend_up`/`trend_down`/`trend_stable`/
`trend_not_enough`, ajoutées aux 10 langues complètes + dz/ma/tn.

**Toujours en attente** : `docs/kabyle-symptoms-request.md` (mis à
jour) — 2 lots restants : (1) les 2 dernières questions de "Vos
ressentis" (`SYMPTOM_QUESTIONS_KAB` pas encore créé, il faut les 4
questions ensemble), (2) la phrase "café/TASSE" du mini-test
(`ASSESS_ITEMS_KAB.completion` pas encore créé, il faut les 3
phrases ensemble). Volontairement pas fait par moi-même à l'origine
(phrases complètes à conjuguer, kabyle structurellement trop différent
de l'arabe dialectal) — un brouillon partiel a depuis été ajouté au
fichier de demande, à la demande de l'utilisateur, explicitement
marqué "à corriger, pas à relire".

**v6.104** : parité complète du contenu d'exercices atteinte —
dénomination (92 mots fusionnés avec les 22 déjà sourcés, total
30/42/41 par niveau), complétion (24 phrases) et compréhension (18
questions), toutes fournies par l'utilisateur (`Kabyle_Complet.xlsx`),
traduites/relues par une personne kabylophone d'après l'utilisateur.
`docs/kabyle-parity-request.md` et `docs/kabyle-completion-draft.md`
marqués comme comblés/dépassés.

**Ce qui reste vrai malgré cette relecture native** : premier jet, pas
de validation clinique (garde-fou n°8) — une relecture native n'est
pas une validation orthophonique. Les exercices vocaux (répétition,
dénomination orale, fluence) restent en français : le kabyle n'a
toujours pas de `speechLocale`. L'interface (menus, boutons) reste
partielle (22 clés, comme dz/ma/tn avant leur extension — voir plus
bas) : seul le contenu des exercices est maintenant complet.

**Historique** (état avant v6.104) : seuls 22 mots de dénomination
étaient sourcés (2+ sources indépendantes chacun), et
`docs/kabyle-completion-draft.md` ne contenait que 4 phrases de
complétion tirées de corpus, jamais intégrées faute de relecture
native. Message prêt à envoyer à apprendrelekabyle.com (demande
...
si une réponse est arrivée à la reprise (toujours pertinent : ça
concerne l'audio, pas le texte, donc pas résolu par le fichier
Kabyle_Complet.xlsx).

## Japonais — langue complète depuis la v6.91, réellement complète depuis la v6.151

`js/exercises-ja.js` : 130 items de contenu à choix (92 mots de
dénomination sur 3 niveaux + 24 phrases de complétion + 18 questions
de compréhension), fournis intégralement par l'utilisateur via
`Japonais_Complet.xlsx` (v6.89). **Interface complètement traduite
depuis la v6.91** : les 403 clés d'`I18N_STRINGS.ja`,
`COMPANION_PHRASES.ja` et `ASSESS_STRINGS.ja` — retiré de
`PARTIAL_LANGS`, même statut que les 8 autres langues complètes
(en/es/it/pt/de/ar/tr/pl).

**⚠️ VRAI MANQUE trouvé et comblé en v6.151** : malgré ce statut
"complète" depuis la v6.91, le japonais n'avait en fait jamais eu les
3 exercices vocaux (`denomination_orale`, `repetition`, `intonation`)
que les 8 autres langues complètes ont — jamais remarqué parce que
ces exercices reposent sur le micro, pas testés automatiquement de la
même façon que les exercices à choix multiple. Découvert en marge de
la préparation des voix cloud (v6.150), en comparant les comptages de
contenu entre langues. Comblé de ma part (pas de nouveau fichier
fourni par l'utilisateur) : 6/6/6 pour denomination_orale, 8/8/8 pour
repetition (mots volontairement longs/complexes au niveau 3, même
logique que les autres langues, pas une traduction terme à terme),
6/6/6 pour intonation.

**Remarque importante** : vérification faite par échantillonnage
manuel d'une vingtaine de mots japonais contre mes propres
connaissances de la langue : vocabulaire standard et correct.

**Comme pour les 8 autres langues complètes** (aucune d'entre elles
n'a été relue par un∙e professionnel∙le natif∙ve non plus) : reste en
attente d'une vraie relecture avant tout usage clinique — c'est
justement pour ça que l'utilisateur fait tester le site à un contact
japonais natif. **speechLocale:'ja-JP'** actif : synthèse/reconnaissance
vocale du navigateur disponibles pour le japonais, contrairement au
kabyle et aux dialectes maghrébins.

**Limitation connue, inchangée** : lecture hiragana/rōmaji fournie
dans le fichier source mais pas encore affichée dans l'interface (pas
de furigana) — demanderait une extension du moteur d'affichage des
choix sans casser la comparaison réponse/choix déjà en place. Proposé
à l'utilisateur, pas encore tranché.

## Dialectes maghrébins (algérien, marocain, tunisien) — état précis (v6.109)

**Interface : parité complète atteinte (443/443, v6.109)**, en 6 lots
(voir README pour le détail lot par lot). **dz/ma/tn restent
volontairement dans `PARTIAL_LANGS`** malgré ça — pas une langue
"complète" comme le japonais, pour deux raisons qui persistent : (1)
contenu d'exercices toujours incomplet (voir juste en dessous), (2)
traduction jamais relue par un∙e locuteur∙rice natif∙ve (contrairement
au japonais fourni par l'utilisateur). Le bandeau d'avertissement
`lang-partial-note` doit rester visible pour ces trois langues.

**Contenu d'exercices (v6.103)** : dénomination (92 mots) et
complétion (24 phrases) intégrées pour l'**algérien (dz) uniquement**,
fournis par l'utilisateur (5 fichiers Excel), dans
`js/exercises-dz.js`. Il manque encore la compréhension (18
questions, voir `docs/dz-parity-request.md`). Marocain (ma) et
tunisien (tn) n'ont toujours aucun contenu d'exercices — chantier
séparé de l'interface, ne peut pas être improvisé (garde-fou n°3).

`js/i18n.js`, blocs `dz`/`ma`/`tn` dans `I18N_STRINGS`.

**Pourquoi une prudence plus forte que pour le kabyle** : l'arabe
dialectal maghrébin est avant tout parlé, avec une variation
régionale importante même à l'intérieur d'un seul pays, et un mélange
constant avec le français — risque réel de produire quelque chose qui
sonne "arabe standard à peine modifié" plutôt qu'un vrai dialecte.
Différenciation
réelle recherchée entre les trois (pas trois copies du même texte) :
marqueurs possessifs distinctifs — تاع (algérien), ديال (marocain),
متاع (tunisien) — et vocabulaire différent pour "maintenant" (دروك/
دابا/توا). **Aucun des trois n'a été relu par un∙e locuteur∙rice
natif∙ve** — premier jet à considérer comme tel avant tout usage
clinique réel, y compris pour les nouvelles clés du lot 1 et des lots
suivants.

`speechLocale:null` pour les trois : aucun navigateur ne propose de
voix dédiée à ces dialectes (contrairement à l'arabe standard déjà
présent avec `ar-SA`).

**Toujours pas de contenu d'exercices** (contrairement au japonais) —
et ceci **ne change pas** avec l'extension de l'interface : aucune
banque `exercises-dz.js`/`exercises-ma.js`/`exercises-tn.js`. Les
écrans d'exercices resteront en français pour dz/ma/tn même une fois
l'interface à 100 %, tant qu'aucun vocabulaire source n'est fourni
(même principe que le fichier Excel japonais) — ce contenu ne peut pas
être inventé, contrairement aux clés d'interface (garde-fou n°3).
...
- **Compléter les infos réelles sur les pages légales** (v6.88) —
  `mentions-legales.html` et `confidentialite.html` contiennent
  encore de vrais champs "à compléter" (identité/raison sociale,
  adresse postale, email de contact, SIRET le cas échéant, durée de
  conservation, dates de mise à jour). `politique-cookies.html`,
  `gestion-cookies.html`, `cgv.html`, `cgu.html` sont déjà rédigées
  et fonctionnelles (`gestion-cookies.html` a même un vrai outil
  "voir ce qui est stocké / tout effacer"). Ces informations
  personnelles ne peuvent venir que de vous, avant toute relecture
  juridique.
...
- **(Optionnel, pas bloquant) Contrainte `check` en base sur
  `sessions.type` et `error_events.category`** (v6.83) — ces deux
  colonnes acceptent n'importe quelle chaîne côté RPC
  (`log_session`/`log_error`), sans liste fermée de valeurs. Le risque
  XSS correspondant est déjà neutralisé côté affichage (échappement
  ajouté partout où c'est montré), donc pas urgent — mais une
  contrainte `check (type in (...))` en base ajouterait une deuxième
  couche de protection à la source. Je ne l'ai pas ajoutée moi-même
  pour éviter de deviner une liste de valeurs incomplète et casser un
  type d'exercice légitime.
- **État de sécurité — résumé pour ne pas repartir de zéro** (v6.86) :
  - Fait : RLS partout, fonctions `security definer`, MFA admin +
    orthophoniste, échappement XSS audité (v6.83), version de
    supabase-js figée (v6.86).
  - Connu et documenté comme choix assumé, pas un oubli : le code de
    suivi patient n'a ni expiration ni limite de tentatives (voir
    README.md, section authentification) ; les URLs de
    `patient-media` (bucket Storage) ne sont pas non plus protégées
    par un vrai contrôle d'accès, juste non devinables.
  - Non fait, à considérer si besoin : limite de fréquence
    (rate-limiting) sur `contribuer.html` (formulaire public, aucune
    limite de soumissions aujourd'hui) ; hash d'intégrité (SRI) sur
    les scripts chargés depuis un CDN.

- **Voix neuronales (ElevenLabs/Google/Azure) — mise de côté
  explicitement par l'utilisateur** (discuté le même jour que le
  correctif du bug de synthèse vocale, v6.92). Décision : générer une
  fois pour toutes des fichiers audio de bien meilleure qualité pour
  le contenu des exercices (même mécanisme que l'audio kabyle déjà en
  place), plutôt que la voix système parfois robotique. **À
  rappeler à l'utilisateur une fois le site en ligne** (nom de domaine
  définitif, `reparole.fr` déjà entrevu dans les réglages Supabase) —
  demande explicite de sa part de ne pas y toucher avant.
- **Configurer un SMTP personnalisé (Resend, SendGrid...)** — le
  service d'emails par défaut de Supabase est limité à **2 emails par
  heure**, documenté comme prévu pour du test léger, pas pour un
  usage réel. Suffisant pour l'instant, mais bloquant dès les
  premiers vrais patients (confirmation d'inscription, mot de passe
  oublié...). À faire avant toute mise en ligne réelle : Supabase >
  Authentication > SMTP Settings > activer Custom SMTP avec les
  identifiants du fournisseur choisi.
- **Migration SQL pour l'historique des connexions et les erreurs
  techniques** (v6.97) — les tables `login_events`/`client_errors` et
  leurs 4 fonctions RPC sont déjà écrites dans `sql/schema.sql`, pas
  encore appliquées à votre projet Supabase. Les deux nouvelles cartes
  du tableau de bord admin resteront vides tant que cette migration
  n'est pas jouée.
- **Migration SQL pour les conseils visibles au patient** (v6.95) —
  la colonne `visible_to_patient` sur `notes` et la fonction
  `get_patient_visible_notes` sont déjà écrites dans
  `sql/schema.sql`, pas encore appliquées à votre projet Supabase.
  La case "Visible par le patient" fonctionne déjà en local (repli
  automatique) mais les conseils n'apparaîtront pas côté patient sur
  le vrai site tant que cette migration n'est pas jouée.
- **Migration SQL pour corriger `get_patient()`** (v6.93, urgent) —
  vrai bug racine : la fonction était déclarée `returns patients` (une
  ligne composite unique) au lieu de `returns setof patients`, ce qui
  faisait qu'un code inexistant renvoyait une ligne pleine de `NULL`
  plutôt qu'aucune ligne — touchait potentiellement aussi la connexion
  patient elle-même, pas seulement le rattachement côté orthophoniste.
  Déjà corrigée dans `sql/schema.sql`, mais **doit être rejouée dans
  Supabase** (SQL Editor) pour prendre effet sur le vrai projet.
- **Arabe standard (فصحى) ≠ dialecte algérien** — vérifié en
  répondant à une question directe : la traduction arabe déjà en
  place dans l'app est de l'arabe standard moderne (formel, écrit,
  compris à l'école dans tout le monde arabophone), **pas** le
  dialecte algérien parlé au quotidien (beaucoup d'emprunts au
  français/berbère, grammaire et vocabulaire vraiment différents pour
  l'usage courant). Même situation que kabyle vs arabe standard, ou
  pt-PT vs pt-BR (déjà rencontré) : si un jour une variante
  dialectale algérienne est demandée, ce serait une langue à part
  entière à construire, pas une adaptation mineure de l'arabe déjà
  présent — même principe de prudence que pour le kabyle (jamais de
  traduction inventée, relecture native nécessaire).
- **Migration SQL pour le droit à l'effacement** (v6.87) — la fonction
  `delete_patient_account` est déjà écrite dans `sql/schema.sql`, pas
  encore appliquée à votre projet Supabase. Le bouton "Supprimer mon
  compte" fonctionne déjà en local (repli automatique) mais échouera
  côté cloud tant que cette migration n'est pas jouée.
- **Piste Brésil (portugais brésilien, LGPD)** — discuté en détail :
  meilleur candidat identifié parmi Afrique francophone/Japon/Brésil
  (besoin réel documenté, paiement Stripe+Pix déjà viable, marché
  moins occupé qu'au Japon). Reste à faire si le sujet revient :
  traduction pt-BR distincte du pt-PT déjà présent (relue par un∙e
  locuteur∙rice brésilien∙ne natif∙ve, même principe que le kabyle),
  et une vraie consultation LGPD avec un∙e avocat∙e brésilien∙ne à
  distance — pas besoin de structure locale ni de déplacement.
- **Dossier de préparation réglementaire (ISO 13485, ISO 14971,
  marquage CE, dossier technique MDR)** — objectif explicite à long
  terme si l'activité devient rentable, pas pour maintenant. Voir
  `PREPARATION-REGLEMENTAIRE.md` : état des lieux + registre de
  risques préliminaire, PAS une certification. Rappel important :
  ReParole n'est probablement PAS un dispositif médical au sens du
  MDR aujourd'hui (son positionnement "aide à patienter, ne remplace
  pas le rendez-vous" le maintient hors de ce champ) — la première
  vraie décision à prendre le jour venu est de choisir consciemment
  si on veut y rester ou basculer vers une finalité médicale, avant
  d'engager le moindre budget de certification.
...
2. **Pas d'exercices de déglutition/dysphagie.** Risque réel de fausse
   ...
   soignante.
3. **Contenu en langue partielle (kabyle, dialectes maghrébins) :
   uniquement du
...
- Relecture native du kabyle et des 3 dialectes maghrébins au-delà du
  vocabulaire déjà sourcé.
...
├── PREPARATION-REGLEMENTAIRE.md ← dossier de préparation ISO 13485/14971/MDR
│                                  (PAS une certification — état des lieux +
│                                  registre de risques préliminaire, pour le
│                                  jour où le budget le permettra)
├── docs/kabyle-parity-request.md, docs/kabyle-completion-draft.md, docs/{dz,ma,tn}-parity-request.md
├── css/{style.css, ortho.css, companion.css}
├── audio/kab/README.md ← comment ajouter de vrais enregistrements
├── js/
...
│   ├── exercises-kab.js, exercises-{dz,ma,tn}.js ← langues partielles
...
└── tests/ (50 fichiers, 567 tests — voir liste ci-dessous)
...
```
Doit afficher **50 suites, toutes ✅, 567 tests**. Sinon, lire le détail
