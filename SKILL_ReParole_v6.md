...
vocaux) ; kab, sg et ja (**partielles** — interface non traduite,
repli français, mécanisme commun via `PARTIAL_LANGS`). kab/sg :
dénomination sourcée uniquement, pas de synthèse/reconnaissance
vocale disponible. **ja (v6.89) fait exception dans les langues
partielles** : contenu bien plus riche (dénomination + complétion +
compréhension, 130 items, fourni par l'utilisateur) ET
synthèse/reconnaissance vocale disponible (`speechLocale:'ja-JP'`,
contrairement à kab/sg) — classée partielle uniquement parce que
l'interface n'est pas traduite et le contenu pas encore relu par
un∙e natif∙ve. Voir "Sango", "Kabyle" et "Japonais" ci-dessous pour
...
## Japonais (langue partielle, mais contenu riche) — état précis (v6.89)

`js/exercises-ja.js` : 130 items au total (92 mots de dénomination sur
3 niveaux + 24 phrases de complétion + 18 questions de compréhension),
fournis intégralement par l'utilisateur via `Japonais_Complet.xlsx`.

**Remarque importante** : structure identique (92+24+18) à celle
demandée pour la parité sango (`Sango.txt`, jamais intégré à cause de
mots suspects ressemblant à du français à peine modifié — voir
ci-dessus). Vérification faite par échantillonnage manuel d'une
vingtaine de mots japonais contre mes propres connaissances de la
langue : vocabulaire standard et correct (犬/inu=chien, 牛乳/gyuunyuu=
lait, etc.), grammaire des phrases de complétion cohérente — pas
d'indice de mots fabriqués comme pour le sango. Reste néanmoins **non
relu par un∙e locuteur∙rice natif∙ve ou un∙e professionnel∙le de
santé japonais∙e** — même statut que tout contenu non validé
cliniquement (garde-fou n°8).

Distracteurs (choix multiples) générés automatiquement par mes soins,
mais exclusivement à partir du vocabulaire déjà fourni (jamais de mot
japonais inventé) — script de génération documenté en tête de
`js/exercises-ja.js`. `speechLocale:'ja-JP'` actif (contrairement au
kabyle/sango) : synthèse/reconnaissance vocale du navigateur
disponibles pour le japonais.

**Limitation connue** : lecture hiragana/rōmaji fournie dans le
fichier source mais pas encore affichée dans l'interface (pas de
furigana) — demanderait une extension du moteur d'affichage des choix
sans casser la comparaison réponse/choix déjà en place.
...
└── tests/ (40 fichiers, 479 tests — voir liste ci-dessous)
...
```
Doit afficher **40 suites, toutes ✅, 479 tests**. Sinon, lire le détail
