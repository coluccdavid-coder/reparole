...
4. **"Espace aidant" (proche du patient)** : l'utilisateur veut un vrai
...
│   └── plan-and-mfa.test.js       ← 18 tests : gratuit/pro, MFA (faux Supabase), paiement (faux fetch)
└── sql/schema.sql             ← RLS réelle, fonctions RPC, colonnes plan/stripe_*

npm install && npm test  →  doit afficher 4 suites, toutes ✅, avant toute livraison
```
