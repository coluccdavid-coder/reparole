...
1. **Rien de cassé ni de bloquant** — la dernière session s'est terminée
   proprement (12 suites de tests automatisés au vert, 245 tests, les
   4 pistes proposées par l'utilisateur toutes terminées : séance
   courte, journal, populations cliniques, mots personnalisés aidant —
   v6.41/v6.42/v6.43).
2. **Toujours en attente de l'utilisateur** (je ne peux pas le faire à sa
   ...
   - **Nommer au moins un·e administrateur·rice** (v6.38, procédure
     ...
     s'accumulent sans jamais être relues. Dépend aussi du vrai compte
     Supabase (point ci-dessus).
