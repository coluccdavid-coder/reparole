...
1. **Rien de cassé ni de bloquant** — la dernière session s'est terminée
   proprement (15 suites de tests automatisés au vert, 267 tests).
   ...
   **v6.57/v6.58 — EN COURS : ajout du sango, 19/22 mots confirmés.**
   Deux liens fournis par l'utilisateur (centrafrique.sango.free.fr,
   article Persée) + recherches complémentaires (dictionnaire SIL/
   Webonary, base académique) + 4 mots confirmés par sa personne
   sango-phone. **Il ne reste que 3 mots à trancher** (voiture, bœuf,
   olive/olivier — voir `docs/sango-translation-request.md`). Rien à
   coder tant que ces 3 réponses ne sont pas revenues — dès qu'elles le
   sont, construire `js/exercises-sango.js` sur le modèle exact de
   `js/exercises-kab.js`.
...
npm install && npm test  →  doit afficher 15 suites, toutes ✅ (267 tests), avant toute livraison
```
...
```
Doit afficher 15 suites toutes ✅ (21 + i18n/pwa + 21 + 107 + 15 + 4 + 17 + 9 + 5 + 9 + 11 + 4 + 5 + 11 = 267 tests).
