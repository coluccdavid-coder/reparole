...
## Kabyle (langue partielle) — état précis (v6.104)

**En attente** : `docs/kabyle-symptoms-request.md` (v6.111) — 2 lots
en attente de traduction par le contact kabylophone de l'utilisateur :
(1) les 4 questions du questionnaire "Vos ressentis"
(`SYMPTOM_QUESTIONS` dans `js/assessment.js`), (2) les 6 phrases du
mini-test d'accueil pour les domaines complétion/compréhension
(`ASSESS_ITEMS` — la dénomination est déjà faite,
`ASSESS_ITEMS_KAB.denomination`). Volontairement pas fait par
moi-même : phrases complètes à conjuguer, et le kabyle (berbère) est
structurellement trop différent de l'arabe dialectal pour que j'aie la
même confiance. Une fois reçu : ajouter
`window.SYMPTOM_QUESTIONS_KAB` + `completion`/`comprehension` à
`ASSESS_ITEMS_KAB`, tout dans `js/assessment.js`.
