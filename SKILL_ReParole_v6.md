...
1. **Rien de cassé ni de bloquant** — la dernière session s'est terminée
   proprement (13 suites de tests automatisés au vert, 249 tests).
...
9. **Déploiement Supabase réel — pour l'essentiel TERMINÉ (v6.44-v6.46).**
   ...
   - **Ce qui reste, si l'utilisateur le demande** : nommer un·e
     ...
     cette session.
10. **v6.49 — deux leçons à ne pas oublier, trouvées en testant l'app
...
npm install && npm test  →  doit afficher 13 suites, toutes ✅ (249 tests), avant toute livraison
```
...
```
Doit afficher 13 suites toutes ✅ (21 + i18n/pwa + 19 + 105 + 15 + 4 + 17 + 9 + 5 + 9 + 11 + 4 = 249 tests).
