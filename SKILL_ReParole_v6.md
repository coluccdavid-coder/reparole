...
exercices — ja depuis la v6.91) ; kab, sg, **dz, ma, tn** (**partielles**
— socle minimal 22 clés, écran de connexion + tableau de bord de
base, repli français pour le reste). dz/ma/tn (v6.94) : dialectes
maghrébins (algérien/marocain/tunisien), ajoutés avec une prudence
plus forte encore que le kabyle — voir section dédiée plus bas. Voir
"Sango", "Kabyle", "Japonais" et "Dialectes maghrébins" ci-dessous
pour l'état précis de chacune.
...
## Japonais — langue complète depuis la v6.91

`js/exercises-ja.js` : 130 items (92 mots de dénomination sur 3
niveaux + 24 phrases de complétion + 18 questions de compréhension),
fournis intégralement par l'utilisateur via `Japonais_Complet.xlsx`
(v6.89). **Interface complètement traduite depuis la v6.91** : les
403 clés d'`I18N_STRINGS.ja`, `COMPANION_PHRASES.ja` et
`ASSESS_STRINGS.ja` — retiré de `PARTIAL_LANGS`, même statut que les
8 autres langues complètes (en/es/it/pt/de/ar/tr/pl).

**Remarque importante** : structure du contenu d'exercices identique
(92+24+18) à celle demandée pour la parité sango (`Sango.txt`, jamais
intégré à cause de mots suspects ressemblant à du français à peine
modifié — voir ci-dessus). Vérification faite par échantillonnage
manuel d'une vingtaine de mots japonais contre mes propres
connaissances de la langue : vocabulaire standard et correct, pas
d'indice de mots fabriqués comme pour le sango.
...
(le problème qui a fait rejeter `Sango.txt`). Différenciation
...
- **Voix neuronales (ElevenLabs/Google/Azure) — mise de côté
  ...
  le contenu des exercices (même mécanisme que l'audio kabyle/sango
  déjà en place), plutôt que la voix système parfois robotique. **À
