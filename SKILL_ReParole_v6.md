...
## Dialectes maghrébins (algérien, marocain, tunisien) — état précis (v6.94)

`js/i18n.js`, blocs `dz`/`ma`/`tn` dans `I18N_STRINGS` : 22 clés
chacun (écran de connexion + tableau de bord de base), même périmètre
exact que le kabyle. Ajoutés en langues **partielles**
(`PARTIAL_LANGS`), pas complètes.
...
**Pas de contenu d'exercices** (contrairement au japonais) : seule
l'interface de base existe, aucune banque `exercises-dz.js`/
`exercises-ma.js`/`exercises-tn.js`. Si un jour l'utilisateur fournit
du vocabulaire source pour l'un de ces dialectes (même principe que
le fichier Excel japonais), le même mécanisme pourra être réutilisé.
...
└── tests/ (48 fichiers, 540 tests — voir liste ci-dessous)
...
```
Doit afficher **48 suites, toutes ✅, 540 tests**. Sinon, lire le détail
