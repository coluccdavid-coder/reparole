...
## v6.28 — audit de la charte d'Ami + parcours aidant (6 juillet)

Suite au message de l'utilisateur définissant explicitement le
personnage d'Ami ("Ami n'est pas une mascotte. Ami est un compagnon...")
et demandant 3 chantiers un par un avec test avant de passer au suivant :
parcours aidant, audit d'Ami, bilan PDF pro. Fait dans cet ordre-là.

**Audit d'Ami vs la charte** : relecture complète de `js/companion.js`
(7 langues, tous les contextes). Aucune violation trouvée — les
garde-fous déjà en place depuis la v6 (jamais de "faux/raté/échec",
toujours bienveillant même sur un score bas) couvraient déjà l'essentiel
de la charte. Formalisé :
- La charte est maintenant écrite explicitement en tête de
  `js/companion.js` (mots de l'utilisateur, pas paraphrasés).
- Nouveau test `tests/companion-charter.test.js` : scanne chaque phrase
  des 7 langues contre une liste de mots bannis par langue (faux, raté,
  nul, wrong, fail, sbagliato, falsch, خطأ...). Filet de sécurité contre
  une régression future, pas une preuve de compréhension du sens — reste
  une heuristique par mots-clés.

**Parcours aidant** (nouveau, `aidant.html`) : troisième parcours à côté
du patient et de l'orthophoniste. Un proche entre le code de suivi du
patient (même code, pas de nouveau système de comptes) et voit, en
LECTURE SEULE : séances/réussite/niveau/tendance (mêmes stats que le
patient), et une liste de conseils de communication non cliniques
(laisser le temps de répondre, phrases courtes, valoriser l'effort...).
Multilingue (contrairement à `report.html`, réservé à l'ortho). Ami
accueille l'aidant avec un registre dédié (`aidant_welcome` dans
`js/companion.js`, différent de `welcome` côté patient). Lien ajouté sur
l'écran de connexion (`index.html`), à côté du lien orthophoniste.

Ajout technique : `I18N.list(key)` (dans `js/i18n.js`) renvoie le
tableau COMPLET d'une clé de traduction (les conseils aidant, affichés
tous en même temps), contrairement à `I18N.rand()` qui n'en tire qu'un
au hasard (utilisé pour les phrases d'Ami).

**Nouveau test** `tests/aidant.test.js` (6 tests) : code manquant/inconnu
gérés sans plantage, stats affichées correctement, conseils affichés EN
ENTIER (pas un seul au hasard), changement de langue depuis cette page
retraduit tout, réinitialisation propre du bouton "voir un autre code".
**`CACHE_NAME` incrémenté à `'reparole-v6-30'`** (nouveau fichier
`aidant.html` + nouvelles clés i18n + companion.js).

**Non fait / hors de portée, comme pour tout le reste de l'i18n** :
traductions des conseils aidant (7 langues) pas relues par un locuteur
natif. Les conseils eux-mêmes sont génériques (communication, pas
clinique) — cohérent avec le garde-fou n°8 (jamais d'avis médical).

## v6.29 — bilan PDF professionnel (6 juillet, 3ᵉ chantier de la série)

Dernier des 3 chantiers demandés un par un (parcours aidant → audit
Ami → bilan PDF). `report.html` (réservé à l'orthophoniste, resté
français uniquement) retravaillé pour se lire comme un vrai document
clinique à archiver, pas un export d'appli :
- Bandeau discret "ReParole — suivi de rééducation" au-dessus du titre
  (qui devient "Bilan de suivi orthophonique", plus clinique que
  l'ancien "Rapport de suivi").
- Bloc identité à deux colonnes : Patient (nom + code) / Réalisé par
  (nom de l'orthophoniste — nouveau paramètre `orthoName` dans l'URL,
  ajouté par `dashboard-ortho.js` à la génération du lien ; repli
  discret "Non renseigné" si absent, ex. lien généré par une ancienne
  version en cache — jamais de plantage).
- Le profil clinique déclaré (Type Broca, Wernicke...), jusqu'ici
  invisible en dehors du tableau de bord ortho, s'affiche maintenant
  sur le document lui-même quand il est renseigné (rien n'apparaît si
  "Non précisé").
- Ligne de signature en bas ("Date" / "Signature et cachet du
  professionnel") — un document qu'on imprime pour l'archiver doit
  pouvoir être signé.
- CSS d'impression : `@page` avec marges correctes (A4, 16mm/14mm),
  `page-break-inside:avoid` sur les sections et les lignes du tableau
  d'historique pour éviter qu'une section soit coupée au milieu d'une
  page.

**Pas fait / hors de portée** : logo ou en-tête personnalisé du cabinet
(nécessiterait un nouveau champ dans le profil ortho — pas demandé,
piste possible plus tard si besoin).

**Nouveau test** `tests/report-professional.test.js` (8 tests) : titre +
bandeau présents, nom de l'orthophoniste affiché (ou repli si absent),
profil clinique affiché seulement s'il est renseigné, ligne de signature
présente, régressions déjà couvertes avant (code manquant/inconnu)
toujours vertes. **`CACHE_NAME` incrémenté à `'reparole-v6-31'`**
(`js/dashboard-ortho.js` modifié, dans l'app shell).

## v6.30 — bug de persistance du "plan" + déverrouillage QA (6 juillet)

Suite à une capture d'écran montrant l'écran "Fonctionnalité Pro"
verrouillé, l'utilisateur a demandé soit un compte de test, soit un
système de paiement avec une carte bancaire factice pour continuer à
tester. **Refusé de construire un faux formulaire de carte bancaire**
(discussion avec l'utilisateur) : ça ressemblerait à un vrai écran de
paiement, trompeur même en test, a fortiori sur une app de santé.

**Bug réel trouvé en creusant** (bloquait complètement la demande) :
`Store.savePatient()`/`loadPatient()` (`js/storage.js`) ignoraient
silencieusement le champ `plan` :
- Mode cloud : `get_patient()` renvoie bien la colonne (elle fait
  `select *`), mais `loadPatient()` oubliait de la reprendre dans l'objet
  JS → un patient passé "pro" à la main dans Supabase (méthode déjà
  documentée depuis la v6.24) s'affichait quand même "free" partout.
  `upsert_patient()` n'acceptait de toute façon aucun paramètre pour
  l'écrire.
- Mode navigateur (localStorage) : `savePatient()` reconstruisait le
  dossier patient à partir d'une liste de champs qui n'incluait pas
  `plan` → le champ ne survivait à aucune sauvegarde, y compris la toute
  première (juste après la création du patient).
- **Conclusion : avant ce correctif, AUCUN patient, dans AUCUN mode,
  n'a jamais pu réellement passer "pro"**, même en éditant Supabase à la
  main — la capture d'écran de l'utilisateur en est très probablement la
  conséquence directe.

Corrigé : `plan` repris/envoyé des deux côtés (JS). Côté cloud, nécessite
en plus une migration SQL (`sql/migrations/002_plan_upsert.sql`, aussi
appliquée directement dans `sql/schema.sql` pour les nouvelles
installations) — `upsert_patient()` accepte maintenant un `p_plan`
optionnel, avec `coalesce(p_plan, patients.plan)` pour rester
rétrocompatible (un ancien client qui n'enverrait pas ce paramètre ne
réinitialise pas le plan existant à 'free').

**Déverrouillage QA ajouté** (`js/app.js`, écran "Fonctionnalité Pro") :
un champ + bouton "Code de test QA" (`QA_UNLOCK_CODE = 'REPAROLE-QA-PRO'`
dans `js/app.js`), clairement étiqueté comme outil de test — PAS un
système de paiement. ⚠️ Documenté explicitement dans le code : ce n'est
pas un mécanisme de sécurité (visible dans le JS servi au navigateur), à
retirer ou remplacer par un vrai contrôle serveur avant de brancher un
vrai système de paiement.

**Pour débloquer le Pro sur le déploiement actuel de l'utilisateur** :
1. Exécuter `sql/migrations/002_plan_upsert.sql` dans Supabase (SQL
   Editor) — nécessaire pour que la persistance fonctionne en mode
   cloud.
2. Sur l'écran verrouillé, entrer le code `REPAROLE-QA-PRO`. (Ou, comme
   avant, modifier `plan` directement dans la table `patients` via
   Supabase — ça fonctionne maintenant réellement, contrairement à
   avant.)

**Nouveaux tests** : `tests/plan-persistence.test.js` (3 tests, mode
cloud avec faux client Supabase) et `tests/qa-unlock.test.js` (4 tests :
code invalide refusé, bon code débloque + persiste, exercice
précédemment verrouillé devient accessible). **`CACHE_NAME` incrémenté à
`'reparole-v6-32'`**.

## v6.30 — bug réel : le mode Pro ne s'activait jamais + déverrouillage QA (6 juillet)

Capture d'écran de l'utilisateur montrant l'écran "Fonctionnalité Pro"
verrouillé, avec la demande de créer un compte de test ou un système de
paiement factice pour continuer les tests.

**Refusé volontairement** : construire un faux formulaire de carte
bancaire. Même en test, ça ressemble à un vrai écran de paiement — sur
une app de santé, ce genre de fausse UI de paiement est trompeur et à
éviter, même en interne. Proposé et fait à la place : un code de test
QA, clairement étiqueté comme tel.

**Bug réel trouvé en implémentant ce code de test** (probablement LA
cause du screenshot) : `Store.savePatient()`/`loadPatient()`
(`js/storage.js`) ignoraient silencieusement le champ `plan` :
- En mode cloud, `get_patient()` renvoie bien la colonne `plan` (la
  fonction fait `select *`), mais le mapping JS de la ligne vers un
  objet, dans `loadPatient()`, oubliait de la reprendre → un patient
  passé "pro" à la main dans Supabase (méthode documentée depuis la
  v6.24) s'affichait quand même comme "free" partout dans l'app, tout
  le temps. C'est très probablement exactement ce qui est arrivé ici.
- `upsert_patient()` (SQL) n'acceptait de toute façon aucun paramètre
  pour le plan — aucun mécanisme dans l'app n'aurait pu l'écrire, même
  après avoir corrigé la lecture.
- En mode navigateur (localStorage), `savePatient()` reconstruisait un
  "record" qui omettait aussi `plan` à chaque sauvegarde.

**Corrigé** :
- `js/storage.js` : `loadPatient()` reprend `row.plan||'free'` (cloud) ;
  `savePatient()` inclut `plan` dans le record local ET envoie `p_plan`
  au RPC cloud.
- **Nouvelle migration SQL À EXÉCUTER dans Supabase** :
  `sql/migrations/002_plan_upsert.sql` — `upsert_patient()` accepte
  maintenant `p_plan` (paramètre optionnel, `coalesce()` avec la valeur
  existante si absent, pour rester rétrocompatible avec un déploiement
  JS/SQL non simultané). `sql/schema.sql` mis à jour aussi, pour que les
  nouvelles installations aient directement la version corrigée.
- **Sans exécuter cette migration, le compte cloud restera bloqué en
  "free"** quoi qu'on fasse côté app (le déverrouillage QA fonctionnera
  seulement pour la session en cours, sans persister, tant que
  l'ancienne fonction SQL est en place).

**Déverrouillage QA du mode Pro** (`js/app.js`) : sur l'écran de
verrouillage ("Fonctionnalité Pro"), un champ + bouton permet d'entrer
un code de test (`REPAROLE-QA-PRO`, visible dans le code source — ⚠️ pas
un mécanisme de sécurité, juste un outil QA — voir le commentaire
`QA_UNLOCK_CODE` dans `js/app.js`, à retirer avant de brancher un vrai
système de paiement). Traduit dans les 7 langues.

**Nouveaux tests** : `tests/plan-persistence.test.js` (3 tests, mode
cloud simulé) et `tests/qa-unlock.test.js` (4 tests). **`CACHE_NAME`
incrémenté à `'reparole-v6-32'`**.

## v6.31 — bug réel : "Niveau adapté : level_null" (6 juillet)

Capture d'écran de l'utilisateur (compte de test "toto") montrant
`Niveau adapté : level_null` sur le tableau de bord — la clé de
traduction brute affichée au lieu d'un nom de niveau. Cause : un dossier
avec `level` manquant/invalide (null, undefined, ou hors de {1,2,3})
n'était jamais protégé par les fonctions `levelName()` (et équivalents),
qui renvoyaient tel quel ce que `I18N.t()` retourne pour une clé
introuvable : la clé elle-même. Origine exacte du `level=null` sur ce
dossier précis non confirmée avec certitude (probablement un dossier de
test créé/modifié directement dans Supabase plutôt que via le vrai
parcours de bilan, qui ne produit lui-même toujours que 1, 2 ou 3).

**Corrigé à deux niveaux, pas seulement cosmétique** :
1. Filet défensif à l'affichage, dans les 5 endroits qui affichent un
   nom de niveau (`js/app.js`, `js/dashboard-ortho.js`, `report.html`,
   `patient-report.html`, `aidant.html`) : un niveau hors de {1,2,3}
   retombe sur 2 (Intermédiaire).
2. **Réparation du dossier lui-même à la connexion** (`login()`,
   `js/app.js`) : si `user.level` n'est pas dans {1,2,3}, il est corrigé
   à 2 puis re-sauvegardé — le bug ne doit plus jamais réapparaître pour
   ce dossier après une connexion.

**Pour corriger tout de suite un dossier déjà affecté sans attendre une
connexion** (ex. "toto") — SQL à lancer dans Supabase si besoin :
```sql
update patients set level=2 where level is null or level not in (1,2,3);
```

**Nouveau test** `tests/level-repair.test.js` (4 tests) : niveau null
affiché correctement + réparé, niveau invalide (ex. 99) réparé et
persisté, niveau valide jamais modifié. **`CACHE_NAME` incrémenté à
`'reparole-v6-33'`**.

## v6.32 — audit complet du site + quota gratuit réparé (6 juillet)

Demande explicite de l'utilisateur : "examine tout le site et vérifie
tout ce qui va et ne va pas." Audit systématique effectué :

**Vérifié et confirmé sain** : cohérence RPC JS ↔ SQL (chaque appel a sa
fonction), toutes les clés `data-i18n` du HTML existent bien dans
`i18n.js`, aucun fichier référencé manquant, les 7 langues à support
complet ont exactement 60 exercices chacune, le mode dyslexie l'emporte
bien sur le changement de police Playfair Display (spécificité CSS
vérifiée), et le verrouillage gratuit/pro est bien vérifié dans les 3
endroits qui pouvaient sembler y échapper (mémoire, tenue vocale,
conversation guidée — un doute levé, pas un bug).

**Un vrai bug trouvé et corrigé (confirmé avec l'utilisateur avant
d'y toucher, vu la migration SQL nécessaire)** : le quota de 5 séances
gratuites/jour ne se sauvegardait NULLE PART — ni localStorage, ni
Supabase. Il vivait uniquement dans les variables en mémoire
`user.dailySessionsDate`/`user.dailySessionsCount`. Un compte gratuit
qui rechargeait la page ou se reconnectait repartait avec un quota
neuf : la limite ne protégeait quasiment rien en pratique.

**Corrigé** :
- `recordDailySession()` (`js/app.js`) sauvegarde maintenant le
  patient après incrément (fire-and-forget, pas de `await` — appelée
  depuis 4 endroits dont certains non-async : mémoire, tenue vocale,
  conversation guidée, moteur d'exercice principal).
- `js/storage.js` : `loadPatient()`/`savePatient()` lisent/écrivent
  `dailySessionsDate`/`dailySessionsCount`, en local ET en cloud.
- **Nouvelle migration SQL À EXÉCUTER dans Supabase** :
  `sql/migrations/003_daily_quota.sql` — ajoute les colonnes
  `daily_sessions_date`/`daily_sessions_count` à `patients`, et met à
  jour `upsert_patient()` pour les accepter (mêmes garde-fous de
  rétrocompatibilité que `002_plan_upsert.sql` — `coalesce()` pour ne
  pas réinitialiser le quota si un ancien client appelle sans ces
  paramètres). `sql/schema.sql` mis à jour aussi pour les nouvelles
  installations.
- **Sans cette migration, le quota restera non persistant en mode
  cloud** (comme pour "plan" avant la migration 002).

**Nouveaux tests** : `tests/daily-quota.test.js` (2 tests, mode
navigateur — persistance + quota réellement appliqué à la 6ᵉ séance) et
extension de `tests/plan-persistence.test.js` (+3 tests, mode cloud
simulé). **`CACHE_NAME` incrémenté à `'reparole-v6-35'`**.

## v6.33 — bulle d'aide au survol/appui long (6 juillet)

Demande explicite de l'utilisateur, après confirmation qu'Ami bouge
déjà en vrai (les captures d'écran ne montrent jamais les animations —
jambes, clignement des yeux, étincelle, léger mouvement continu, tout
existe déjà depuis des versions antérieures) : une bulle d'aide qui
explique les icônes/boutons peu clairs au survol (souris) ou à l'appui
long (tactile), sans déclencher l'action du bouton par erreur.

**Nouveau fichier `js/help-tooltip.js`** : mécanisme générique par
délégation d'événements sur `document` — n'importe quel élément avec
`data-help="clé_i18n"` en profite automatiquement, y compris ajouté
dynamiquement après coup (le widget Ami est re-rendu à chaque
changement d'humeur). Comportement :
- Souris : survol 350ms -> bulle. Départ avant le délai -> jamais affichée.
- Clavier : focus -> bulle immédiate (accessibilité) ; blur -> masquée.
- Tactile : appui maintenu 500ms -> bulle affichée ET l'action du
  bouton annulée pour cet appui (`preventDefault()`) ; un tap normal
  déclenche l'action comme toujours, sans jamais montrer la bulle.
- Échap, ou clic ailleurs sur la page : masque la bulle ouverte.

**Appliqué à 4 endroits jugés "peu clairs"** (patient uniquement pour
l'instant — pas encore fait côté ortho/aidant) :
- Les 5 badges "VOIX" (exercices avec micro).
- Les 2 boutons d'accessibilité ("Lecture facilitée", "Police adaptée"),
  présents 2 fois (écran de connexion + tableau de bord).
- Le widget Ami lui-même ("C'est Ami, votre compagnon...").

Nouvelles clés i18n (7 langues) : `help_voice_badge`,
`help_dyslexia_toggle`, `help_dysfont_toggle`, `help_companion`.

**Nouveau test** `tests/help-tooltip.test.js` (6 tests) : survol
prolongé affiche, départ rapide n'affiche jamais, focus/blur clavier,
tap normal n'affiche jamais et ne bloque pas l'action, appui long
affiche et bloque l'action, Échap masque. **`CACHE_NAME` incrémenté à
`'reparole-v6-36'`**.

**Pas fait / piste ouverte** : étendre `data-help` à
`dashboard-ortho.html` et `aidant.html` si l'utilisateur le souhaite —
le mécanisme (`js/help-tooltip.js`) est générique et prêt, il suffirait
de charger le script + ajouter les attributs `data-help` sur les
éléments concernés de ces pages.
...
├── report.html                ← v6.29 : bilan PDF professionnel (orthophoniste, français uniquement)
├── patient-report.html        ← v6.27 : résumé imprimable côté PATIENT (multilingue, simplifié)
├── aidant.html                ← v6.28 : parcours aidant (lecture seule, multilingue)
...
│   ├── companion-charter.test.js  ← v6.28 : charte d'Ami (mots bannis, 7 langues)
│   ├── aidant.test.js             ← v6.28 : parcours aidant (lecture seule, multilingue)
│   ├── report-professional.test.js ← v6.29 : bilan PDF professionnel (identité, profil clinique, signature)
│   ├── plan-persistence.test.js   ← v6.30 : persistance du champ "plan" en mode cloud
│   └── qa-unlock.test.js          ← v6.30 : déverrouillage QA du mode Pro (pas un paiement)
