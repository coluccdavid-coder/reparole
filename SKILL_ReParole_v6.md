...
### Ce qui est fait mais PAS ENCORE VÉRIFIÉ (à faire en priorité à la reprise)
- **Limite de patients côté orthophoniste** (`js/dashboard-ortho.js`,
  `ORTHO_FREE_PATIENT_LIMIT = 3`) : code écrit dans `assign()`, mais
  **jamais testé**, même par simulation. À vérifier avant de considérer
  que c'est fini.
- **Mot de passe renforcé** (`OrthoApp._checkPasswordStrength()`) :
  8 caractères + minuscule + majuscule + chiffre. Code écrit, jamais
  testé.
- **Double authentification TOTP complète** (le plus gros morceau) :
  - `js/storage.js` : `mfaEnroll()`, `mfaChallenge()`, `mfaVerify()`,
    `mfaListFactors()`, `mfaUnenroll()`, et `signInOrtho()` modifié pour
    détecter si un défi MFA est nécessaire (`getAuthenticatorAssuranceLevel`)
    et renvoyer `{mfaRequired:true, factorId, challengeId}` plutôt que de
    terminer la connexion. Nouvelle fonction `completeMfaSignIn()`.
  - `js/dashboard-ortho.js` : écran de défi (`submitMfaCode`,
    `cancelMfaChallenge`), et carte "Sécurité du compte" avec
    inscription (`startMfaEnroll`, `confirmMfaEnroll`) et désactivation
    (`disableMfa`, `refreshMfaStatus`).
  - `dashboard-ortho.html` : nouvel écran `#ortho-mfa-challenge`
    (code à 6 chiffres à la connexion) + carte "Sécurité du compte"
    dans `#ortho-list` (QR code, clé secrète, statut).
  - **J'ai commencé à tester ce flux avec un faux client Supabase
    simulé** (`window.supabase.createClient` mocké, imitant l'API MFA
    documentée), mais le test avait un bug **dans le test lui-même**,
    pas dans le code de l'app : `dom.window.Store` n'existe pas —
    le bon nom est `dom.window.ReParoleStore` (voir
    `js/storage.js` : `window.ReParoleStore = ReParoleStore;`, et
    `js/dashboard-ortho.js` ligne ~14 : `const Store =
    window.ReParoleStore;` — un alias LOCAL à ce fichier, pas exposé
    sur `window`, même piège de portée JS que plusieurs fois dans cette
    session). **Action à la reprise** : relancer un test similaire en
    utilisant `dom.window.ReParoleStore.signUpOrtho(...)` etc.
    directement, ou en injectant le scénario de test DANS le même
    `eval()` que `storage.js`. Une fois ce test passé, câbler la même
    vérification que style `tests/i18n-completeness.test.js` n'est pas
    nécessaire ici (ce n'est pas du contenu multilingue), mais un test
    manuel réel avec un vrai compte Supabase reste indispensable avant
    de livrer cette fonctionnalité comme "terminée".
...
disparaître partout, pas juste ici. Incohérence trouvée et corrigée au
