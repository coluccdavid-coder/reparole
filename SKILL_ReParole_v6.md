...
1. **Rien de cassé ni de bloquant** — la dernière session s'est terminée
   proprement (12 suites de tests automatisés au vert, 245 tests).
   **Important : `js/storage.js` contient maintenant les VRAIES clés
   Supabase de l'utilisateur en dur** (URL + clé `anon public`), à sa
   demande explicite — voir point 9 pour le détail complet et un vrai
   ...
   **Police Fraunces (J/f de "Bonjour !") : `opsz 17` est la bonne
   valeur, `opsz 144` était une erreur (v6.31, corrigée en v6.47)** —
   l'axe "wonk" de Fraunces (formes excentriques) s'active
   automatiquement dès que `opsz` dépasse 18 (documenté par le
   créateur de la police, Undercase Type, sur GitHub). Ne pas remonter
   cette valeur sans une bonne raison sourcée.
2. **Toujours en attente de l'utilisateur** (je ne peux pas le faire à sa
   place, comme pour GitHub) :
   - **Supabase — pour l'essentiel FAIT (v6.44-v6.46), voir point 9 pour
...
6. **Base de connaissances communautaire (v6.38)** : terminée et
...
7. **Ajout de 6 langues européennes, EN COURS (v6.40 = 2/6 faites).**
...
8. **4 pistes proposées par l'utilisateur en réponse à "quelle idée
...
9. **Déploiement Supabase réel — pour l'essentiel TERMINÉ (v6.44-v6.46).**
   ...
   - **Ce qui reste, si l'utilisateur le demande** : nommer un·e
     administrateur·rice (point 2) pour que `admin.html` serve à
...
│   ├── app.js                 ← moteur d'exercices + structure gratuit/pro (PAYWALL_ENABLED=false) + tarifs + séance courte + journal + normalize() Unicode-aware
...
npm install && npm test  →  doit afficher 12 suites, toutes ✅ (245 tests), avant toute livraison
```
...
```
Doit afficher 12 suites toutes ✅ (21 + i18n/pwa + 19 + 105 + 15 + 4 + 17 + 9 + 5 + 9 + 11 = 245 tests).
