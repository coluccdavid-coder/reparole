...
function loadApp(){
  ...
  for(const src of scripts){
    ...
    // v6.248 : depuis le découpage v6.247, les pages ne chargent que le
    // français — les 13 autres langues sont dans js/i18n/<code>.js,
    // chargées à la demande. Ici, il FAUT toutes les charger, sinon
    // I18N.t() retombe sur le français et le manifeste capture des
    // chaînes mi-françaises (« Citez le plus possible de… palabras que
    // empiezan por B ») : des fichiers audio bilingues auraient été
    // générés. Cassé silencieusement par le découpage, réparé ici.
    if(src === 'js/i18n.js'){
      code += '\n' + fs.readdirSync(path.join(ROOT, 'js/i18n'))
        .filter(f => f.endsWith('.js')).sort()
        .map(f => fs.readFileSync(path.join(ROOT, 'js/i18n', f), 'utf8'))
        .join('\n');
    }
...
async function main(){
  ...
  for(const lang of LANGS){
    ...
    // Conversation guidée — v6.165, trouvé lors de l'audit complet
    // demandé par l'utilisateur. Le bouton "réécouter" de chaque étape
    // (js/conversation.js, "conv_replay") lit step.ai à voix haute —
    // jamais couvert par ce script jusqu'ici, même trou que
    // l'acalculie avant sa correction (v6.164). Retombe sur
    // CONV_SCENARIOS (français) si CONV_SCENARIOS_XX n'existe pas pour
    // cette langue — même mécanisme que getConvScenarios() en
    // production, donc les mêmes textes que ce que verrait réellement
    // un patient dans cette langue.
    try{
      const convKey = lang==='fr' ? 'CONV_SCENARIOS' : ('CONV_SCENARIOS_'+lang.toUpperCase());
      const scenarios = dom.window[convKey] || dom.window.CONV_SCENARIOS;
      Object.values(scenarios).forEach(scenario=>{
        (scenario.steps||[]).forEach(step=>{ if(step && step.ai) set.add(step.ai); });
      });
    }catch(e){}
