...
**⚠️ DÉPASSÉ depuis la v6.104.** Les 24 phrases de complétion (et les 18
questions de compréhension) sont maintenant intégrées dans
`js/exercises-kab.js` (`BANK_KAB.completion`/`BANK_KAB.comprehension`),
traduites/relues par une personne kabylophone (fichier
Kabyle_Complet.xlsx fourni par l'utilisateur) — un vrai statut de
relecture native, contrairement aux 4 phrases ci-dessous qui n'avaient
que des correspondances de corpus, jamais une relecture par un∙e
locuteur∙rice natif∙ve. Conservé ci-dessous pour l'historique
uniquement, plus une piste à suivre.
