...
**Cache des voix (v6.244).** Les mp3 ne sont PAS dans le cache de version.
Ils vivent dans `AUDIO_CACHE = 'reparole-audio-v1'` (voir `sw.js`), que
l'évènement `activate` épargne : sans ça, incrémenter `CACHE_NAME` faisait
re-télécharger toutes les voix à chaque déploiement. Rien à invalider :
un mp3 porte le hash de son texte, un texte modifié donne un nouveau nom.
Ne renommez `AUDIO_CACHE` que si le contenu d'un fichier existant change
vraiment — un test refuse d'ailleurs qu'il contienne un numéro de version.

**Langues concernées.** Seules les 10 langues de `CLOUD_VOICE_LANGS`
(`js/app.js`, alignées sur `LANGUAGE_CODES` du script de génération) ont
des voix pré-générées. dz, ma, tn et kab passent directement par la
synthèse du navigateur ; leurs dossiers `audio/` ne contiennent que des
enregistrements humains éventuels, joués par un autre chemin
(`playPartialLangWordUI`). Si une de ces langues reçoit un jour des voix
cloud, il faut l'ajouter aux DEUX listes — le test le vérifie.
