...
**v6.87 : le droit d'effacement et le droit de portabilité sont
désormais réellement exerçables dans l'app**, pas seulement mentionnés
ici — un vrai manque trouvé et corrigé (avant cette version, le texte
ci-dessus promettait un droit qu'aucun bouton ne permettait d'exercer) :
- **Portabilité** : bouton "📥 Télécharger mes données" sur le tableau
  de bord patient (v6.69).
- **Effacement** : bouton "🗑️ Supprimer mon compte" sur le tableau de
  bord patient (v6.87), confirmation à deux niveaux (mot à taper +
  confirmation), suppression réelle et immédiate de toutes les
  données liées au code (séances, journal, favoris, photos, notes,
  rapports — tout, via les contraintes `on delete cascade` déjà en
  place dans `sql/schema.sql`).
