...
(function(){
  function report(message, stack){
    ...
    // v6.207 : les erreurs cross-origin opaques (« Script error. » sans
    // pile ni détail, typiquement générées par des extensions de
    // navigateur) n'ont aucune valeur de diagnostic — on ne les
    // enregistre pas pour ne pas noyer les vraies erreurs dans l'admin.
    const msg = String(message || '').trim();
    if(/^Script error\.?$/i.test(msg) && !stack) return;
    try{
