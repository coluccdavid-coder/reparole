...
// v6.17 : libellés anglais des domaines — ASSESS_DOMAINS.label reste le
// texte français par défaut (repli), domainLabel() choisit selon la langue.
...
function domainLabel(key){
  ...
  if(lang==='en' && ASSESS_DOMAIN_LABELS_EN[key]) return ASSESS_DOMAIN_LABELS_EN[key];
...
function symptomQuestions(){
  const lang=(window.Prefs && Prefs.data.lang) || 'fr';
  return lang==='en' ? SYMPTOM_QUESTIONS_EN : SYMPTOM_QUESTIONS;
}
...
function assessItems(domainKey){
  const lang=(window.Prefs && Prefs.data.lang) || 'fr';
  return (lang==='en' && ASSESS_ITEMS_EN[domainKey]) ? ASSESS_ITEMS_EN[domainKey] : ASSESS_ITEMS[domainKey];
}
