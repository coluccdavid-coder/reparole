...
const Assessment = {
  ...
  _symptom(){
    ...
    this._el().innerHTML=`
      <div class="ex-header"><h2 style="font-size:1.2rem">${AS('feelings_title')}</h2><span style="color:var(--ink-soft);font-size:.9rem">${s.symptomIdx+1} / ${symptomQuestions().length}</span></div>
      ${kabNote}
      <div class="prompt-card">
        <div class="prompt-main" style="font-size:1.4rem">${qq.q}</div>
        <button class="speak-btn" onclick="speak('${qq.q.replace(/'/g,"\\'")}')">${AS('listen_question')}</button>
      </div>
  ...
  _bilanItem(){
    ...
    const domLabel = useKab ? BANK_KAB.denomination.title : domainLabel(domain.key);
    ...
    this._el().innerHTML=`
      <div class="ex-header"><h2 style="font-size:1.2rem">${domLabel}</h2><span style="color:var(--ink-soft);font-size:.9rem">${doneItems+1} / ${totalItems}</span></div>
  ...
  answerBilan(chosen,answer){
    ...
    s.itemIdx++;
    const items=assessItems(domain.key);
    setTimeout(()=>{
