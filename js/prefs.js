...
// =====================================================================
const Prefs = {
  data:{ dyslexia:false, dysFont:false, lang:'fr', shortSession:false, bigTargets:false, darkMode:false, oneHanded:false, soundFeedback:false, warmUp:false, memorySpeed:'normal', voiceURI:'' },
  ...
  load(){
    ...
    // enregistrée) — si le navigateur/appareil est configuré en kabyle ou
    // en sango, on les propose directement plutôt que de toujours partir
    // du français. Volontairement limité à PARTIAL_LANGS (kab/sg) : ce
    // sont les langues les plus difficiles à découvrir dans le sélecteur
    // manuel, alors que les autres langues déjà proposées (anglais,
    // espagnol...) n'ont pas ce problème de découvrabilité — rebasculer
    // silencieusement tout le monde vers la langue système serait plus
    // perturbant qu'utile pour un public de patients en rééducation. La
    // bannière "langue partielle" continue de s'afficher normalement dans
    // ce cas (voir apply() ci-dessous) : pas de fausse promesse de
  ...
  detectPartialLangFromBrowser(){
    try{
      const partials = (typeof PARTIAL_LANGS!=='undefined' && PARTIAL_LANGS) || ['kab','sg'];
