...
const Learner = {
  ...
  // =====================================================================
  //  v6.131 — SIGNAL DE PLATEAU
  //  ---------------------------------------------------------------------
  //  Différent de fatigueSignal() (qui détecte 2-3 échecs D'AFFILÉE dans
  //  une même séance) : ici, on regarde si le niveau (Doux/Intermédiaire/
  //  Avancé) est resté identique sur plusieurs séances consécutives,
  //  quel que soit le score de chacune. Ce n'est pas forcément un
  //  problème (la stabilité peut être voulue), donc ce signal est
  //  seulement remonté à l'espace orthophoniste — jamais présenté comme
  //  une alerte au patient, qui n'a pas à s'inquiéter d'un plateau.
  //  Seuil : 5 séances minimum au même niveau pour éviter un faux signal
  //  sur un tout début de suivi.
  // =====================================================================
  plateauSignal(history){
    if(!history || history.length<5) return { onPlateau:false, sessionsAtLevel:0 };
    const recent = history.slice(-5);
    const levels = recent.map(s=>s.level).filter(l=>l!=null);
    if(levels.length<5) return { onPlateau:false, sessionsAtLevel:0 };
    const allSame = levels.every(l=>l===levels[0]);
    if(!allSame) return { onPlateau:false, sessionsAtLevel:0 };
    // compte combien de séances d'affilée (au-delà des 5) partagent ce niveau
    let n=0;
    for(let i=history.length-1;i>=0;i--){
      if(history[i].level===levels[0]) n++; else break;
    }
    return { onPlateau:true, level:levels[0], sessionsAtLevel:n };
  },
