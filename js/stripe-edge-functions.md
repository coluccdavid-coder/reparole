...
## 5. Créer les trois Supabase Edge Functions
...
### 5c. `create-portal-session` — résiliation "en 3 clics" (v6.56)

Obligation légale française : il doit être aussi simple d'arrêter un
abonnement que d'y souscrire. Stripe propose un **Customer Portal**
tout fait (factures, moyen de paiement, résiliation) — cette fonction
se contente d'ouvrir une session vers ce portail pour le bon client,
sans qu'on ait à reconstruire cette interface nous-mêmes.

Dans Supabase : **Edge Functions** → **Create a function**, nommez-la
`create-portal-session` (rappel : le nom se tape dans le champ
"Function name" en bas de l'éditeur, pas dans le nom du fichier — le
fichier de code, lui, doit s'appeler `index.ts`) :
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!customerId) return new Response(JSON.stringify({ error: 'Compte invalide' }), { status: 400, headers })
...
**"Verify JWT"** : laissez activé, comme pour `create-checkout-session`
(l'app envoie déjà la clé anon en en-tête).

**⚠️ Étape en plus, uniquement pour celle-ci** : contrairement aux deux
premières fonctions, le Customer Portal doit être **activé/configuré
au moins une fois** côté Stripe avant de fonctionner — sinon l'appel
échoue avec une erreur du type "No configuration provided". Dans
Stripe : **Paramètres** → cherchez **"Customer portal"** (ou
**Facturation → Portail client**) → suivez l'assistant de
configuration (vous pouvez garder les réglages par défaut, ou
personnaliser ce que le patient peut faire : changer de carte,
annuler, voir les factures). Cette configuration est **séparée** entre
mode test et mode production — à refaire quand vous basculerez en
mode réel.
...
**`create-portal-session` n'a besoin d'aucun secret supplémentaire** —
les secrets sont partagés entre toutes les fonctions d'un même projet
Supabase, donc `STRIPE_SECRET_KEY` (déjà ajouté ci-dessus) suffit.
