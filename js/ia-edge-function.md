...
const DAILY_CAP = 40;
const ORTHO_TASKS = ['report_draft', 'suggest_words', 'prep_note', 'rewrite_note', 'generate_exercise', 'evolution_story', 'cabinet_digest', 'research_exercises', 'ami_journal', 'clinical_reco'];
// v6.188 : research_exercises reste aussi une tâche admin — la veille
// scientifique sert autant le cabinet que le développement du produit.
...
Réponds en TEXTE BRUT, sans mise en forme Markdown (aucun ##, aucun **),
sauf si un format JSON est explicitement exigé.`;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { task, patient_code, text, lang, days } = await req.json();
    ...
    if (ORTHO_TASKS.includes(task)) {
      // v6.187 : cabinet_digest est la seule tâche ortho SANS patient —
      // elle agrège les patients rattachés au compte (vérifiés plus bas).
      const NO_PATIENT_TASKS = ['cabinet_digest', 'research_exercises'];
      if (!NO_PATIENT_TASKS.includes(task) && !patient_code) return json({ error: 'patient_code requis' }, 400);
      if (!NO_PATIENT_TASKS.includes(task)) {
      ...
      const { data: adm } = await supa.from('admins')
        .select('code').eq('code', accountCode).maybeSingle();
    ...
    if (task === 'report_draft' || task === 'prep_note' || task === 'suggest_words' || task === 'generate_exercise' || task === 'evolution_story' || task === 'ami_journal' || task === 'clinical_reco') {
      ...
      const { data: hist } = await supa.from('sessions')
        .select('type, score, total, at').eq('code', patient_code).order('at', { ascending: false }).limit(30);
      const { data: errs } = await supa.from('error_events')
        .select('category, target').eq('code', patient_code).order('at', { ascending: false }).limit(200);
      const { data: words } = await supa.from('caregiver_words').select('word').eq('code', patient_code);
      ...
      (errs ?? []).forEach(e => {
        ...
        if (e.target) errWords[e.target] = (errWords[e.target] ?? 0) + 1;
      });
      const summary = {
        ...
        seances_recentes: (hist ?? []).map(s => ({ type: s.type, score: s.total ? Math.round(100*s.score/s.total)+'%' : null, date: (s.at||'').slice(0,10) })),
        categories_erreurs: errCounts,
        mots_souvent_en_erreur: Object.entries(errWords).sort((a,b)=>b[1]-a[1]).slice(0,15),
        mots_deja_cibles: (words ?? []).map(w => w.word),
        productions_vocales: (voice ?? []).map(v => ({ mot: v.word, verdict: v.verdict })),
        exercices_perso_deja_crees: (customs ?? []).map(c => c.title),
      };
      // v6.195 (prompts v2) : libellés humains des types — l'IA ne doit
      // plus franciser les codes à sa façon (« complétude », « croisés »).
      summary['libelles_types'] = {
        denomination: 'Nommer les images', denomination_orale: 'Nommer à voix haute',
        comprehension: 'Comprendre la consigne', completion: 'Compléter la phrase',
        syntax: 'Structure de phrase', association: 'Associer les images',
        rhyme: 'Rimes', story: 'Lire et comprendre', repetition: 'Répéter à voix haute',
        intonation: 'Répéter avec intonation', fluence: 'Fluence verbale',
        photos_perso: 'Nommer ses photos', heure: "Lire l'heure", monnaie: 'Compter la monnaie',
        prix: 'Estimer un prix', calcul_quotidien: 'Calcul du quotidien',
        comparaison_nombres: 'Comparer les nombres', anagramme: 'Le mot en morceaux',
        verif: 'Ça va ensemble ?', croises: 'La grille en images', memoire_liste: 'La liste à retenir',
      };
      if (task === 'evolution_story') summary['periode_jours'] = Number((typeof days !== 'undefined' && days) || 30);
...
Utilise les libellés humains fournis dans "libelles_types" (traduis-les si
la langue demandée n'est pas le français) — jamais les codes techniques.
Les scores de fluence ne sont pas des pourcentages classiques (comptage de
mots évoqués) : ne les présente pas comme un taux d'échec. Les mots listés
en erreur peuvent provenir de phrases à compléter : ce sont des mots
cibles, pas des « phrases complexes ». Ton professionnel et sobre. Termine par : ce brouillon est généré
...
"answer" doit être STRICTEMENT égal à l'un des "choices". Tout en ${L}.`;
      ...
      if (task === 'evolution_story') {
        system = `${HUMAN_RULE}
Rédige un RÉCIT D'ÉVOLUTION factuel sur la période demandée
(voir la donnée "periode_jours" fournie) :
compare le DÉBUT et la FIN de la liste "seances_recentes" (elles sont
triées de la plus récente à la plus ancienne) : volume de séances,
tendance des scores par type d'exercice, mots qui ont cessé (ou
commencé) de poser problème, productions vocales. Uniquement des faits
tirés des données, formulés en 8-12 lignes lisibles par la famille comme
par un confrère. Termine par : ce récit est généré automatiquement et
doit être relu et validé par l'orthophoniste. Rédige en ${L}.`;
      }

      if (task === 'ami_journal') {
        // Volet 4 du dossier clinique : un JOURNAL DE BORD qualitatif.
        // L'IA relit la succession des séances récentes et en tire un
        // récit de suivi séance par séance — le ton d'un carnet de bord
        // clinique, PAS d'un ressenti d'Ami (Ami ne prétend rien
        // éprouver, règle constante du projet).
        system = `${HUMAN_RULE}
Tu rédiges un JOURNAL DE BORD qualitatif à partir de "seances_recentes"
(triées de la plus récente à la plus ancienne). Pour les 6 à 8 séances
les plus récentes, écris UNE ligne par séance : la date (champ "date"),
le domaine travaillé (via "libelles_types"), et une observation
factuelle courte (progrès, difficulté, régularité) tirée UNIQUEMENT des
scores et des données. Style carnet de bord clinique, sobre, à la 3e
personne ("le patient"). N'invente aucun ressenti, n'attribue aucune
émotion. Termine par une ligne : synthèse qualitative en une phrase.
Texte brut, une ligne par séance, en ${L}.`;
      }

      if (task === 'clinical_reco') {
        // Volet 5 du dossier clinique : des PISTES d'action. Strictement
        // non-décisionnaires : l'IA propose, l'ortho dispose. Chaque
        // piste doit être justifiée par une donnée précise.
        system = `${HUMAN_RULE}
Propose 3 à 5 PISTES D'ACTION pour la prochaine séance, chacune fondée
sur une donnée précise du résumé (un domaine faible dans
"seances_recentes", une catégorie dominante de "categories_erreurs", un
mot de "mots_souvent_en_erreur", une régularité, un domaine maîtrisé à
complexifier). Format : une piste par ligne, commençant par un verbe
d'action, suivie entre parenthèses de la donnée qui la motive. Ce sont
des SUGGESTIONS à l'appréciation de l'orthophoniste, jamais des
prescriptions : n'emploie pas d'impératif catégorique, écris "envisager
de…", "il pourrait être utile de…". Utilise "libelles_types" pour nommer
les exercices. Termine par : ces pistes sont générées automatiquement et
relèvent de votre seule décision clinique. Texte brut, en ${L}.`;
      }
    }

    if (task === 'cabinet_digest') {
      // Vue cabinet : agrégats ANONYMES des patients rattachés au compte.
      // Aucun nom ne part vers l'IA : Patient A, B, C… (ordre stable).
      const { data: assigns } = await supa.from('patient_assignments')
        .select('patient_code').eq('ortho_code', accountCode).limit(30);
      const codes = (assigns ?? []).map(a => a.patient_code);
      const rows = [];
      const since30 = new Date(Date.now() - 30*24*3600*1000).toISOString();
      for (let i = 0; i < codes.length; i++) {
        const codeI = codes[i];
        const { data: p } = await supa.from('patients')
          .select('level, sessions, correct, total, streak').eq('code', codeI).single();
        const { data: recent } = await supa.from('sessions')
          .select('score, total, at').eq('code', codeI).gte('at', since30).order('at', { ascending: false }).limit(30);
        const { count: pendingVoice } = await supa.from('voice_recordings')
          .select('*', { count: 'exact', head: true }).eq('code', codeI).eq('verdict', 'pending');
        rows.push({
          patient: String.fromCharCode(65 + (i % 26)),
          niveau: p?.level ?? null,
          seances_30j: (recent ?? []).length,
          reussite_30j: (recent ?? []).length ? Math.round(100 * recent.reduce((s, r) => s + (r.score||0), 0) / Math.max(1, recent.reduce((s, r) => s + (r.total||0), 0))) + '%' : null,
          derniere_activite: recent && recent[0] ? (recent[0].at||'').slice(0,10) : null,
          verdicts_vocaux_en_attente: pendingVoice ?? 0,
        });
      }
      system = `${HUMAN_RULE}
Rédige la VUE CABINET du matin en 6-10 lignes : parmi ces patients
anonymisés (A, B, C…), signale — uniquement d'après les données — les
inactifs depuis 7 jours ou plus, les plateaux (réussite stable et
séances régulières), les progrès nets, et le total des verdicts vocaux
en attente. Ton factuel, lisible en 20 secondes. Aucun nom n'existe :
utilise strictement les lettres fournies. Rédige en ${L}.`;
      userMsg = 'Patients (JSON) :\n' + JSON.stringify(rows, null, 2);
    }
...
supprime aucune information, n'interprète pas. Rédige en ${L}.`;
    ...
    if (task === 'triage_suggestions') {
      const { data: suggs } = await supa.from('suggestions')
        .select('source, message, created_at, status').order('created_at', { ascending: false }).limit(100);
