    ...
    if (task === 'cabinet_digest') {
      ...
      for (let i = 0; i < codes.length; i++) {
        ...
        const { data: recent } = await supa.from('sessions')
          .select('correct, total, at').eq('code', codeI).gte('at', since30).order('at', { ascending: false }).limit(30);
        ...
        rows.push({
          ...
          reussite_30j: (recent ?? []).length ? Math.round(100 * recent.reduce((s, r) => s + (r.correct||0), 0) / Math.max(1, recent.reduce((s, r) => s + (r.total||0), 0))) + '%' : null,
