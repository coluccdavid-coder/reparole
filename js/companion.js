...
//
// =====================================================================
//  CHARTE D'AMI (v6.28) — formalisée à la demande explicite de
//  l'utilisateur. Ami n'est pas une mascotte, c'est un compagnon. Son
//  rôle est de créer un lien avec le patient :
//   - Il accueille.
//   - Il explique.
//   - Il encourage.
//   - Il rassure.
//   - Il ne plaisante JAMAIS au détriment du patient.
//   - Il ne minimise JAMAIS une difficulté (on ne dit pas "c'est facile"
//     à quelqu'un pour qui ça ne l'est pas — voir "tip" et "encourage"
//     ci-dessous : la difficulté est reconnue, jamais balayée).
//   - Il ne dramatise JAMAIS une erreur (pas de "faux", "raté", "échec" —
//     déjà un garde-fou existant avant cette charte, voir plus bas).
//   - Son ton est TOUJOURS chaleureux, simple et respectueux.
//  Audit v6.28 : chaque banque de phrases (7 langues) a été relue par
//  rapport à cette charte — aucune violation trouvée, c'était déjà
//  globalement le cas grâce aux garde-fous plus anciens ci-dessous.
//  tests/companion-charter.test.js vérifie maintenant cela automatique-
//  ment (liste de mots à bannir par langue) pour que ça ne régresse pas
//  au prochain ajout de phrase.
//
...
const COMPANION_PHRASES = {
  fr: {
    ...
    // v6.28 : parcours aidant — Ami s'adresse ici au proche, pas au
    // patient (registre différent : "votre proche" plutôt que "vous").
    aidant_welcome: [
      "Merci de l'accompagner.",
      "Voici comment se passe son entraînement.",
      "Votre soutien compte, même dans les petits moments du quotidien."
    ]
  },
  en: {
    welcome: [
    ...
    aidant_welcome: [
      ...
      "Here's how their training is going.",
      "Your support matters, even in small everyday moments."
    ]
  },
  es: {
    welcome: [
    ...
    aidant_welcome: [
      "Gracias por acompañarlo/a.",
      "Así va su entrenamiento.",
      "Su apoyo importa, incluso en los pequeños momentos del día a día."
    ]
  },
  it: {
    welcome: [
    ...
    aidant_welcome: [
      "Grazie per esserle/gli vicino.",
      "Ecco come sta andando il suo allenamento.",
      "Il tuo sostegno conta, anche nei piccoli momenti di ogni giorno."
    ]
  },
  pt: {
    welcome: [
    ...
    aidant_welcome: [
      "Obrigado por o/a acompanhar.",
      "Eis como está a correr o treino dele/dela.",
      "O seu apoio conta, mesmo nos pequenos momentos do dia a dia."
    ]
  },
  de: {
    welcome: [
    ...
    aidant_welcome: [
      "Danke, dass Sie ihn/sie begleiten.",
      "So läuft gerade sein/ihr Training.",
      "Ihre Unterstützung zählt, auch in kleinen Alltagsmomenten."
    ]
  },
  ar: {
    welcome: [
    ...
    aidant_welcome: [
      "شكرًا لمرافقتك له/لها.",
      "هكذا يسير تدريبه/ها.",
      "دعمك مهم، حتى في اللحظات الصغيرة من الحياة اليومية."
    ]
  }
};
...
const Companion = {
  ...
  svg(mood){
    ...
    return (
      '<div class="companion-enter" data-help="help_companion" tabindex="0">'+
