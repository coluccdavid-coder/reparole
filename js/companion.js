...
const Companion = {
  ...
  render(containerId, opts){
    ...
    bubble.className = arriving ? 'companion-bubble companion-bubble-delayed' : 'companion-bubble';
    bubble.setAttribute('role','status');
    bubble.setAttribute('aria-live','polite');
    bubble.textContent = this.message || '';
    textWrap.appendChild(name);
    textWrap.appendChild(bubble);
    el.innerHTML = this.svg(this.mood, {arriving});
  ...
  svg(mood, opts){
    ...
    // v6.34 : entrée normale (léger glissement) vs entrée "arrivée sur
    // l'exercice" (marche depuis plus loin, plus lente) — voir
    // companion.css pour le détail des deux animations.
