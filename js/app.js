...
// =====================================================================
//  v6.71 — CODES MÉMORISÉS (accès rapide au retour)
//  ---------------------------------------------------------------------
//  Constat en creusant l'accessibilité motrice : même un∙e patient∙e
//  seul∙e sur son propre appareil doit retaper son code de suivi
//  (~14 caractères aléatoires) à CHAQUE visite — un vrai obstacle pour
//  ce public. Solution : mémoriser localement (sur cet appareil
//  uniquement, jamais envoyé à un serveur) les profils déjà utilisés,
//  pour un accès en un geste. Couvre au passage le cas "plusieurs
//  personnes sur le même appareil" (chacune reste dans la liste).
//  Le code lui-même n'est pas un secret cryptographique (voir
//  js/storage.js) — le stocker sur l'appareil personnel du patient est
//  cohérent avec ce modèle, mais reste explicite et réversible (bouton
//  "oublier" par profil) pour un appareil partagé/public.
// =====================================================================
...
// une connexion réussie. Limité à 6 pour rester un raccourci pratique,
...
function rememberCurrentProfile(){
  ...
  renderRememberedProfiles();
}
function forgetRememberedProfile(code){
  saveRememberedProfiles(loadRememberedProfiles().filter(p=>p.code!==code));
  renderRememberedProfiles();
}
// Pré-remplit le formulaire puis déclenche la connexion normale — pas de
// chemin de code séparé, pour rester couvert par les mêmes vérifications
// que login().
async function quickLogin(code){
  const remembered = loadRememberedProfiles().find(p=>p.code===code);
  const codeEl = document.getElementById('code');
  const nameEl = document.getElementById('name');
  if(codeEl) codeEl.value = code;
  if(nameEl && remembered) nameEl.value = remembered.name;
  await login();
}
function renderRememberedProfiles(){
  const container = document.getElementById('remembered-profiles');
  if(!container) return;
  const list = loadRememberedProfiles();
  if(!list.length){ container.style.display='none'; container.innerHTML=''; return; }
  container.style.display='';
  const label = (window.I18N && I18N.t('remembered_profiles_label')) || 'Se reconnecter rapidement :';
  const forgetLabel = (window.I18N && I18N.t('forget_profile_label')) || 'Oublier ce profil sur cet appareil';
  container.innerHTML = `<p class="hint" style="margin-bottom:8px">${escapeHTML(label)}</p>` +
    `<div style="display:flex;flex-wrap:wrap;gap:8px">` +
    list.map(p=>`
      <span class="remembered-profile">
        <button type="button" class="btn-ghost" onclick="quickLogin('${p.code.replace(/'/g,"\\'")}')">${escapeHTML(p.name || '—')}</button>
        <button type="button" class="remembered-profile-forget" aria-label="${escapeHTML(forgetLabel)}" title="${escapeHTML(forgetLabel)}" onclick="forgetRememberedProfile('${p.code.replace(/'/g,"\\'")}')">✕</button>
      </span>`).join('') +
    `</div>`;
}
...
function logout(){ stopRecognition(); window.speechSynthesis?.cancel(); user=null; userCode=null; renderRememberedProfiles(); show('login'); }
...
async function deleteMyAccount(){
  ...
  if(!userCode || !statusEl || !confirmEl) return;
  ...
  if(!confirm(I18N.t('account_delete_confirm_dialog'))) return;
...
// v6.73 : VRAI BUG CORRIGÉ — le dossier audio du sango s'appelle
// `audio/sango/` (voir audio/sango/README.md, choisi pour rester lisible
// pour un∙e contributeur∙rice humain∙e), mais le code de langue est 'sg'
// (LANGUAGES.sg dans js/i18n.js). playPartialLangWordUI construisait le
// chemin directement à partir du code de langue (`audio/sg/...`), qui ne
// correspond à aucun dossier réel — jamais remarqué jusqu'ici puisque
// aucun enregistrement sango n'existe encore, donc le message "pas encore
// d'enregistrement" s'affichait dans les deux cas, masquant le problème.
// Trouvé en construisant l'outil de suivi des enregistrements manquants
// (voir audio-checklist.html), qui aurait sinon vérifié le mauvais
// dossier et affiché un faux résultat.
const PARTIAL_LANG_AUDIO_FOLDER = { sg:'sango' };
...
Object.assign(window, {login,createNewPatient,dismissCodeBanner,logout,goDashboard,startExercise,checkChoice,answer_feedback,toggleListen,toggleFluency,finishFluencyItem,speak,uploadMedia,deleteMedia,toggleReminder,saveReminderEmail,playPartialLangWordUI,showPricing,startCheckout,manageSubscription,generateCaregiverAccess,revokeCaregiverAccess,renderCaregiverSection,openMySummary,saveJournalEntry,renderJournal,exportMyData,restoreMyData,showExerciseHelp,quickLogin,forgetRememberedProfile,renderRememberedProfiles,rememberCurrentProfile,toggleFavoriteCurrentWord,renderWordsToReview,deleteMyAccount,renderVoiceSelector,setPreferredVoice});
