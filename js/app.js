...
function levelName(n){ return I18N.t('level_'+(n||2)); } // v6.31 : filet de sécurité — voir login() plus bas et SKILL_ReParole_v6.md
...
// v6.30 : CODE DE DÉVERROUILLAGE QA — PAS un système de paiement, et pas
// un mécanisme de sécurité (visible dans le code source, comme tout JS
// côté client). But unique : permettre de tester les fonctionnalités Pro
// sans devoir éditer Supabase/localStorage à la main à chaque fois (voir
// commentaire v6.24 plus haut — "le passage en pro se fait à la main").
// ⚠️ À RETIRER (ou remplacer par un vrai contrôle serveur) avant de
// brancher un vrai système de paiement : en l'état, n'importe qui lisant
// le code source peut débloquer le Pro gratuitement avec ce code. Ce
// n'est pas un problème tant qu'il n'y a rien à payer, mais ça en
// deviendrait un le jour où un vrai abonnement existera.
const QA_UNLOCK_CODE = 'REPAROLE-QA-PRO';
...
function recordDailySession(){
  ...
  // v6.32 : BUG RÉEL trouvé lors d'un audit complet du site — ce champ
  // n'était jamais sauvegardé nulle part (ni localStorage, ni Supabase),
  // uniquement gardé en mémoire le temps que la page reste ouverte. Un
  // compte gratuit qui rechargeait la page ou se reconnectait repartait
  // avec un quota neuf : la limite de 5 séances/jour ne protégeait
  // quasiment rien en pratique. Fire-and-forget (pas de await) : cet
  // appel est fait depuis 4 endroits (ici, memory.js, phonation.js,
  // conversation.js), certains non-async — bloquer sur la sauvegarde
  // retarderait le début de l'exercice pour un gain minime ; en cas
  // d'échec réseau, le compteur reste au moins correct en mémoire pour
  // la session en cours, et sera re-sauvegardé à la prochaine tentative.
  if(userCode) Store.savePatient(userCode, user).catch(()=>{});
}
...
function showUpsell(reason){
  ...
  document.getElementById('ex-body').innerHTML = `
    <div class="prompt-card" style="text-align:center">
      ...
      <!-- v6.30 : déverrouillage QA, pas un vrai paiement — voir le
           commentaire sur QA_UNLOCK_CODE plus haut dans ce fichier. -->
      <div style="margin-top:22px;padding-top:16px;border-top:1px solid var(--line)">
        <input id="qa-unlock-input" type="text" placeholder="${I18N.t('qa_unlock_ph')}" style="width:100%;padding:10px 12px;border:1.5px solid var(--line);border-radius:10px;font-family:inherit;text-align:center">
        <button class="btn-ghost" style="margin-top:8px;width:100%" onclick="unlockProForTesting()">${I18N.t('qa_unlock_btn')}</button>
        <p id="qa-unlock-msg" style="font-size:.8rem;margin-top:6px" aria-live="polite"></p>
      </div>
    </div>`;
  show('exercise');
}

// v6.30 : voir QA_UNLOCK_CODE — outil de test, pas un système de paiement.
async function unlockProForTesting(){
  const input = document.getElementById('qa-unlock-input');
  const msgEl = document.getElementById('qa-unlock-msg');
  const code = (input.value||'').trim();
  if(code !== QA_UNLOCK_CODE){
    msgEl.style.color = 'var(--error)';
    msgEl.textContent = I18N.t('qa_unlock_error');
    return;
  }
  user.plan = 'pro';
  await Store.savePatient(userCode, user);
  msgEl.style.color = 'var(--accent-dark)';
  msgEl.textContent = I18N.t('qa_unlock_success');
  setTimeout(goDashboard, 900);
}

// v6.32 : un audit complet m'a d'abord fait croire que Memory.start()/
// Phonation.intro()/Conversation.menu() (appelées directement depuis
// index.html) contournaient tout le cloisonnement gratuit/pro. Faux
// diagnostic corrigé après vérification : les trois vérifient déjà
// lockReason() + recordDailySession() en interne, dès la v6.24 (voir
// js/memory.js, js/phonation.js, js/conversation.js). Une tentative de
// "correction" ici aurait fait compter le quota quotidien EN DOUBLE.
// Rien à faire — laissé en commentaire pour ne pas re-suspecter ce
// point à un futur audit sans revérifier d'abord ces 3 fichiers.
...
async function login(){
  ...
  if(!code){ alert(I18N.t('alert_enter_code')); return; }
  userCode = code;
  ...
  if(existing){
    ...
    // v6.31 : BUG RÉEL trouvé via capture d'écran ("Niveau adapté :
    // level_null") — un dossier avec un niveau manquant/invalide (null,
    // undefined, ou hors de 1/2/3) affichait la clé de traduction brute
    // partout où levelName() était appelé, au lieu d'un nom de niveau.
    // Origine exacte non confirmée (dossier de test créé/modifié
    // directement dans Supabase plutôt que via le vrai parcours de
    // bilan, qui lui ne produit jamais autre chose que 1/2/3) — corrigé
    // ici de façon défensive ET réparé dans le dossier lui-même, pas
    // seulement masqué à l'affichage (voir aussi levelName() plus haut,
    // et les mêmes filets ajoutés dans report.html/patient-report.html/
    // aidant.html/dashboard-ortho.js, qui avaient le même problème).
    if(![1,2,3].includes(user.level)) user.level = 2;
...
// v6.27 : résumé imprimable côté patient, à apporter au rendez-vous — voir
// patient-report.html (contrairement à report.html qui reste réservé à
// l'orthophoniste : celui-ci est multilingue et ne montre pas l'analyse
// des erreurs, qui demande une interprétation professionnelle).
function printSummary(){
  if(!userCode) return;
  window.open('patient-report.html?code='+encodeURIComponent(userCode), '_blank');
}
...
async function renderDashboard(){
  ...
  if(histEl && typeof Charts!=='undefined'){
    ...
    if(trendEl){
      // v6.27 : ceci était en dur en français (bug trouvé en construisant
      // le résumé imprimable côté patient, qui réutilise les mêmes clés) —
      // trend_up/trend_down/trend_stable existaient déjà dans les 7 langues
      // (créées pour l'espace ortho, v6.26) mais n'étaient jamais utilisées ici.
...
async function startExercise(type){
  ...
  if(type==='photos_perso'){
    ...
    Companion.mount('companion-exercise');
    Companion.say('exerciseStart');
    show('exercise');
    renderQuestion();
    return;
  }
...
Object.assign(window, {login,createNewPatient,dismissCodeBanner,logout,goDashboard,startExercise,checkChoice,answer_feedback,toggleListen,toggleFluency,finishFluencyItem,speak,uploadMedia,deleteMedia,toggleReminder,saveReminderEmail,playKabWordUI,onLangChange,unlockProForTesting});
