...
function rememberActiveSession(code){
  try{ sessionStorage.setItem(ACTIVE_SESSION_KEY, code); }catch(e){}
}
...
async function attemptAutoLogin(){
  let code = null;
  try{ code = sessionStorage.getItem(ACTIVE_SESSION_KEY); }catch(e){}
  if(!code){
...
async function login(){
  ...
  if(existing){
    ...
    rememberActiveSession(userCode);
...
function renderQuestion(){
  ...
  if(c.type==='denomination'){
    ...
    consigne = I18N.t('association_prompt');
    promptHTML=`<div class="prompt-emoji">${q.text}</div><div class="prompt-text">${consigne}</div>`;
  }
