...
// v6.243 : le mur payant patient s'active selon le réglage admin
...
async function applyBillingPaywall(){
  try{
    ...
    // v6.243 : la case « Patients : passer au payant » pilote seule le mur
...
function pickVoiceForSpeech(){
  ...
  // v6.243 : jusqu'ici on prenait la PREMIÈRE voix de la liste, souvent la
...
function speakBrowserTTS(text){
  ...
  u.rate = 0.94; u.pitch = 1.02;  // v6.243 : débit/hauteur plus naturels
...
// v6.244 : les langues qui disposent RÉELLEMENT de voix pré-générées —
// exactement celles couvertes par LANGUAGE_CODES dans
// scripts/generate-voice-audio.js. Pour les autres (dz, ma, tn, kab),
// aucun fichier n'a jamais été généré : tenter la requête produisait un
// 404 à chaque mot prononcé, et retardait la parole du temps de
// l'aller-retour réseau. On va directement à la synthèse du navigateur.
// ⚠️ À compléter le jour où une de ces langues reçoit des voix cloud.
const CLOUD_VOICE_LANGS = ['fr','en','es','it','pt','de','ar','tr','pl','ja'];
window.__testCloudVoiceLangs = CLOUD_VOICE_LANGS;

function speak(text){
  if(_cloudAudio){ _cloudAudio.pause(); _cloudAudio = null; }
  const lang = (window.Prefs && Prefs.data.lang) || 'fr';
  if(!CLOUD_VOICE_LANGS.includes(lang)){ speakBrowserTTS(text); return; }
  ...
  // v6.248 — BUG DE LA DOUBLE VOIX, signalé par l'utilisateur (« toujours
  // les voix de robot »). Avant : si le mp3 mettait plus de 2,5 s à
  // arriver, le filet lançait la voix du navigateur — puis canplaythrough
  // arrivait et lançait la voix cloud PAR-DESSUS, sans vérifier que le
  // repli avait déjà parlé. Deux voix superposées, et la robotique en
  // premier. Cas aggravé en test avec Ctrl+Maj+R, qui contourne le service
  // worker : chaque écoute redevient une « première » écoute réseau.
  // Désormais : si le repli a parlé, la voix cloud arrivée en retard se
  // tait (le fichier est de toute façon mis en cache par le service
  // worker : la PROCHAINE écoute sera la bonne voix, immédiatement).
  audio.addEventListener('canplaythrough', ()=>{
    if(settled) return;               // le repli a déjà parlé : on ne superpose pas
    settled = true;
  ...
  minuteur = setTimeout(fallback, 2500);
}
...
async function attemptAutoLogin(){
  applyBillingPaywall(); // v6.243 : branche le mur payant sur le réglage admin (fire-and-forget)
  // v6.243 : arrivée depuis la vitrine (accueil.html, « Je commence »)
  ...
  // v6.243 : liens profonds depuis la vitrine (accueil.html) — un seul
...
async function createNewPatient(){
  ...
  // v6.243 : la création a son PROPRE champ prénom (#new-name), dans le
...
// v6.243 : copier la clé personnelle en un geste (réduit le risque de la perdre).
...
// v6.246 : le message ne promet plus « rien n'est perdu ». Il ne pouvait
// pas tenir cette promesse quand le stockage du navigateur était plein, et
// affirmer dans l'interface ce qu'on ne sait pas vrai est précisément ce
// que la règle 5 du projet interdit. On dit ce qui est en attente, et on
// dit aussi ce qui n'a pas pu partir.
if(typeof Store !== 'undefined' && Store.onSaveStatusChange){
  Store.onSaveStatusChange((pending, perdus)=>{
    const el=document.getElementById('save-status');
    if(!el) return;
    const lignes=[];
    if(pending>0) lignes.push(`⏳ ${pending} enregistrement(s) en attente de connexion — l'envoi se fera automatiquement au retour du réseau.`);
    if(perdus>0)  lignes.push(`⚠️ ${perdus} enregistrement(s) n'ont pas pu être envoyés. Vos exercices sont bien faits — c'est leur trace qui manque. Pensez à le signaler à votre orthophoniste.`);
...
async function exportMyData(){
  if(!userCode) return;
  // v6.250 (audit RGPD) : l'export doit couvrir TOUTES les données de la
  // personne — les favoris et la liste des enregistrements de voix
  // manquaient. Les fichiers audio eux-mêmes sont purgés à 30 jours et ne
  // sont pas re-téléchargeables ici ; leurs métadonnées, si.
  const [patient, profile, history, journal, errors, caregiverWords, favorites, voiceRecordings] = await Promise.all([
    ...
    Store.loadFavoriteWords ? Store.loadFavoriteWords(userCode) : [],
    Store.listVoiceRecordings ? Store.listVoiceRecordings(userCode) : []
  ]);
  const bundle = {
...
async function uploadMedia(){
  ...
  if(!result){
    // v6.245 : deux causes possibles, deux messages. Dire « problème
    // côté serveur » à quelqu'un dont le navigateur est simplement plein
    // l'enverrait chercher au mauvais endroit. Les deux clés existent
    // déjà dans les 14 langues — aucune traduction inventée.
    const msg = Store.lastStorageFull ? I18N.t('voice_error') : I18N.t('photos_err_upload_failed');
...
function renderQuestion(){
  ...
  if(c.type==='denomination'){
    ...
    promptHTML=`${ExoImages.html(q.text)}<div class="emoji-label">${emojiLabel(q.text)}</div><div class="prompt-text">${consigne}</div>`;
  }
    ...
    promptHTML=`<div class="prompt-text" style="font-size:1.15rem;line-height:1.6;text-align:start">${escapeHTML(storyText)}</div><div class="prompt-main" style="font-size:1.3rem;margin-top:14px">${escapeHTML(questionText)}</div>`;
...
function renderVoice(q){
  ...
  const visual = c.type==='photos_perso'
    ...
    ? `${ExoImages.html(q.emoji)}<div class="prompt-text">${I18N.t('denom_orale_prompt')}</div>`
    : c.type==='intonation'
...
function toggleListen(target){
  ...
  recognition.onresult=(e)=>{ const alts=[...e.results[0]].map(r=>r.transcript); const said=alts[0]; const ok=alts.some(a=>isCloseEnough(a,target)); mic.classList.remove('listening'); stopRecognition();
    if(ok){ heard.innerHTML=`${I18N.t('heard_label')} <b>« ${escapeHTML(said)} »</b> 👏`; setTimeout(()=>answer_feedback(true, said),400); }
...
function renderCroises(body){
  ...
  body.innerHTML=`
    <div class="prompt-card">
      ${ExoImages.html(w.emoji, {small:true})}
...
function renderAnagramme(q, body){
  ...
  body.innerHTML=`
    <div class="prompt-card">
      ${ExoImages.html(q.emoji, {small:true})}
...
async function finishExercise(){
  ...
  document.getElementById('ex-body').innerHTML=`
    <div class="prompt-card">
      ...
      <div class="ai-note" style="text-align:start;margin-top:18px"><span>🤖</span><div><b>${I18N.t('assistant_learned')}</b>${AI.insight()}</div></div>
  ...
  async render(){
    ...
    listEl.innerHTML = words.map(w => {
      ...
      <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;background:var(--surface-soft);border-radius:10px;padding:10px 14px">
        ...
        ${verdictHtml}
        <button type="button" class="btn-ghost" style="margin-inline-start:auto;padding:6px 14px" data-voice-word="${escapeHTML(word)}">🎙 <span>${I18N.t('voice_record_btn')}</span></button>
      </div>`;
    }).join('');
...
const CustomExos = {
  ...
  async render(){
    ...
    listEl.innerHTML = exos.map(e => `
      <button type="button" class="btn-ghost" style="width:100%;text-align:start;display:flex;align-items:center;gap:10px" data-custom-exo="${escapeHTML(String(e.id))}">
...
// =====================================================================
//  v6.243 — ICÔNES MODERNES (espace patient)
//  ---------------------------------------------------------------------
