...
async function saveJournalEntry(){
  ...
  if(!text){ statusEl.textContent = 'Écrivez quelque chose avant d\'ajouter.'; statusEl.style.color = '#b23b3b'; return; }
  const { error } = await Store.addJournalEntry(userCode, text);
  if(error){ statusEl.textContent = "Ça n'a pas fonctionné, réessayez."; statusEl.style.color = '#b23b3b'; return; }
  textEl.value = '';
  statusEl.textContent = '✅ Ajouté à votre journal.';
...
async function startExercise(type){
  ...
  const queueSource = (window.Prefs && Prefs.data.shortSession) ? set.slice(0, Math.min(3, set.length)) : set;
...
Object.assign(window, {login,createNewPatient,dismissCodeBanner,logout,goDashboard,startExercise,checkChoice,answer_feedback,toggleListen,toggleFluency,finishFluencyItem,speak,uploadMedia,deleteMedia,toggleReminder,saveReminderEmail,playKabWordUI,showPricing,startCheckout,generateCaregiverAccess,revokeCaregiverAccess,renderCaregiverSection,openMySummary,saveJournalEntry,renderJournal});
