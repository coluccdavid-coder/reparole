...
const AdminPanel = {
  ...
  async purgeLoginEvents(){
    ...
    if(!btn.dataset.confirm){
      ...
      btn.textContent = '⚠️ Confirmer : supprimer les connexions de plus de 30 jours';
      setTimeout(()=>{ delete btn.dataset.confirm; btn.textContent = '🧹 Faire le ménage (garder 30 jours)'; }, 5000);
      return;
    }
    delete btn.dataset.confirm;
    btn.disabled = true;
    const res = await ReParoleStore.purgeLoginEvents();
    btn.disabled = false;
    btn.textContent = '🧹 Faire le ménage (garder 30 jours)';
