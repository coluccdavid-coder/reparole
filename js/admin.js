...
const AdminPanel = {
  ...
  _afterLogin(res){
    ...
    AdminPanel.renderBilling();
    AdminPanel.renderOverview();
    AdminPanel.renderInactive();
  },
  ...
  // ===================================================================
  //  v6.207 — Paiement & tarifs : configuration sans code (app_settings)
  //  L'admin règle solution/devise/prix ici ; l'app lit ces valeurs
  //  (ex. tarifs de l'espace ortho). Champs conditionnels lien/API.
  // ===================================================================
  async renderBilling(){
    const provider = document.getElementById('bill-provider');
    if(!provider) return;
    provider.onchange = ()=>{ document.getElementById('bill-provider-other-row').style.display = (provider.value==='autre') ? '' : 'none'; };
    const method = document.getElementById('bill-method');
    method.onchange = ()=>{
      const link = method.value==='link';
      document.getElementById('bill-link-row').style.display = link ? '' : 'none';
      document.getElementById('bill-api-rows').style.display = link ? 'none' : '';
    };
    ...
    set('bill-provider', st.provider); provider.onchange();
    set('bill-provider-other', st.providerOther);
    set('bill-method', st.method); method.onchange();
    set('bill-link', st.paymentLink);
    set('bill-shop', st.shopId);
    set('bill-mode', st.mode);
    set('bill-currency', st.currency);
    set('bill-patient-monthly', st.patientMonthly);
    set('bill-patient-annual', st.patientAnnual);
    set('bill-ortho-monthly', st.orthoMonthly);
    set('bill-ortho-annual', st.orthoAnnual);
    chk('bill-free-beta', st.freeBeta);
    chk('bill-paywall-patients', st.paywallPatients);
    chk('bill-paywall-ortho', st.paywallOrtho);
    chk('bill-free-tier', st.freeTier);
    chk('bill-foreign-cards', st.foreignCards);
  },
  ...
  async saveBilling(){
    ...
    const settings = {
      provider: val('bill-provider'),
      providerOther: val('bill-provider-other'),
      method: val('bill-method'),
      paymentLink: val('bill-link'),
      shopId: val('bill-shop'),
      // volontairement : la clé secrète n'est PAS stockée en clair côté
      // lecture publique — champ conservé pour saisie mais stocké tel quel
      // uniquement si renseigné (à déplacer vers un secret Edge en prod).
      apiKey: val('bill-key') || undefined,
      mode: val('bill-mode'),
      currency: val('bill-currency'),
      patientMonthly: parseFloat(val('bill-patient-monthly'))||null,
      patientAnnual: parseFloat(val('bill-patient-annual'))||null,
      orthoMonthly: parseFloat(val('bill-ortho-monthly'))||null,
      orthoAnnual: parseFloat(val('bill-ortho-annual'))||null,
      freeBeta: chk('bill-free-beta'),
      // v6.233 : deux interrupteurs indépendants (patients / orthophonistes)
      paywallPatients: chk('bill-paywall-patients'),
      paywallOrtho: chk('bill-paywall-ortho'),
      freeTier: chk('bill-free-tier'),
      foreignCards: chk('bill-foreign-cards')
    };
    const msg=document.getElementById('bill-msg');
    msg.textContent='Enregistrement…';
    const res = await ReParoleStore.saveBillingSettings(settings);
    msg.textContent = (res && res.error) ? "Échec de l'enregistrement (table app_settings créée ? voir sql/app-settings.sql)" : '✅ Réglages enregistrés.';
  },

  // ===================================================================
  //  v6.209 — Activation Pro manuelle : après un paiement reçu par lien
  //  bancaire (confirmation hors app), l'admin bascule le compte en Pro.
  // ===================================================================
  async setPlan(){
    const type = document.getElementById('plan-type').value;
    const id = document.getElementById('plan-id').value.trim();
    const plan = document.getElementById('plan-value').value;
    const msg = document.getElementById('plan-msg');
    if(!id){ msg.textContent = 'Indiquez la clé patient (p-…) ou l’e-mail ortho.'; return; }
    msg.textContent = 'Mise à jour…';
    const res = await ReParoleStore.adminSetPlan(type, id, plan);
    msg.textContent = res.ok ? ('✅ Compte passé en « '+plan+' ».')
      : ('Échec : '+(res.error && res.error.message ? res.error.message : 'fonction admin_set_plan créée ? (sql/app-settings.sql)'));
  },

  // ===================================================================
  //  v6.224 — Tuiles de la vue d'ensemble : chiffres RÉELS uniquement.
  //  Si la fonction SQL n'est pas encore créée (sql/app-settings.sql),
  //  les tuiles restent à « — » : jamais de valeur inventée.
  // ===================================================================
  async renderOverview(){
    if(!document.getElementById('admin-tiles')) return;
    const st = await ReParoleStore.getAdminOverview();
    const diag = document.getElementById('admin-tiles-msg');
    if(!st || st.__error){
      if(diag){
        diag.textContent = st && st.__error
          ? ('Chiffres indisponibles — ' + st.__error + ' · Exécutez sql/app-settings.sql (fonction get_admin_overview) dans Supabase.')
          : 'Chiffres indisponibles — mode hors-ligne ou session expirée.';
    ...
    const put=(id,v)=>{ const el=document.getElementById(id);
      if(el && v!=null) el.textContent = Number(v).toLocaleString('fr-FR'); };
    put('tile-patients', st.patients_actifs);
    put('tile-sessions', st.sessions_7d);
    put('tile-logins',   st.connexions_today);
    put('tile-sugg',     st.suggestions_new);
  },

  // ===================================================================
  //  v6.235 — Patients inactifs : libellés pseudonymisés + purge en lot.
  //  Aucune clé patient n'arrive dans le navigateur (voir SQL).
  // ===================================================================
  async renderInactive(){
    const box = document.getElementById('inactive-list');
    if(!box) return;
    const days = parseInt((document.getElementById('inact-days')||{}).value || '14', 10);
    box.innerHTML = '<p class="hint">Chargement…</p>';
    const rows = await ReParoleStore.getInactivePatients(days);
    if(!rows || rows.__error){
      box.innerHTML = '<p class="hint" style="color:var(--error)">Indisponible — ' +
        ((rows && rows.__error) || 'exécutez sql/app-settings.sql') + '</p>';
      return;
    }
    if(!rows.length){ box.innerHTML = '<p class="hint">Aucun compte inactif sur cette période. 🎉</p>'; return; }
    // v6.242 : plusieurs comptes peuvent porter le même libellé masqué
    // (« M…e »). On affiche donc aussi la date de création, et on signale
    // clairement les comptes créés puis jamais utilisés.
    const jour = d => { if(!d) return ''; const p = String(d).slice(0,10).split('-');
      return p.length === 3 ? (p[2] + '/' + p[1] + '/' + p[0]) : String(d); };
    box.innerHTML = rows.map(r =>
      '<div class="history-row" style="display:flex;justify-content:space-between;gap:12px;padding:10px 0;flex-wrap:wrap">' +
      '<b style="font-family:\'Fredoka\',sans-serif">' + r.label +
        (r.created_on ? ' <span class="hint" style="font-weight:400">· créé le ' + jour(r.created_on) + '</span>' : '') + '</b>' +
      '<span class="hint">' + (Number(r.sessions_count) === 0
        ? 'jamais utilisé · inactif depuis ' + r.days_inactive + ' j'
        : r.days_inactive + ' j sans séance · ' + r.sessions_count + ' séance(s) au total') +
      '</span></div>').join('');
  },

  async purgeInactive(){
    const btn = document.getElementById('purge-inactive-btn');
    const status = document.getElementById('purge-inactive-status');
    if(!btn.dataset.confirm){
      btn.dataset.confirm = '1';
      btn.textContent = '⚠️ Confirmer la suppression définitive';
      setTimeout(()=>{ delete btn.dataset.confirm; btn.textContent = '🗑️ Supprimer les comptes inactifs depuis 30 jours'; }, 6000);
      return;
    }
    delete btn.dataset.confirm;
    btn.disabled = true;
    status.textContent = 'Suppression…';
    const res = await ReParoleStore.deleteInactivePatients(30);
    btn.disabled = false;
    btn.textContent = '🗑️ Supprimer les comptes inactifs depuis 30 jours';
    status.textContent = res.error
      ? ('Échec : ' + (res.error.message || 'fonction admin_delete_inactive_patients absente'))
      : ('✅ ' + (res.deleted || 0) + ' compte(s) supprimé(s).');
    AdminPanel.renderInactive();
  },
      ...
      <div class="patient-row" style="flex-direction:column;align-items:stretch;gap:8px;cursor:default">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">
          <div>
            ...
            ${it.status==='new' ? '<span class="pro-tag" style="background:#b23b3b;margin-inline-start:6px">Nouveau</span>' : ''}
          </div>
          <span style="font-size:.72rem;color:var(--ink-soft)">${new Date(it.created_at).toLocaleDateString('fr-FR')}</span>
        </div>
  ...
  // =====================================================================
  // v6.186 : ménage de l'historique des connexions (> 30 jours).
  // Double clic de confirmation plutôt qu'un confirm() natif (cohérence
  // avec le reste de l'app, jamais de dialogue système non traduit).
  async purgeLoginEvents(){
    const btn = document.getElementById('purge-logins-btn');
    const status = document.getElementById('purge-logins-status');
    const sel = document.getElementById('purge-days');
    const days = sel ? parseInt(sel.value, 10) : 30;   // v6.223 : 7, 14 ou 30 jours
    if(!btn.dataset.confirm){
      btn.dataset.confirm = '1';
      btn.textContent = '⚠️ Confirmer : supprimer au-delà de ' + days + ' jours';
      setTimeout(()=>{ delete btn.dataset.confirm; btn.textContent = '🧹 Faire le ménage'; }, 5000);
      return;
    }
    delete btn.dataset.confirm;
    btn.disabled = true;
    const res = await ReParoleStore.purgeLoginEvents(days);
    btn.disabled = false;
    btn.textContent = '🧹 Faire le ménage';
    status.textContent = res.error ? ('Ménage impossible : ' + res.error) : '✅ Ménage fait.';
    if(!res.error) AdminPanel.renderLoginHistory();
  },

  // v6.250 (audit RGPD) : purge des erreurs techniques — même double clic
  // de confirmation que purgeLoginEvents, mêmes durées bornées côté SQL.
  async purgeClientErrors(){
    const btn = document.getElementById('purge-errors-btn');
    const status = document.getElementById('purge-errors-status');
    const sel = document.getElementById('purge-errors-days');
    ...
    delete btn.dataset.confirm;
    btn.disabled = true;
    const res = await ReParoleStore.purgeClientErrors(days);
    btn.disabled = false;
    btn.textContent = '🧹 Faire le ménage';
    status.textContent = res.error ? ('Ménage impossible : ' + res.error) : '✅ Ménage fait.';
    if(!res.error) AdminPanel.renderClientErrors();
  },
  ...
  async iaResearchExercises(){
    const out = document.getElementById('ai-research-out');
    const btn = document.getElementById('ai-research-btn');
    btn.disabled = true;
    out.textContent = 'Veille en cours (recherche web)… cela peut prendre un moment.';
    const res = await ReParoleStore.iaAssist('research_exercises', { lang: 'fr' });
    btn.disabled = false;
    out.textContent = res.error
      ? (res.error === 'indisponible' ? 'Fonction IA non déployée (voir js/ia-edge-function.md).' : 'Veille impossible : ' + res.error)
      : res.result;
  },
...
if(typeof window !== 'undefined'){ AdminPanel.copyResearch = async function(){
  // v6.194 : la veille se copie — pour la coller dans des notes, un
  // document de travail, ou la partager. (Elle n'est pas stockée :
  // l'humain décide de ce qui mérite d'être gardé.)
  const out = document.getElementById('ai-research-out');
  const fb = document.getElementById('ai-research-copied');
  if(!out || !out.textContent.trim()) return;
  try{ await navigator.clipboard.writeText(out.textContent); if(fb) fb.textContent = '✅ Copié.'; }
  catch(e){}
};
