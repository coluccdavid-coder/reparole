...
const AdminPanel = {
  ...
  async renderDailyConnections(){
    ...
    if(!rows.length){
      ...
      // juste au-dessus). Cause probable trouvée : log_patient_connection()
      // dépend de l'extension pgcrypto (voir sql/schema.sql), qui peut
      // échouer silencieusement à s'activer selon le plan/les droits
      // Supabase — un échec d'écriture invisible qui ressemblait
      // jusqu'ici à "zéro connexion" tout court, sans distinction.
      el.innerHTML = `<p class="hint">Aucune connexion enregistrée pour l'instant.</p>
        <p class="hint" style="margin-top:6px">Si des séances apparaissent bien ci-dessus malgré tout, l'enregistrement des connexions échoue probablement en silence — vérifiez que l'extension <code>pgcrypto</code> est activée dans Supabase (Database → Extensions), puis consultez la console du navigateur (F12) au prochain login pour voir l'erreur exacte.</p>`;
      return;
    }
