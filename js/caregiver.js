...
async function renderCaregiverWordList(){
  ...
  // v6.173 : la liste peut maintenant contenir des mots ciblés par
  // l'orthophoniste (source='ortho') — on les affiche avec un badge :
  // l'aidant voit ce que le/la professionnel·le travaille en priorité,
  // et peut s'en inspirer au quotidien.
  el.innerHTML = words.map(w => `
    <div style="background:var(--surface-soft);border-radius:10px;padding:8px 14px;display:flex;align-items:center;gap:10px">
      ...
      ${w.source === 'ortho' ? `<span class="badge-pro" style="font-size:.68rem">${escapeHTML(cgT('cg_word_by_ortho'))}</span>` : ''}
    </div>`).join('');
}
