...
const Memory = {
  ...
  _renderShow(){
    ...
    s.sequence.forEach((sym,i)=>{
      setTimeout(()=>{
        // v6.248 : vraie photo si disponible (voir js/exo-images.js) —
        // l'emoji reste le repli. Demandé par le propriétaire : les emoji
        // seuls font enfantin pour des adultes après un AVC.
        stage.innerHTML = `<div style="animation:pop .3s">${ExoImages.html(sym)}</div>`;
      }, i*stepMs);
    });
    ...
    this._el().innerHTML = `
      <div class="card">
        ...
        <div class="choices" id="memory-choices" style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap">
          ${choices.map(sym=>`<button class="choice" style="padding:10px 14px" onclick="Memory.pick('${sym}',this)">${ExoImages.html(sym,{small:true})}</button>`).join('')}
        </div>
      </div>`;
  },
  ...
  pick(sym, btn){
    ...
    if(s.showing) return;
    s.userSeq.push(sym);
    document.getElementById('memory-picked').innerHTML = s.userSeq.map(x=>ExoImages.html(x,{small:true})).join('');
