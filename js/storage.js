...
async function initCloud(){
  ...
  if(!window.supabase){
    await new Promise((res, rej)=>{
      ...
      // v6.86 : version figée (2.108.2) plutôt que "@2" (qui pointe
      // toujours vers la toute dernière 2.x). Charger une plage de
      // version plutôt qu'une version exacte, c'est le principal vecteur
      // des attaques de chaîne d'approvisionnement — une nouvelle
      // version, même compromise, se chargerait automatiquement sans
      // que rien ne change de votre côté. Sans risque de casser
      // quoi que ce soit ici : seules des fonctions d'authentification/
      // RPC/storage déjà stables sont utilisées.
...
const ReParoleStore = {
  ...
  // v6.175 : brouillon de compte-rendu par IA. L'edge function (voir
  // js/ia-edge-function.md) authentifie l'ortho par son JETON DE SESSION
  // (pas la clé anon), re-vérifie le rattachement, ANONYMISE (ni nom ni
  // code ne partent vers l'IA) et plafonne à 20/jour. Si la fonction
  // n'est pas déployée, l'appel échoue proprement (404 -> 'indisponible').
  async orthoGenerateReportDraft(patientCode, lang){
    ...
    const base = SUPABASE_URL.replace(/\/$/,'');
    try{
      const res = await fetch(base + '/functions/v1/generate-report-draft', {
        method:'POST',
        headers:{ 'Content-Type':'application/json', 'Authorization':'Bearer '+token },
        body: JSON.stringify({ patient_code: patientCode, lang: lang || 'fr' })
      });
