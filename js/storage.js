...
const ReParoleStore = {
  ...
  async savePatient(code, p){
    ...
    if(CLOUD_ENABLED){
      ...
      const record = { code, name:p.name, level:p.level, sessions:p.sessions, correct:p.correct, total:p.total, streak:p.streak, plan:p.plan || 'free', last_seen:new Date().toISOString() };
