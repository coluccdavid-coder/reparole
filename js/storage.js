...
// =====================================================================
//  v6.245 — TOUTE ÉCRITURE LOCALE PASSE PAR ICI
//  ---------------------------------------------------------------------
//  `_lsSet()` LÈVE une exception quand le quota du
//  navigateur est atteint (QuotaExceededError, ~5 Mo par site). Jusqu'ici
//  21 écritures de ce fichier n'étaient pas protégées : l'exception
//  remontait au milieu d'un exercice ou d'un ajout de photo, sans être
//  attrapée nulle part. Concrètement, le patient ne voyait RIEN — ni
//  succès, ni erreur — et perdait sa progression en silence.
//
//  Le quota est atteignable : en mode navigateur (sans compte cloud —
//  le mode par défaut), les photos sont stockées en base64 dans
//  localStorage, et l'historique des séances n'a aucun plafond.
//
//  Cette fonction ne masque rien : elle renvoie false, et c'est à
//  l'appelant de décider quoi en dire au patient.
// =====================================================================
// v6.246 — LECTURE SÛRE DU STOCKAGE LOCAL
// ---------------------------------------------------------------------
// En mode local (sans compte cloud), localStorage EST la base de données
// du patient. Jusqu'ici, 31 lectures faisaient `JSON.parse(...)` sans
// filet : une seule valeur corrompue — écriture interrompue par un quota
// plein, plantage du navigateur, extension de nettoyage — levait une
// exception au chargement et cassait l'écran, alors que les données
// étaient toujours là, simplement illisibles, et sans aucun moyen de
// s'en sortir pour la personne devant l'écran.
//
// Choix important : une valeur illisible n'est PAS écrasée en silence.
// Elle est mise de côté une seule fois sous `<clé>:corrompu`, pour rester
// récupérable (l'export de données la retrouvera), et la lecture repart
// sur la valeur par défaut. Perdre l'affichage est réparable ; perdre les
// données ne l'est pas.
function _lsGet(key, fallback){
  let raw = null;
  try{ raw = localStorage.getItem(key); }
  catch(e){ return fallback; }              // stockage totalement inaccessible (mode privé strict)
  if(raw === null || raw === '') return fallback;
  try{
    const value = JSON.parse(raw);
    if(value === null || value === undefined) return fallback;
    // Une écriture interrompue peut produire du JSON valide mais de la
    // mauvaise forme (une chaîne là où on attend une liste). Sans ce
    // contrôle, le premier .push() ou .filter() lèverait une exception —
    // exactement le plantage qu'on cherche à éviter.
    if(Array.isArray(fallback) && !Array.isArray(value)) return fallback;
    if(fallback && typeof fallback === 'object' && !Array.isArray(fallback) &&
       (typeof value !== 'object' || Array.isArray(value))) return fallback;
    return value;
  }catch(e){
    try{
      const bac = key + ':corrompu';
      if(localStorage.getItem(bac) === null) localStorage.setItem(bac, raw);
    }catch(e2){ /* plus de place pour la copie : on continue quand même */ }
    console.warn('Donnée locale illisible, mise de côté sous ' + key + ':corrompu —', e && e.name);
    return fallback;
  }
}
...
function _lsSet(key, value){
  try{
    ...
    console.warn('Écriture locale refusée (espace du navigateur plein ?) :', key, e && e.name);
    return false;
  }
}

// =====================================================================
//  File d'attente hors-ligne (v5, durcie en v6.246)
// ---------------------------------------------------------------------
//  Trois défauts corrigés, tous silencieux, donc tous dangereux :
//
//  1. La file n'avait AUCUNE borne. Un patient longtemps hors ligne, ou
//     un Supabase injoignable, la faisait grossir jusqu'à saturer le
//     stockage du navigateur.
//  2. `_enqueue()` ignorait la valeur de retour de `_pendingSave()`.
//     Quand le quota était atteint, l'élément n'était jamais écrit et
//     personne ne le savait — perte pure et simple, sans trace.
//  3. Un élément qui échoue pour une raison PERMANENTE (charge
//     malformée, rejet RLS) restait dans la file et était rejoué à
//     chaque reconnexion, indéfiniment. La file ne pouvait que grossir.
//
//  Le compteur d'échecs, le plafond et la purge par âge existent pour
//  que la file puisse toujours se vider. Et si quelque chose est
//  réellement perdu, c'est COMPTÉ et dit — jamais masqué (règle 5 du
//  projet : on n'affirme pas dans l'interface ce qu'on ne sait pas vrai).
// =====================================================================
const PENDING_KEY  = 'reparole:pending';
const ABANDON_KEY  = 'reparole:pending-abandonnes';   // compteur de ce qui n'a pas pu partir
const PENDING_MAX  = 300;      // au-delà, le plus ancien cède la place
const PENDING_TRIES = 5;       // échecs consécutifs avant abandon d'un élément
const PENDING_MAX_AGE_MS = 30 * 24 * 60 * 60 * 1000;  // 30 jours

function _pendingList(){ return _lsGet(PENDING_KEY, []); }
function _pendingSave(list){ return _lsSet(PENDING_KEY, JSON.stringify(list)); }
function _abandonCount(){ const n = _lsGet(ABANDON_KEY, 0); return (typeof n === 'number' && n >= 0) ? n : 0; }
function _abandon(nb, raison){
  if(nb <= 0) return;
  console.warn(nb + ' enregistrement(s) abandonné(s) — ' + raison);
  _lsSet(ABANDON_KEY, JSON.stringify(_abandonCount() + nb));
  // Une perte doit remonter là où quelqu'un la verra : la table des
  // erreurs client, déjà affichée dans l'espace administrateur. Aucune
  // interface nouvelle, aucune clé de traduction — et surtout, une perte
  // silencieuse devient une perte constatable.
  try{
    if(typeof ReParoleStore !== 'undefined' && ReParoleStore.logClientError){
      ReParoleStore.logClientError(
        'File d\'attente : ' + nb + ' enregistrement(s) abandonné(s) — ' + raison,
...
function _notifyStatus(){
  const n = _pendingList().length, perdus = _abandonCount();
  _statusListeners.forEach(cb=>{ try{ cb(n, perdus); }catch(e){} });
}

function _enqueue(kind, payload){
  const list = _pendingList();
  list.push({ kind, payload, at: Date.now(), tries: 0 });
  // Plafond : on sacrifie les plus anciens, jamais celui qui vient
  // d'arriver — l'exercice que le patient vient de terminer compte plus
  // qu'une séance vieille de trois semaines déjà remplacée par d'autres.
  let sacrifies = 0;
  while(list.length > PENDING_MAX){ list.shift(); sacrifies++; }
  if(sacrifies) _abandon(sacrifies, 'file d\'attente pleine (' + PENDING_MAX + ' éléments)');
  // v6.246 : on REGARDE si l'écriture a réussi. Avant, un quota plein
  // faisait disparaître l'élément sans un mot.
  if(!_pendingSave(list)) _abandon(1, 'stockage du navigateur plein — écriture impossible');
  _notifyStatus();
}
...
async function _flushPending(){
  if(!CLOUD_ENABLED) return;
  let list = _pendingList();
  if(!list.length) return;

  // Purge par âge AVANT tout appel réseau : inutile de réessayer
  // éternellement une séance vieille de plus d'un mois.
  const limite = Date.now() - PENDING_MAX_AGE_MS;
  const frais = list.filter(it => !it.at || it.at >= limite);
  if(frais.length !== list.length) _abandon(list.length - frais.length, 'plus de 30 jours en attente');
  list = frais;
  if(!list.length){ _pendingSave(list); _notifyStatus(); return; }
  ...
  for(const item of list){
    let echec = false;
    try{ const { error } = await _replayOne(item); if(error) echec = true; }
    catch(e){ echec = true; }
    if(!echec) continue;
    item.tries = (item.tries || 0) + 1;
    // Après PENDING_TRIES échecs, l'élément est probablement rejeté pour
    // une raison qui ne changera pas. On arrête de le rejouer plutôt que
    // de bloquer la file derrière lui pour toujours.
    if(item.tries >= PENDING_TRIES) epuises++;
    else remaining.push(item);
  }
  if(epuises) _abandon(epuises, PENDING_TRIES + ' tentatives d\'envoi sans succès');
...
const ReParoleStore = {
  ...
  // v6.246 : nombre d'enregistrements qui n'ont PAS pu partir (stockage
  // plein, file saturée, ou trop d'échecs). Sert à dire la vérité dans
  // l'interface au lieu de promettre que rien n'est perdu.
  lostCount(){ return _abandonCount(); },
  // Remise à zéro une fois l'information vue/traitée (espace admin).
  clearLostCount(){ _lsSet(ABANDON_KEY, '0'); _notifyStatus(); },
  onSaveStatusChange(cb){ _statusListeners.push(cb); cb(this.pendingCount(), this.lostCount()); },
  ...
  async createCheckoutSession(planKey, accountCode, accountType){
    // v6.207 : si l'admin a configuré un LIEN de paiement (banque française
    // via Cyberpaiement/PayZen/Systempay/Stancer, ou autre), on y renvoie
    // directement — quel que soit le plan. Sinon, circuit Stripe habituel.
    if(CLOUD_ENABLED){
      try{
        const bill = await ReParoleStore.getBillingSettings();
        if(bill && bill.method==='link' && bill.paymentLink){
          return { url: bill.paymentLink };
        }
      }catch(e){ /* réglages indisponibles : on continue en Stripe */ }
    }
    if(!CLOUD_ENABLED) return { error:new Error('Le mode cloud n\'est pas configuré.') };
  ...
  async loadPatient(code){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:'+code, null);
    }
  },
  ...
  async deleteAccount(code){
    if(CLOUD_ENABLED){
      ...
      const rec = _lsGet('reparole:'+code, null);
  ...
  async saveLevels(code, levels, attempts){
    ...
    if(!CLOUD_ENABLED){
      const record = _lsGet('reparole:'+code, {});
      _lsSet('reparole:'+code, JSON.stringify({ ...record, levels: levels||{}, levelAttempts: attempts||{} }));
      return;
    }
    try{
      await initCloud();
  ...
  async savePatient(code, p){
    ...
    if(CLOUD_ENABLED){
      ...
      _lsSet('reparole:'+code, JSON.stringify({ ...(_lsGet('reparole:'+code, {})), ...record }));
    }
  },
  ...
  async logSession(code, entry){
    ...
    const payload = { p_code:code, p_type:entry.type, p_score:entry.score, p_total:entry.total, p_level:entry.level, p_lang:lang,
      p_duration_sec: (Number.isFinite(entry.duration_sec) ? entry.duration_sec : null) }; // v6.197
    if(CLOUD_ENABLED){
      await initCloud();
  ...
  async history(code){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:hist:'+code, []);
    }
  },
  ...
  async loadJournalEntries(code){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:journal:'+code, []);
    }
  },
  ...
  async loadProfile(code){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:profile:'+code, null);
    }
  },
  async saveProfile(code, profile){
    if(CLOUD_ENABLED){
      ...
      _lsSet('reparole:profile:'+code, JSON.stringify(profile));
    }
  },
  ...
  async updateClinicalProfile(code, profileKey){
    if(CLOUD_ENABLED){
      ...
      const rec = _lsGet(key, {});
      rec.clinical_profile = profileKey;
      _lsSet(key, JSON.stringify(rec));
    }
  },
  ...
  async generateCaregiverCode(code){
    if(CLOUD_ENABLED){
      ...
      _lsSet(key, JSON.stringify(rec));
      _lsSet('reparole:caregiver-index:'+newCode, code);
      return newCode;
    }
  },
  ...
  async loadCaregiverData(caregiverCode){
    if(CLOUD_ENABLED){
      ...
      const rec = _lsGet('reparole:'+patientCode, {});
      if(!rec || !rec.name) return null;
      const allSessions = _lsGet('reparole:hist:'+patientCode, []);
      const sessions = allSessions.slice(-14).reverse();
      const errors = _lsGet('reparole:errors:'+patientCode, []);
  ...
  async loadCaregiverWords(code){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:caregiver-words:'+code, []);
    }
  },
  ...
  async loadCaregiverAddedWords(caregiverCode){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:caregiver-words:'+patientCode, []);
    }
  },
  ...
  async toggleFavoriteWord(code, word){
    ...
    if(CLOUD_ENABLED){
      ...
      if(idx>=0){ arr.splice(idx,1); _lsSet(key, JSON.stringify(arr)); return { isFavorite:false }; }
      arr.unshift(trimmed);
      _lsSet(key, JSON.stringify(arr));
  ...
  async loadFavoriteWords(code){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:favorites:'+code, []);
    }
  },
  ...
  async listSuggestions(){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:suggestions-local', []);
    }
  },
  ...
  // v6.224 : compteurs agrégés de l'en-tête admin (aucune donnée
  // individuelle). Renvoie null si la fonction SQL n'est pas encore
  // créée — l'interface affiche alors « — », jamais de chiffre inventé.
  // v6.235 : patients inactifs — libellés pseudonymisés (1re + dernière
  // lettre), jamais la clé du patient. Purge en lot bridée à 30 jours.
  async getInactivePatients(days){
    if(!CLOUD_ENABLED) return null;
    await initCloud();
    const { data, error } = await supa.rpc('get_admin_inactive_patients', { p_days: days || 14 });
    if(error){ console.warn('get_admin_inactive_patients:', error.message); return { __error: error.message }; }
    return data || [];
  },
  async deleteInactivePatients(days){
    if(!CLOUD_ENABLED) return { error:new Error('Mode cloud requis.') };
    await initCloud();
    const { data, error } = await supa.rpc('admin_delete_inactive_patients', { p_days: days || 30 });
  ...
  async getAdminOverview(){
    if(!CLOUD_ENABLED) return null;
    await initCloud();
    const { data, error } = await supa.rpc('get_admin_overview');
    // v6.227 : on REMONTE l'erreur (au lieu de renvoyer null en silence),
    // pour que l'admin puisse voir pourquoi les chiffres ne s'affichent
    // pas — typiquement : fonction SQL pas encore créée.
    if(error){ console.warn('Supabase get_admin_overview:', error.message); return { __error: error.message }; }
    return data || null;
  },
  ...
  async errorHistory(code){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:errors:'+code, []);
    }
  },

  // v6.230 : CSV réellement ouvrable dans Excel/LibreOffice en français.
  //  • séparateur POINT-VIRGULE (Excel FR découpe sur ; pas sur ,)
  //  • marqueur d'encodage (BOM) pour que les accents s'affichent
  //  • en-têtes et valeurs traduisibles, dates lisibles (JJ/MM/AAAA HH:MM)
  //  `labels` : en-têtes affichés ; `format` : transformation par colonne.
  toCSV(rows, columns, opts){
    const o = opts || {};
    const labels = o.labels || {};
    const format = o.format || {};
    const esc = v => `"${String(v??'').replace(/"/g,'""')}"`;
    const sep = ';';
    const head = columns.map(c => esc(labels[c] || c)).join(sep);
    const body = rows.map(r => columns.map(c => {
      const raw = r[c];
      return esc(typeof format[c] === 'function' ? format[c](raw, r) : raw);
    }).join(sep)).join('\r\n');
    return '\ufeff' + head + '\r\n' + body;
  },
  ...
  // v6.240 : lecture directe de l'état « tous les jeux révélés » pour un
  // patient. La fiche s'appuyait jusqu'ici sur l'instantané renvoyé par
  // listPatients() : si cette copie ne contenait pas la colonne (schéma
  // plus ancien, cache d'une liste chargée avant la modification), la
  // case revenait toujours décochée alors que la base disait l'inverse
  // — signalé par l'utilisateur. On relit donc la vérité en base.
  async orthoGetGamesUnlock(patientCode){
    if(!CLOUD_ENABLED) return null;
    await initCloud();
    const { data, error } = await supa
      .from('patients').select('games_all_unlocked')
      .eq('code', patientCode).maybeSingle();
    if(error){ console.warn('Supabase orthoGetGamesUnlock:', error.message); return null; }
    return data ? !!data.games_all_unlocked : null;
  },
  ...
  // v6.197 : volets 4 & 5 du dossier clinique — générés par l'IA À LA
  // DEMANDE (bouton), plafonnés, à valider par l'ortho. Rien n'est
  // stocké : la génération est un brouillon de travail, pas un dossier.
  async orthoAmiJournal(patientCode, lang){
    const r = await this.iaAssist('ami_journal', { patient_code: patientCode, lang: lang || 'fr' });
    return r.error ? r : { text: r.result };
  },
  async orthoClinicalReco(patientCode, lang){
    const r = await this.iaAssist('clinical_reco', { patient_code: patientCode, lang: lang || 'fr' });
    return r.error ? r : { text: r.result };
  },
  ...
  async addMedia(code, label, file){
    if(CLOUD_ENABLED){
      ...
      arr.push(item);
      // v6.245 : c'est ICI que le quota lâche en premier — une photo
      // convertie en base64 pèse ~1,3× le fichier d'origine, et le mode
      // navigateur n'a que ~5 Mo pour TOUT (photos, historique, journal).
      // Avant, l'exception remontait sans être attrapée : le patient ne
      // voyait ni confirmation ni erreur. On signale l'échec par la même
      // convention que le mode cloud (null), en distinguant la cause pour
      // que l'interface ne parle pas d'un « problème serveur » inexistant.
      ReParoleStore.lastStorageFull = false;
      if(!_lsSet(key, JSON.stringify(arr))){
        ReParoleStore.lastStorageFull = true;
        return null;
      }
      return item;
    }
  ...
  async listMedia(code){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:media:'+code, []);
    }
  },
  async deleteMedia(code, id){
    if(CLOUD_ENABLED){
      ...
      const key='reparole:media:'+code;
      const arr=_lsGet(key, []).filter(m=>m.id!==id);
      _lsSet(key, JSON.stringify(arr));
    }
  },
  ...
  async saveReportMeta(code, orthoCode, summary, periodStart, periodEnd){
    ...
    if(CLOUD_ENABLED){
      ...
      const arr=_lsGet(key, []);
      arr.push(record); _lsSet(key, JSON.stringify(arr));
    }
  },
  async listReports(code){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:reports:'+code, []).reverse();
    }
  },
  ...
  async listNotes(code){
    if(CLOUD_ENABLED){
      ...
      return _lsGet('reparole:notes:'+code, []).reverse();
    }
  },
  ...
  async loadPatientVisibleNotes(code){
    if(CLOUD_ENABLED){
      ...
      const arr = _lsGet('reparole:notes:'+code, []);
  ...
  // v6.187 : quête — l'ortho peut révéler tous les jeux pour un patient
  // (autorité clinicienne ; fonction SQL gatée par le rattachement).
  async orthoSetGamesUnlock(patientCode, unlock){
    if(!CLOUD_ENABLED) return { error: 'cloud requis' };
    await initCloud();
    const { error } = await supa.rpc('set_games_unlock', { p_code: patientCode, p_unlock: !!unlock });
    if(error){ console.warn('Supabase set_games_unlock:', error.message); return { error: error.message }; }
    return {};
  },

  async purgeLoginEvents(days){
    if(!CLOUD_ENABLED) return { error: 'cloud requis' };
    await initCloud();
    // v6.223 : durée au choix (7/14/30 j). Repli sur l'ancienne fonction
    // sans paramètre si admin_purge_login_events n'existe pas encore.
    let { error } = await supa.rpc('admin_purge_login_events', { p_days: days });
    if(error){ ({ error } = await supa.rpc('purge_login_events')); }
    if(error){ console.warn('Supabase purge_login_events:', error.message); return { error: error.message }; }
    return {};
  },

  // v6.250 (audit RGPD) : les erreurs techniques contiennent des
  // user_agent — donnée personnelle, donc purgeable comme les connexions.
  async purgeClientErrors(days){
    if(!CLOUD_ENABLED) return { error: 'cloud requis' };
    await initCloud();
    const { error } = await supa.rpc('admin_purge_client_errors', { p_days: days });
    if(error){ console.warn('Supabase purge_client_errors:', error.message); return { error: error.message }; }
    return {};
  },
...
// =====================================================================
//  v6.207 — RÉGLAGES PAIEMENT & TARIFS (table app_settings, clé 'billing')
//  Modifiables depuis l'espace admin, lus par l'app (tarifs ortho…).
//  Jamais bloquant : en cas d'échec, l'app garde ses valeurs par défaut.
// =====================================================================
// v6.209 — Activation Pro manuelle (paiement par lien bancaire confirmé
// hors app) : bascule un compte patient/ortho en 'pro' ou 'free'.
// Réservé aux admins (contrôle côté serveur, fonction admin_set_plan).
ReParoleStore.adminSetPlan = async function(accountType, identifier, plan){
  try{
    const { data, error } = await supabase.rpc('admin_set_plan',
      { p_type: accountType, p_id: identifier, p_plan: plan });
    return { ok: !error && data === 'ok', error };
  }catch(e){ return { ok:false, error:e }; }
};

ReParoleStore.getBillingSettings = async function(){
  try{
    const { data, error } = await supabase.from('app_settings')
      .select('value').eq('key','billing').maybeSingle();
    if(error || !data) return null;
    return data.value || null;
  }catch(e){ return null; }
};
ReParoleStore.saveBillingSettings = async function(settings){
  try{
    const { error } = await supabase.from('app_settings')
      .upsert({ key:'billing', value: settings, updated_at: new Date().toISOString() });
    return { error };
