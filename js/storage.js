...
const ReParoleStore = {
  ...
  async loadPatient(code){
    if(CLOUD_ENABLED){
      ...
      return {
        ...
        // v6.30 : BUG RÉEL trouvé en branchant le déverrouillage QA — ce
        // champ était absent de la conversion de la ligne Supabase en
        // objet JS. Conséquence concrète : même un patient passé "pro" à
        // la main dans Supabase (la méthode documentée depuis la v6.24)
        // se voyait afficher "free" partout, car user.plan était
        // toujours `undefined` après connexion. La colonne existe bien
        // dans la table (voir plus bas dans ce fichier), seule cette
        // ligne de mapping l'ignorait.
        plan:row.plan || 'free',
        // v6.32 : quota gratuit journalier — voir recordDailySession()
        // (js/app.js) et la migration sql/migrations/003_daily_quota.sql
        dailySessionsDate:row.daily_sessions_date || null,
        dailySessionsCount:row.daily_sessions_count || 0
      };
    } else {
  ...
  async savePatient(code, p){
    // v6.30 : p_plan ajouté au payload — jusqu'ici absent, donc le champ
    // "plan" n'était jamais transmis à Supabase par l'app, quelle que
    // soit sa valeur en mémoire. Nécessite la migration SQL
    // sql/migrations/002_plan_upsert.sql (upsert_patient doit accepter
    // ce paramètre) pour prendre effet en mode cloud — voir ce fichier
    // pour le détail. En mode navigateur (localStorage), l'ajout ci-
    // dessous suffit, aucune migration nécessaire.
    // v6.32 : p_daily_sessions_date/p_daily_sessions_count ajoutés — voir
    // sql/migrations/003_daily_quota.sql (migration requise pour que ça
    // prenne effet en mode cloud, même principe que p_plan en v6.30).
    const payload = { p_code:code, p_name:p.name, p_level:p.level, p_sessions:p.sessions,
      p_correct:p.correct, p_total:p.total, p_streak:p.streak, p_plan:p.plan || null,
      p_daily_sessions_date:p.dailySessionsDate ?? null, p_daily_sessions_count:p.dailySessionsCount ?? null };
    if(CLOUD_ENABLED){
      await initCloud();
      ...
      const record = { code, name:p.name, level:p.level, sessions:p.sessions, correct:p.correct, total:p.total, streak:p.streak, plan:p.plan || 'free',
        // v6.32 : quota gratuit journalier — voir recordDailySession() (js/app.js)
        dailySessionsDate:p.dailySessionsDate||null, dailySessionsCount:p.dailySessionsCount||0,
