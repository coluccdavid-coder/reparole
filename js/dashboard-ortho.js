...
const OrthoApp = {
  ...
  async openPatient(code){
    ...
    OrthoApp.syncGamesUnlock(); // v6.240 : confirme la valeur depuis la base
  ...
  // =====================================================================
  // v6.240 : la case « révéler tous les jeux » s'appuyait sur l'instantané
  // renvoyé par listPatients(). Si cette copie ne portait pas la colonne,
  // la case revenait décochée alors que la base disait l'inverse (bug
  // signalé). On relit donc la valeur réelle, sans bloquer l'affichage.
  syncGamesUnlock(){
    const el = document.getElementById('games-unlock-toggle');
    if(!el || !currentPatient || !Store.orthoGetGamesUnlock) return;
    Store.orthoGetGamesUnlock(currentPatient.code).then(function(val){
      if(val === null || val === undefined) return;
      currentPatient.games_all_unlocked = val;
      el.checked = val;
    }).catch(function(){});
  },
