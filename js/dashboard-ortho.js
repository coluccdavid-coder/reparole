...
function levelName(n){ const lvl=[1,2,3].includes(n)?n:2; return (window.I18N && I18N.t('level_'+lvl)) || lvl; } // v6.31 : filet de sécurité (voir SKILL_ReParole_v6.md)
...
const OrthoApp = {
  ...
  openReport(){
    if(!currentPatient) return;
    // v6.29 : orthoName ajouté pour le bilan PDF professionnel — affiché
    // sur le document sous "Réalisé par :". Reste facultatif côté
    // report.html (repli discret si absent) pour ne rien casser si ce
    // lien a été généré par une ancienne version en cache.
    const url = `report.html?code=${encodeURIComponent(currentPatient.code)}&ortho=${encodeURIComponent(orthoCode)}&orthoName=${encodeURIComponent(orthoName||'')}`;
