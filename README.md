...
## v6.178 — bug i18n attrapé sur capture + très grands écrans (3 colonnes) + connexion sur une page

**1. Bug corrigé — `ortho_voice_title` s'affichait en toutes lettres.**
4 clés de la boucle vocale côté ortho (titre, sous-titre, et les deux
boutons de verdict) étaient référencées dans le HTML/JS depuis la v6.174
mais **jamais définies** — le nom brut s'affichait (repéré par
l'utilisateur sur capture, fiche patient). Ajoutées dans les 14 langues.
Pourquoi aucun test ne l'a vu : les tests de **parité** comparent les
langues entre elles — une clé absente de toutes les langues est
« cohérente ». Nouveau `tests/i18n-usage.test.js` : il balaie **tous**
les attributs `data-i18n*` de toutes les pages et **tous** les appels
littéraux `I18N.t/oT/cgT` du JS, et exige que chaque clé existe. Le
balayage confirme que ces 4 clés étaient les seules manquantes — et une
clé référencée demain sans être définie fera échouer la suite.

**2. Très grands écrans : fini les côtés vides.** Retour : « on a encore
pas mal d'espace sur les côtés ». Nouveau palier `≥1500px` : les tableaux
de bord (patient, ortho liste + fiche, aidant, admin) s'élargissent à
1720 px et les cartes passent sur **3 colonnes** ; les exercices passent
à 1100 px. Entre 900 et 1500 px : 2 colonnes comme avant ; téléphone :
inchangé. L'**admin** rejoint le mécanisme commun : son tableau (jusqu'ici
figé à 820 px) passe en pleine largeur avec ses cartes en grille.

**3. Connexion patient : tenir sur UNE page.** Retour : « toujours pas
sur une page ». Nouveau resserrement des espacements sur grand écran
(paddings de carte et de panneau, marges des champs, des liens et des
notes) — toujours **sans toucher aux tailles de police**. Téléphone
inchangé.

`CACHE_NAME` incrémenté (`reparole-v6-177` → `reparole-v6-178`).

## v6.179 — traitement d'une suggestion reçue : sombre moins noir, clair moins blanc

La boîte à idées contenait un retour d'aidant·e (17/07) : « trop de vide
tout autour, c'est vachement sombre le mode sombre et vachement clair ».
Le « vide autour » a été traité en v6.178 (3 colonnes, 1720 px). Cette
version traite les deux ambiances, par retouches volontairement
conservatrices :

**Mode sombre** : fond relevé et teinté vert sapin (`#121815` →
`#17211c`), cartes légèrement éclaircies (`#1b2420` → `#1f2a24`) pour se
détacher du fond — moins « noir écran éteint », plus « soirée ». Les
dégradés des connexions, le décor et les contrastes de texte ne changent
pas.

**Mode clair** : fond adouci (`#F6FAF7` → `#EDF4F0`) — moins éblouissant,
les cartes restent blanches pour garder le relief.

De la couleur pure : à juger à l'œil dans les deux modes, facile à pousser
plus loin ou à revenir en arrière. Une fois satisfait, penser à « Marquer
lu » la suggestion dans l'admin.

`CACHE_NAME` incrémenté (`reparole-v6-178` → `reparole-v6-179`).

## v6.180 — cette fois c'est VISIBLE : sombre nettement relevé + la connexion occupe l'écran

Retour : « rien n'a changé, on n'occupe toujours pas l'espace, c'est
toujours trop sombre ». Deux causes, toutes deux de notre côté :

**1. La v6.179 était trop timide pour être perçue.** Passer le fond de
`#121815` à `#17211c`, personne ne le voit. Cette fois le bond est net :
fond sombre `#1e2a24`, cartes `#28362f` — un vrai vert sapin, plus un
noir. Et la **carte de connexion en mode sombre** cesse d'être un bloc
noir posé sur le dégradé : teinte vert-bleuté en dégradé doux + fine
bordure lumineuse, en harmonie avec le fond au lieu de le trouer. Le
panneau « Votre confort » suit (teinte translucide).

**2. Le palier grands écrans (v6.178) ne visait que les tableaux de
bord — pas la connexion.** Paliers dédiés ajoutés : la carte de
connexion patient passe à 1260 px dès 1200 px d'écran et 1440 px dès
1600 px, avec espacement des colonnes élargi. Elle occupe l'écran au
lieu de flotter au milieu.

**Si « rien ne change » après déploiement** : c'est le cache de
l'application (PWA) — recharger avec Ctrl+F5, ou attendre la bannière de
mise à jour et cliquer dessus. La version active se vérifie dans les
outils du navigateur (Application → Service Workers → `reparole-v6-180`).

`CACHE_NAME` incrémenté (`reparole-v6-179` → `reparole-v6-180`).

## v6.181 — les écrans d'activité occupent l'écran (exercices, bilan, conversation, mémoire, phonation)

Retour (captures d'exercice « Associer les images » et de fin de séance) :
l'espace reste à résoudre — le contenu plafonnait vers 950 px au milieu
d'un grand écran. Les écrans d'activité rejoignent les paliers de la
connexion : **1280 px dès 1200 px d'écran, 1480 px dès 1600 px**, pour
`#exercise`, `#assessment`, `#conversation`, `#memory` et `#phonation`.
Les choix de réponse (grille auto) profitent directement de la largeur.
**Téléphone : non impacté** — tout est dans des paliers ≥1200 px, la
colonne unique mobile ne change pas.

`CACHE_NAME` incrémenté (`reparole-v6-180` → `reparole-v6-181`).

## v6.182 — la suite IA : « l'IA prépare, l'humain décide », partout

L'edge function devient **`ia-assist`, multi-tâches** : un seul
déploiement, six usages, les mêmes garanties partout (jeton de session,
rôle re-vérifié côté serveur — rattachement patient pour l'ortho, table
`admins` pour l'admin —, **anonymisation**, plafond porté à 40
appels/jour toutes tâches confondues, clé API en secret serveur). Chaque
prompt commence par la même règle absolue, verrouillée par test : *tu
prépares, l'humain décide — aucun diagnostic, aucune décision*.

**Côté orthophoniste (Pro) — 3 assistances de plus :**
- **✨ Suggérer des mots (IA)** dans la carte Mots ciblés : propositions
  d'après les mots souvent en erreur (en évitant les déjà ciblés), avec
  la raison en 6 mots. **Aucun ajout automatique** : chaque mot n'est
  ciblé que si l'ortho clique dessus — verrouillé par test (orthoAddWord
  ne peut être appelé que dans le gestionnaire de clic).
- **🗒️ Préparer la séance (IA)** : note de 5-8 lignes à lire en 30
  secondes avant le rendez-vous (volume, tendance, mots bloquants,
  verdicts vocaux en attente) — dans la zone éditable, jamais stockée.
- **✨ Reformuler (IA)** sous les notes cliniques : le brouillon est
  reformulé DANS le champ ; relire, ajuster, et c'est toujours l'ortho
  qui ajoute la note.

**Côté admin — 2 assistances (page en français, ton propre compte) :**
- **✨ Analyser les suggestions (IA)** : la boîte à idées classée par
  thème, bugs probables signalés en tête — les **emails de contact ne
  sont pas transmis** à l'IA. Suite directe du message raté dans la
  boîte à idées (v6.179) : plus rien ne devrait passer inaperçu.
- **✨ Résumer les causes probables (IA)** sur les erreurs techniques.

**Positions maintenues** (assumées, non négociables) : pas de scoring IA
des enregistrements vocaux (le verdict humain EST le différenciant), pas
d'IA en conversation directe avec le patient sans validation d'un∙e
orthophoniste, Ami reste scripté.

Traductions : 9 clés × 14 langues (validées par le test d'usage i18n).
Nouveau `tests/ia-suite.test.js` (8 tests d'invariants).

⚠ **Déploiement** : la fonction s'appelle désormais `ia-assist`
(remplace `generate-report-draft` si elle avait été déployée) — guide
complet dans `js/ia-edge-function.md`. Sans déploiement, chaque bouton
affiche « fonction non déployée », rien ne casse.

`CACHE_NAME` incrémenté (`reparole-v6-181` → `reparole-v6-182`).

## v6.183 — l'IA crée des exercices sur mesure (validés par l'ortho) + veille scientifique avec vraie recherche web

Deux nouvelles tâches derrière la même edge function `ia-assist` (8 au
total), mêmes garanties (jeton de session, rôles re-vérifiés,
anonymisation, plafond, clé serveur), même principe : **l'IA prépare,
l'humain décide**.

**1. Exercices sur mesure (ortho, Pro).** Nouvelle carte dans la fiche
patient : l'IA rédige UN exercice personnalisé (6 questions, schéma
`text/choices/answer` du moteur de compréhension existant) d'après les
catégories d'erreurs dominantes, les mots ciblés et le niveau — en
variant par rapport aux exercices déjà créés. L'ortho **prévisualise
chaque question** puis « ✅ Proposer au patient » ou « Ignorer » — la
génération n'enregistre RIEN, seule la validation envoie (verrouillé par
test : `addCustomExercise` n'existe que dans `acceptExercise`). Côté SQL :
table `custom_exercises` + fonctions gatées `auth.uid()` +
rattachement. Côté patient : carte « ✨ Exercices de votre
orthophoniste », et la session tourne dans le **moteur existant**
(`current={…}` comme les photos personnelles) avec garde de robustesse
sur les payloads malformés. Contenu dynamique lu par la synthèse du
navigateur : **pas de régénération de voix**.

**2. Veille scientifique (admin).** « L'IA apprend-elle des nouveautés ? »
— honnêtement : un modèle seul ne peut que réciter sa mémoire, datée, au
risque d'inventer des études. La tâche `research_exercises` branche donc
la **recherche web réelle** (outil Anthropic `web_search`) : pistes
récentes en rééducation de l'aphasie adaptables à l'écran, chacune avec
sa **SOURCE (URL)**, et l'instruction explicite « si les résultats sont
incertains, dis-le honnêtement plutôt que d'inventer ». L'admin lit les
sources et décide ce qui devient un exercice.

**Adaptation au patient** : chaque génération repart des données de suivi
fraîches (erreurs, niveau, mots ciblés, exercices déjà créés) — c'est le
moteur adaptatif existant + l'IA générative par-dessus, sans « boîte
noire » qui déciderait à la place de qui que ce soit.

Traductions : 12 clés × 14 langues (dont kabyle réel — et le test vérifie
maintenant l'absence de caractères parasites). Nouveau
`tests/ia-exercises.test.js` (9 tests). `tests/dashboard-grid` passe à
13 cartes.

⚠ **Rejouer `sql/schema.sql`** (table `custom_exercises`) et
**redéployer `ia-assist`** (2 tâches de plus) — guide dans
`js/ia-edge-function.md`. Note : la tâche de veille avec recherche web
coûte plus cher qu'un appel simple (quelques centimes) — plafond partagé
inchangé.

`CACHE_NAME` incrémenté (`reparole-v6-182` → `reparole-v6-183`).

## v6.184 — deux jeux adaptatifs, dans les 14 langues, gratuits

Demande : « je veux qu'il crée des jeux, une vraie IA adaptative pour le
patient, et surtout dans toutes les langues » (avec des sources de
rééducation à l'appui — la répétition intensive qui déclenche la
neuroplasticité, cf. Flint Rehab).

**Architecture assumée, verrouillée par les tests** : la MÉCANIQUE des
jeux est du code ; le CONTENU vient des banques déjà traduites et
relues des 14 langues (kabyle et darijas comprises) + les mots ciblés
par l'ortho/l'aidant (déjà fusionnés) ; l'ADAPTATION vient du moteur
d'apprentissage existant (niveau par type → longueur des mots et
subtilité des pièges ; chaque réponse nourrit `AI.record`). **Aucun LLM
face au patient** — c'est précisément ce qui rend le « toutes les
langues » honnête (garde-fou n°8) : un modèle génératif est faible en
kabyle, les banques relues ne le sont pas.

**🧩 « Le mot en morceaux »** — l'image en modèle, les lettres mélangées
(jamais présentées déjà dans l'ordre), le patient les touche dans
l'ordre. « Effacer » repart de zéro **sans pénalité**. La longueur des
mots suit le niveau (5 → 7 → 10 lettres).

**🎯 « Ça va ensemble ? »** — vérification mot-image (tâche clinique
classique) : une image, un mot, correspondent-ils ? Moitié de pièges
plausibles (mot d'un autre item du même niveau). Deux grandes cibles
Oui/Non, **sans chronomètre** — la fatigue post-AVC est réelle, la
série remplace le compte à rebours.

Les deux jeux sont **gratuits** (hors liste Pro — l'esprit « accessible
à tous »), concluent par le même `answer_feedback` que tout le reste
(mêmes encouragements d'Ami, même fin de séance, même apprentissage), et
les mots retrouvés difficilement déclenchent la reconnaissance fine de la
v6.131. 9 clés × 14 langues.

⚠ **Voix** : les mots des jeux viennent des banques déjà voicées — rien à
régénérer pour eux. Les 2 nouvelles consignes (`anagram_hint`,
`verif_hint`) passeront par la synthèse du navigateur tant que
l'extraction/génération n'a pas été relancée (même commande que
d'habitude, à l'occasion).

`CACHE_NAME` incrémenté (`reparole-v6-183` → `reparole-v6-184`).

## v6.185 — 4 ajustements sur captures : centrage en Lecture facilitée, emojis abstraits filtrés, footer collé en bas, intonation

**1. Le contenu des exercices était à gauche — cause trouvée : le mode
« Lecture facilitée » (dys).** Une règle historique alignait TOUT le
contenu des cartes à gauche en mode dys. L'intention (la gauche aide la
lecture des *phrases* en dyslexie) était bonne mais trop large : un
emoji, un mot seul, un micro n'ont aucune raison d'être à gauche.
Raffiné : **visuels centrés, consignes (`.prompt-text`) à gauche**
(`text-align:start` — compatible RTL). Ça corrige d'un coup les jeux,
« Répéter avec intonation », la dénomination orale, etc. Les jeux de la
v6.184 passent aussi sur les classes standard (`prompt-emoji`,
`prompt-main`), « Effacer » se centre, et les boutons Oui/Non
s'affichent côte à côte au lieu d'un bouton étiré sur toute la largeur.

**2. « C'est quoi au-dessus de CHAMPIGNON ? »** — c'était la bulle de
parole 💬 : un piège *volontaire* du jeu (image d'un autre mot, réponse
attendue « Non »), mais le retour est juste : un emoji abstrait n'est
pas une image reconnaissable pour ce public. **Filtre ajouté** : bulles,
flèches, symboles (💬🗨️💭❓⭐✨💡🔔…) sont exclus du pool des deux jeux —
une image de jeu doit être un objet concret.

**3. Mentions légales au milieu de l'écran** (jeu de mémoire et autres
écrans courts) : le pied de page ne se poussait pas en bas. Correctif
global : body en colonne pleine hauteur, l'écran visible s'étire, le
footer descend (`margin-top:auto`) — vérifié sur l'ensemble des pages
(elles partagent style.css et js/footer.js).

**4. « Répéter avec intonation »** : couvert par le raffinement du point
1 (consigne à gauche, flèche d'intonation + phrase + micro centrés).

Tout est verrouillé par tests (filtre d'emojis dans adaptive-games,
dys raffiné + footer dans screens-completeness).

`CACHE_NAME` incrémenté (`reparole-v6-184` → `reparole-v6-185`).

## v6.186 — « je peux pas tester » réglé (IA ouverte en bêta + erreurs claires), ménage de l'historique, accueil embelli

**1. « Je peux pas tester » — deux causes, deux correctifs.**
- Le « NetworkError when attempting to fetch resource » brut (captures
  admin) : quand l'edge function n'est **pas déployée**, le navigateur
  échoue dès le pré-vol CORS — le fetch lève une exception AVANT
  d'obtenir un 404, donc le message « fonction non déployée » ne
  s'affichait jamais. Corrigé : tout échec réseau de `iaAssist` se mappe
  vers `indisponible` → chaque bouton affiche désormais « Fonction IA
  non déployée (voir js/ia-edge-function.md) » au lieu du jargon.
- Le paywall Pro bloquait le développeur sur son propre compte
  (capture « Fonction réservée à l'abonnement Pro »). Nouvel
  interrupteur **`IA_BETA_OPEN = true`** (helper `iaAllowed()`,
  documenté « à passer à false au lancement commercial ») : pendant la
  bêta, l'IA ortho est ouverte sans abonnement. Le plafond serveur
  (40 appels/jour) reste actif dans tous les cas, et le contrôle Pro
  reste dans le code, prêt pour le lancement.
- ⚠ Rappel : pour tester réellement, il reste à **déployer `ia-assist`**
  (guide pas à pas : `js/ia-edge-function.md` — `supabase functions new
  ia-assist`, coller le code, `deploy`, secret `ANTHROPIC_API_KEY`) et à
  **rejouer `sql/schema.sql`**.

**2. Historique des connexions : bouton « 🧹 Faire le ménage »** —
supprime les connexions de plus de 30 jours. Fonction SQL
`purge_login_events()` gatée par la table `admins`, double clic de
confirmation (pas de `confirm()` natif), liste rafraîchie après.

**3. L'encadré blanc de l'accueil embelli** (mode clair) : dégradé
blanc → vert d'eau très doux, fine bordure lumineuse assortie, ombre
teintée sarcelle. Le mode sombre garde son traitement v6.180.

Verrous de tests : mapping `indisponible`, `iaAllowed` (bêta documentée
+ contrôle Pro conservé), purge gatée + double confirmation, dégradé de
la carte.

`CACHE_NAME` incrémenté (`reparole-v6-185` → `reparole-v6-186`).

## v6.187 — le grand chantier : mots croisés illustrés + quête-découverte + récit d'évolution + vue cabinet

Programme acté des deux derniers jours, réalisé d'un bloc (« pour les
décisions je te fais confiance » — les arbitrages retenus sont notés).

**🔡 « La grille en images » (mots croisés illustrés)** — PAS de
définitions textuelles (double obstacle aphasique) : **l'indice est
l'image**. Mini-grilles de 2-3 mots qui se croisent, générées
algorithmiquement depuis la banque de la langue active (14 langues),
**mots ciblés ortho/aidant en priorité**. Saisie par banque de lettres
mélangées, Effacer sans pénalité, pas de chronomètre, gratuit. Les
lettres résolues restent gravées : un mot trouvé AIDE le suivant —
c'est tout l'intérêt cognitif du croisement (même en cas d'erreur, la
grille se complète avec les bonnes lettres pour rester jouable).
**RTL** : coordonnées internes stables, l'affichage inverse les
colonnes pour ar/dz/ma/tn (lettres isolées, standard des mots croisés
arabes). **Repli honnête** : si la banque d'une langue ne permet aucun
croisement (kana japonais rare), message clair plutôt qu'un écran cassé.
Chaque mot conclut par `answer_feedback` → moteur d'apprentissage nourri.

**🎁 Quête-découverte (célébration, jamais barrière)** — les tuiles de
JEU (et seulement elles — jamais les exercices thérapeutiques) se
révèlent progressivement : 1 séance → Le mot en morceaux, 2 → Ça va
ensemble, 3 → La grille en images. Arbitrages : **chaîne fixe** (v1
simple et prévisible), **seuils dérivés de `user.sessions`** (rien à
stocker, et un patient existant à 3+ séances voit tout d'office),
**trois soupapes** : lien discret « voir tous les jeux » (jamais de
vrai mur), **interrupteur de l'ortho** dans la fiche patient
(`patients.games_all_unlocked` + `set_games_unlock` gatée par le
rattachement — l'autorité clinicienne prime sur le système), et
célébration locale (étincelle + mot d'Ami, une fois par appareil,
`prefers-reduced-motion` respecté).

**📈 Récit d'évolution (IA, ortho)** — dans la carte Rapport : période
7/30/90 jours, l'IA compare le début et la fin de la période (volume,
tendances par type, mots débloqués, productions vocales) en 8-12 lignes
lisibles par la famille comme par un confrère — dans la zone éditable,
à relire et valider.

**☀️ Vue cabinet (IA, ortho)** — nouvelle carte en tête de la liste des
patients : le matin en 20 secondes — inactifs 7 j+, plateaux, progrès
nets, verdicts vocaux en attente. **Anonymat par lettres** (A, B, C…) :
aucun nom ne part vers l'IA, verrouillé par test. `ia-assist` passe à
**10 tâches** (7 ortho, 3 admin) ; `cabinet_digest` est la seule tâche
ortho sans patient (rôle vérifié par les rattachements du compte).

17 clés × 14 langues. Nouveau `tests/quest-crosswords.test.js`
(10 tests). ⚠ **Rejouer `sql/schema.sql`** (colonne
`games_all_unlocked` + `set_games_unlock`) et **redéployer `ia-assist`**
(2 tâches de plus). Voix : mots des grilles déjà voicés ; les nouvelles
consignes passeront par la synthèse du navigateur jusqu'à la prochaine
régénération.

`CACHE_NAME` incrémenté (`reparole-v6-186` → `reparole-v6-187`).

## v6.188 — retours sur toto : images jumelles corrigées, journal vivant, espace aidant discret, veille pour l'ortho

**1. « J'ai un vrai doute sur les images »** — bien vu, et le diagnostic
est subtil : PLAGE utilisait 🏖️ et PARASOL ⛱️ — deux emojis
*différents* dans le code, mais **quasi identiques à l'écran** (un
parasol planté dans le sable, dans les deux cas). Dans une même séance,
deux « mêmes » images avec deux réponses attendues ≠ piège involontaire.
Correctif : PLAGE passe à **🏝️** partout où les deux coexistent (fr,
dz, ja, kab) — les mots ne changent pas, donc **aucune voix à
régénérer**. Et l'audit devient un **test permanent** : aucun emoji
(normalisé Unicode) ne peut plus pointer vers deux réponses différentes
dans une banque, et 🏖️/⛱️ ne peuvent plus coexister.

**2. « Je vais moyen depuis le 10 juillet »** — une humeur d'il y a dix
jours ne s'affiche plus comme si c'était aujourd'hui : les entrées du
journal portent un libellé relatif (« 10 juillet 2026 · il y a 9
jours »), et si la dernière note a plus de 2 jours, une invitation
douce apparaît (« comment ça va aujourd'hui ? »). Le journal reste un
espace libre, jamais analysé.

**3. Espace aidant masqué à la demande** — les détails (dont le **code
d'accès**, qui n'a pas à rester affiché en permanence à l'écran) sont
repliés par défaut derrière « 👁️ Afficher l'espace aidant »
(aria-expanded, état non persisté : masqué à chaque chargement).

**4. « Pourquoi je n'ai plus la veille dans l'espace ortho ? »** — elle
n'y a jamais été : la veille scientifique était **côté admin
uniquement**. Décision : elle sert autant le cabinet que le produit →
nouvelle carte **🔭 Veille scientifique (IA + web)** dans l'écran liste
de l'espace ortho (recherche web réelle, sources exigées, pistes à lire
— pas des consignes). `research_exercises` devient une tâche ortho ET
admin (11 tâches au total).

**5. « L'appel IA c'est où ? »** — le message rouge de ta capture
(« Fonction IA non déployée ») est le comportement attendu tant que
l'edge function n'existe pas sur ton projet Supabase. Le déploiement
(une fois, ~10 min) : voir le guide pas à pas dans
`js/ia-edge-function.md` — puis tous les boutons IA prennent vie.

8 clés × 14 langues (dont la première clé-fonction plurielle « il y a N
jours »). Nouveau `tests/feedback-v188.test.js` (4 tests).

`CACHE_NAME` incrémenté (`reparole-v6-187` → `reparole-v6-188`).

## v6.189 — correctif critique : 2 bugs dans le code de l'edge function (guide `ia-assist`)

Trouvés au moment du déploiement réel, en relisant le code collé —
**tous deux introduits par mes éditions automatiques du guide en
v6.187** :

1. **Parenthèse en trop** dans la condition des 5 tâches à données
   patient (`…'generate_exercise') || task === 'evolution_story')`) —
   le code ne compilait pas, le déploiement échouait.
2. **`evolution_story` tombait dans le `else` de `generate_exercise`**
   avant de recevoir son propre prompt : `wantJson` restait à `true`,
   et la prose du récit d'évolution aurait été rejetée en « réponse IA
   non exploitable » à chaque appel. `generate_exercise` devient un
   `else if` explicite.

Verrou ajouté dans `tests/ia-suite.test.js` : équilibre des
parenthèses/accolades/crochets du bloc ts du guide + les deux formes
corrigées exigées. Si le fichier `index.ts` a déjà été collé depuis la
v6.187/188 : recopier le bloc corrigé (ou appliquer les deux
remplacements ci-dessus) puis `supabase functions deploy ia-assist`.

`CACHE_NAME` incrémenté (`reparole-v6-188` → `reparole-v6-189`).

## v6.190 — l'IA parle… et les données arrivent enfin : 5 collisions edge/schéma corrigées

Premier appel réel réussi (vue cabinet + veille sourcée) — et premier
mensonge détecté : « Patient A inactif depuis 30+ jours » alors que
toto avait joué le matin même. Cause : la fonction edge interrogeait
des colonnes qui n'existent pas ; PostgREST renvoyait vide, l'IA
concluait honnêtement sur des données fausses. Audit complet edge ↔
`schema.sql` :

1. `sessions.correct` → **`score`** (collecte patient + vue cabinet) ;
2. `error_events.word` → **`target`** (les catégories d'erreurs
   revenaient vides) ;
3. `caregiver_words.source` → n'existe pas (liste de mots simple) ;
4. `suggestions.role` → **`source`** ;
5. `admins.user_id` → **`code`** (le tri de suggestions et le digest
   d'erreurs auraient répondu « réservé aux administrateurs » à
   l'admin lui-même).

Bonus lisibilité : la règle commune impose désormais le **texte brut**
(fini les `##`/`**` Markdown affichés tels quels dans les cartes).

Verrou `tests/ia-suite.test.js` : les colonnes interrogées par l'edge
doivent exister dans `schema.sql` (assertions croisées des deux
fichiers). **Action : re-glisser le nouvel `index.ts` puis
`supabase functions deploy ia-assist`** — pas de SQL à rejouer.

`CACHE_NAME` incrémenté (`reparole-v6-189` → `reparole-v6-190`).

## v6.191 — l'ordre de bataille : de l'outil à la présence (② → ④ → ① → ③)

Les quatre chantiers demandés, réalisés d'un bloc — **aucun LLM côté
patient**, tout est scripté sur données réelles.

**② Une seule action principale** — carte **☀️ Aujourd'hui** en tête du
tableau de bord patient : UN gros bouton « ▶ Continuer » branché sur le
même moteur de recommandation que le badge « Recommandé »
(`AI.recommend`). Tout le reste reste accessible mais recule d'un cran.

**④ Personnalisation progressive** — l'app PROPOSE au bon moment au
lieu d'attendre qu'on fouille : moteur de « nudges » (une proposition
par session AU MAXIMUM, refus mémorisé en localStorage et jamais
ré-imposé, application uniquement au clic — verrouillé par test). Deux
règles v1 : le soir → mode sombre ; après 15 séances → « Ami peut se
faire plus discret ». Et LE réglage qui rend le compagnon respectueux :
**niveau d'accompagnement d'Ami** (🤗 Présent / Discret / Silencieux)
dans la rangée d'accessibilité — « discret » ne garde que l'important
(célébrations de quête), « silencieux » se tait vraiment (consignes
d'Ami comprises).

**① La mémoire d'Ami (scriptée)** — Ami mentionne des faits réels, une
phrase choisie par priorité : retour après 3 jours et plus (« Vous
revoilà après N jours — reprenons en douceur »), série en cours (« N
jours d'affilée »), ou mot qui résiste (à partir de 3 erreurs
seulement — pas de stigmatisation au premier raté). Chaleur constante
et attention réelle, **jamais d'émotion prétendue** : verrouillé par un
test qui balaie les 3 clés × 14 langues à la recherche d'émotions en
première personne. Protégé par try/catch : la mémoire ne peut jamais
casser le tableau de bord. Respecte le niveau ④.

**③ La fiche ortho raconte avant de chiffrer** — encadré 📖 sous
l'en-tête de la fiche : 2-4 phrases générées LOCALEMENT (zéro appel
IA) depuis les données déjà chargées — volume/réussite/série, tendance
(`AI.trend`), catégorie d'erreurs dominante, dernière activité. Les
chiffres restent en dessous pour qui veut creuser ; le récit IA de
v6.187 reste le grand format à la demande.

19 clés × 14 langues (dont 8 clés-fonctions). Nouveau
`tests/battle-plan-v191.test.js` (6 tests). Pas de SQL, pas d'edge —
rien à redéployer côté serveur pour cette version.

`CACHE_NAME` incrémenté (`reparole-v6-190` → `reparole-v6-191`).

## v6.192 — la documentation devient un verrou (« il faut que tout soit documenté »)

Nouveau dossier `docs/` : **INDEX.md** (portail + ligne « Version
documentée »), **ARCHITECTURE.md** (pile, carte des fichiers, flux,
sécurité), **FONCTIONNALITES.md** (les 4 espaces, les 20 exercices, les
14 langues, la quête, Ami, la personnalisation), **IA.md** (les 11
tâches, garde-fous, coûts, dépannage), **DEPLOIEMENT.md** (site + edge
+ SQL + voix, avec TOUS les pièges réellement rencontrés au premier
déploiement : ref minuscules vs ORG ID, session expirée, presse-papiers
qui corrompt les backticks, `index(1).ts`, dossier root…),
**DONNEES.md** (les 20 tables, ce qui part ou non vers l'IA, RGPD/HDS),
**DEVELOPPEMENT.md** (discipline de version, conventions i18n, banques,
tests).

Et surtout : `tests/documentation.test.js` (8 tests) rend la
documentation **obligatoire et synchronisée** — la version courante de
`sw.js` doit figurer dans INDEX.md, chaque table de `schema.sql` dans
DONNEES.md, chaque tuile `data-type` dans FONCTIONNALITES.md, chaque
tâche IA dans IA.md, les 14 langues listées, et les gestes vitaux du
déploiement écrits noir sur blanc (dont la protection du dossier
`audio/`). **Une version non documentée fait échouer la suite.**

`CACHE_NAME` incrémenté (`reparole-v6-191` → `reparole-v6-192`).

## v6.193 — l'écran liste ortho remplit enfin la largeur + PREMIER COMPTE-RENDU IA RÉEL

**Jour historique consigné** : après le redéploiement v6.190, la suite
IA fonctionne de bout en bout avec les VRAIES données — premier
brouillon de compte-rendu complet généré sur le dossier de test
(assiduité, résultats par type, analyse d'erreurs 92 % sémantiques,
mots en difficulté, mention du ciblage antérieur de MAISON, disclaimers
en place, aucun nom). Pistes d'amélioration notées pour un futur lot de
prompts (voir ci-dessous) — pas de redéploiement pour du cosmétique.

**Correctif visuel** (signalé capture à l'appui) : sur très grand écran
(≥1700px), la mosaïque de l'écran LISTE de l'espace ortho passait à 3
colonnes alors qu'il n'a que 3 cartes courtes — la 3e colonne restait
vide (un tiers de page blanc à droite). Cet écran reste désormais en
**2 colonnes larges** à toutes les tailles.

**Pistes prompts v2 (notées, non déployées)** : libellés humains des
types d'exercices (l'IA francise les codes : « complétude », « exercices
de croisés ») ; contexte du score de fluence (0 % brut trompeur) ; les
items de phrases à compléter présentés comme tels (pas « phrases
complexes »).

Rappel discipline : plus de zip par version — **un zip global sur
demande** (consigne mémorisée).

`CACHE_NAME` incrémenté (`reparole-v6-192` → `reparole-v6-193`).

## v6.194 — le brouillon IA s'imprime en PDF, la veille se copie

**📄 Imprimer / PDF** sous le brouillon de compte-rendu (fiche patient,
carte Rapport) : ouvre une page d'impression propre (police sérif,
en-tête avec nom du patient + date, disclaimer imprimé en pied) et
lance l'impression — « Enregistrer en PDF » du navigateur, comme le
rapport standard. Point clé : on imprime le **contenu ACTUEL du
textarea**, donc la version **relue et corrigée** par l'orthophoniste,
pas la sortie brute de l'IA (verrouillé par test, disclaimer compris).

**📋 Copier** sous les sorties de veille (ortho ET admin) et sous la
vue cabinet — réponse au « et après, il se passe quoi ? » : par
conception, RIEN d'automatique (l'humain décide) ; la copie est le pont
vers les notes, un document de travail, ou un partage. La veille n'est
volontairement pas stockée : ce qui mérite d'être gardé est choisi par
un humain.

2 clés × 14 langues. Verrou dans `tests/ia-suite.test.js`.

`CACHE_NAME` incrémenté (`reparole-v6-193` → `reparole-v6-194`).

## v6.195 — 🧠 « La liste à retenir » : le premier chantier commandé par la veille IA

**Boucle produit historique** : la vue cabinet a dit la vérité sur toto
(actif, 21 séances, 60 %, « situation stable, pas de signal
particulier ») — la suite IA est validée de bout en bout. Et la piste
n°1 de la première veille (fonctions exécutives — mémoire de travail,
rappel de listes) devient un exercice réel.

**🧠 La liste à retenir** (`memoire_liste`) — mémoire de travail :
mémoriser 2-4 images+mots (selon le niveau du jeu), puis les retrouver
parmi des distracteurs. **Auto-rythmé, sans chronomètre** (bouton
« J'ai mémorisé » — adapté à l'aphasie), **rappel LIBRE** (l'ordre
n'est pas demandé — v1, le rappel sériel serait une marche trop haute).
Contenu des banques existantes : **14 langues d'office, zéro
traduction, mots déjà voicés**. Dédoublonnage par emoji (sélection
jamais ambiguë), emojis abstraits exclus, gratuit, nourrit le moteur
adaptatif. Hors quête (chaîne v1 fixe — tuile visible directement).

**Prompts v2** (dans `js/ia-edge-function.md` — partiront au prochain
redéploiement de `ia-assist`, pas d'urgence) : carte `libelles_types`
des 21 exercices transmise à l'IA (fini « complétude » et « exercices
de croisés »), consigne fluence (pas un pourcentage classique),
consigne complétion (mots cibles, pas « phrases complexes »).

**Bandeau d'inactivité honnête** (écran liste ortho) : « 🔴 Aucune
séance depuis plus de 7 jours » n'apparaît QUE si un patient est
réellement inactif — affiché en permanence il semait le doute (signalé).

6 clés × 14 langues. `tests/memoire-v195.test.js` (4 tests). ⚠ Le
prochain redéploiement de l'edge embarquera les prompts v2.

`CACHE_NAME` incrémenté (`reparole-v6-194` → `reparole-v6-195`).

## v6.196 — le grand audit des traductions (« rien au hasard ») + verrou éthique vocal

Audit systématique des 14 langues, demandé par le propriétaire. Butin :

**27 clés fantômes éliminées** : 14 + 13 clés utilisées via `data-i18n`
(descriptions de tuiles d'exercices, encart « Mots à revoir », photos,
conversations, rappels) mais définies uniquement dans les langues
partielles — conséquence : **du texte français s'affichait dans les 9
autres langues**. Toutes traduites dans les 14 langues (378 chaînes).

**13 doublons latents retirés** : `back_to_home` défini deux fois dans
10 langues, `ex_denomination_t` deux fois dans les 3 darijas (en JS, la
dernière définition écrase silencieusement la première).

**Le calque « pas mal ! »** : les félicitations d'Ami en dz/ma/tn
disaient « راك ماشي/موش مليح، برافو ! » — calque du français qui
signifie littéralement « tu n'es PAS BIEN, bravo ! ». Remplacé par de
vraies félicitations (« راك قوي », « مزيان بزاف », « باهي برشا »).

**+ 4 clés manquantes** en darija (tag_autre, vitesse du jeu de
mémoire), **indentation du bloc dz d'Ami** normalisée (elle trompait
les audits).

**Verrou éthique vocal** : la reconnaissance vocale existante (« elle
peut féliciter, jamais sanctionner ») n'était protégée par aucun test —
c'est fait : interdiction testée de tout `answer_feedback(false)` dans
ses gestionnaires, message de non-reconnaissance vérifié neutre.

Nouveau `tests/audit-i18n-v196.test.js` (6 tests) qui rend ces classes
de bugs impossibles à réintroduire : parité totale des 9 langues
complètes avec le français, zéro doublon, **toute clé `data-i18n` des
pages patient/aidant définie dans les 14 langues**, calques bannis,
Ami en 14 langues, éthique vocale.

`CACHE_NAME` incrémenté (`reparole-v6-195` → `reparole-v6-196`).

## v6.197 — le dossier clinique en 5 volets (proposition du propriétaire)

La fiche patient devient un vrai dossier, organisé et navigable :

**📊 Vue d'ensemble** — chiffres clés existants + **temps de
rééducation** (⚠ nouvelle mesure : chronomètre de séance côté client,
colonne `sessions.duration_sec`, fonction `log_session` à 7 paramètres
avec `drop` explicite de l'ancienne signature pour éviter l'ambiguïté
de surcharge — l'ancien client déployé reste compatible ; l'historique
sans durée est **estimé à 45 s/item et annoncé comme tel**) +
**fréquence hebdomadaire** calculée sur l'historique.

**📈 Évolution des capacités** — barres de réussite moyenne par domaine
(tous types confondus, triées, avec le nombre de séances).

**🗓️ Événements** — calculés localement : reprises après pause >7 j,
record personnel (≥80 %), première séance de chaque type — 8 derniers,
datés.

**📔 Journal d'Ami** — bouton « Générer le journal de bord (IA) » : un
carnet de bord qualitatif séance par séance, rédigé par l'IA à la
demande, ton clinique sobre, sans émotion prétendue (règle Ami dans le
prompt). Brouillon de travail à relire, jamais un jugement clinique.

**💡 Recommandations** — bouton « Proposer des pistes (IA) » : 3-5
pistes d'action, chacune justifiée par une donnée précise, formulées
comme des SUGGESTIONS (« envisager de… ») jamais comme des prescriptions.
L'ortho reste seul juge.

Décision du propriétaire : volets 4-5 **en IA** (plus riche), à valider,
plafonnés. Ils ne se déclenchent JAMAIS à l'affichage de la fiche (aucun
coût passif), ne stockent rien, se copient d'un clic. Deux nouvelles
tâches edge (`ami_journal`, `clinical_reco`) réutilisant le pipeline
anonymisé du compte-rendu.

⚠ **SQL à rejouer** (une fois, sans danger) pour la colonne de durée
`sessions.duration_sec`. ⚠ **Redéploiement edge requis** pour activer
les volets 4-5 (régénérer `index.ts` + `deploy` — embarque aussi les
prompts v2 de v6.195). 2 clés bouton × 14 langues ; 7 clés scriptées
mortes retirées. `tests/dossier-clinique-v197.test.js` (5 tests)
verrouille la frontière factuel/IA et le caractère non-décisionnaire.

`CACHE_NAME` incrémenté (`reparole-v6-196` → `reparole-v6-197`).

## v6.198 — finitions signalées sur captures (lisibilité, thèmes, bulle d'Ami)

Trois retours du propriétaire, tous corrigés.

**Plus de bulle fantôme d'Ami** : sur les mots croisés (et la mémoire),
Ami levait la main devant une bulle VIDE. Cause : aucun texte
d'explication n'était défini pour ces exercices. Double correctif :
`render()` efface désormais le widget si Ami n'a rien à dire, ET les
explications manquantes ont été écrites — `croises`, `memoire_liste`,
`anagramme`, `verif` — dans les 10 langues complètes (les 4 partielles
héritent du français, comme le reste des consignes d'Ami). Un test
vérifie qu'Ami a une explication pour CHAQUE type d'exercice jouable.

**Les niveaux par exercice, lisibles** : le pavé gris de 4 lignes sous
le nom du patient, sans titre et illisible, est replié dans un « Voir le
niveau par exercice » — l'info reste accessible d'un clic et vit déjà en
clair dans le volet Évolution.

**Thèmes adoucis** : cartes du mode clair moins crues (#FFFFFF →
#FBFDFC) ; mode sombre relevé d'un cran (#1e2a24 → #26332d, plus
reposant). Contrastes de texte toujours largement suffisants.

1 clé × 14 langues (dépliant) + 40 explications d'Ami.
`tests/ui-polish-v198.test.js` (4 tests).

`CACHE_NAME` incrémenté (`reparole-v6-197` → `reparole-v6-198`).

## v6.199 — Ami, d'assistant à COMPAGNON + 2 chartes (design & ergonomie)

Réponse aux 3 propositions du propriétaire : les principes deviennent des
chartes gravées, Ami est enrichi par incréments, la refonte visuelle
premium reste un chantier dédié futur.

**Deux chartes** dans `docs/` : **DESIGN.md** (une fonction par couleur —
vert = marque, orange = encouragement, rouge = erreur TECHNIQUE
uniquement, violet = Ami ; typo via lecture facilitée ; boutons, cartes,
animations sobres, grille 8 px) et **ERGONOMIE.md** (les 8 règles
troubles du langage + la règle d'or d'Ami). La refonte des 40 écrans y
est cadrée comme un sprint à part.

**Ami compagnon**, par incréments — RÈGLE D'OR verrouillée : **Ami
CONSTATE, ne RESSENT jamais** (aucune émotion 1re personne, testé ×14) :
présence permanente (respiration douce + couleur violette propre),
préparation avant exercice (quoi + durée), détection de fatigue par le
COMPORTEMENT seul (aucun capteur ; 3 erreurs d'affilée ou temps de
réponse qui double → pause PROPOSÉE une fois), résumé de fin (travail +
résultat + suite). Le journal rédigé par Ami existe déjà (volet 4,
v6.197). 6 clés × 14 langues. `tests/ami-compagnon-v199.test.js` (6).

`CACHE_NAME` incrémenté (`reparole-v6-198` → `reparole-v6-199`).

## v6.200 — Ami prend un visage : le hibou

Suite directe de la vision « faire d'Ami le cœur de l'expérience »
(propriétaire). Ami cesse d'être un petit personnage abstrait pour
devenir un **hibou** — chaleureux, digne pour des adultes, avec de grands
yeux expressifs qui portent la présence sans jamais tomber dans le « je
ressens ».

Choix assumé : personnage plus incarné mais **rien de la mécanique n'a
bougé**. Le squelette SVG et TOUTES les classes verrouillées par les
tests sont conservés (`companion-body/belly/eye/mouth/leg/arm`, poses
`wave/celebrate/point/chin`, accessoires par humeur). On n'a fait
qu'ajouter les attributs de hibou (aigrettes, disques du visage, bec) et
redessiner via CSS : plumage brun (dégradé), poitrail crème, bec ambre,
ailes/serres brunes. Les **aigrettes frémissent** doucement (animation
SVG native, coupée en `reduced-motion` / lecture facilitée), en plus de
la respiration et du clignement déjà présents — vivant, jamais agité.

Couleur réservée d'Ami (`--ami`) : **maintenue au violet** conformément à
`docs/DESIGN.md` (« une fonction par couleur » — violet = Ami, orange =
encouragement). Seul le **plumage** du hibou est brun : c'est une couleur
de personnage (comme un pelage), pas un rôle d'UI, donc elle ne rentre
pas en conflit avec la grammaire des couleurs. Aucune phrase, aucune humeur, aucune clé i18n modifiée : la
RÈGLE D'OR (Ami CONSTATE, ne RESSENT jamais) reste verrouillée à
l'identique. Les 3 tests d'Ami (`companion-gestures`,
`companion-explain`, `ami-compagnon-v199`) restent verts.

`CACHE_NAME` incrémenté (`reparole-v6-199` → `reparole-v6-200`).

## v6.201 — accueil : Ami se présente, « Je commence », « clé personnelle »

Trois retouches de mots demandées par le propriétaire, pensées pour le
patient (moins d'administratif, plus de relation) :

1. **Ami se présente** dès l'accueil : le titre devient « 👋 Bonjour. » et
   le sous-titre « Je suis Ami, je vais vous accompagner. » (il se
   présente et annonce ce qu'il fait — toujours dans la règle d'or : il
   ne prétend rien ressentir). Une fois le prénom connu, il enchaîne
   « Bonjour [prénom] » comme avant.
2. **« Créer un nouveau dossier » → « Je commence »** sur le parcours
   PATIENT. Le mot « dossier » est administratif ; il reste en revanche
   dans les espaces orthophoniste et aidant, où il a du sens.
3. **« code de suivi » → « clé personnelle »** côté patient (libellé du
   champ, bannière, messages) : psychologiquement « clé » est plus
   positif et plus juste (c'est *votre* clé). Le mot « code » reste dans
   les espaces pro/aidant.

Traductions faites sur les **10 langues complètes** (fr, en, es, it, pt,
de, ar, tr, pl, ja), avec le nom d'Ami localisé (آمي en arabe, アミ en
japonais). **kab** (kabyle) et **dz/ma/tn** (darijas) NON modifiés
volontairement : le kabyle ne s'invente pas (règle du projet) et les
darijas restent « premier jet » — ces 4 langues sont à faire relire par
un locuteur natif pour aligner ces 3 phrases. Aucune règle éthique, ni
clé i18n ajoutée/supprimée : seules des valeurs changent, donc
complétude et usage i18n restent verrouillés.

`CACHE_NAME` incrémenté (`reparole-v6-200` → `reparole-v6-201`).

## v6.202 → v6.205 — documentation, passation et maquettes

Période consacrée à la **transmission** plutôt qu'au code : rédaction d'un
dossier de passation complet (architecture, cartographie fichier par
fichier, ~20 tables, i18n, exercices, Ami, IA, déploiement, chantiers
HDS/RGPD) et de maquettes de design modernes en 3 thèmes, validées avant
tout portage. Décision structurante prise ici : **réhabiller l'existant
plutôt que réécrire** — même HTML fonctionnel, mêmes identifiants, mêmes
gestionnaires, seul le style change. C'est ce qui a permis aux 941 tests
et à tous les exercices de rester intacts pendant toute la refonte.

## v6.206 — refonte des couleurs (tokens `:root` + mode sombre)

Remap complet des variables de design : palette **émeraude** (`--accent`
`#0E9E77`), **violet** pour Ami, **cyan** pour l'écran de connexion. Le
mode sombre existant devient un « Nuit » **doux** (anthracite `#212E2B`,
cartes `#2B3B36`) au lieu d'un vert sapin sombre. Tout le site change
d'un coup puisque tout passe par les tokens — aucun écran oublié, aucune
fonction touchée.

## v6.207 — polices, vitrine publique, filtre d'erreurs

Fredoka (titres) + **Atkinson Hyperlegible** (corps, conçue pour la basse
vision — pertinente pour le public aphasique) sur les 15 pages ; le test
`perf-hints` est mis à jour en conséquence. Création de `accueil.html`,
vitrine publique française autonome. `js/error-tracking.js` ignore
désormais les « Script error. » opaques (erreurs cross-origin
d'extensions de navigateur) qui noyaient les vraies erreurs dans l'admin.

## v6.208 → v6.209 — paiement configurable sans code

Table `app_settings` (clé/valeur JSON, lecture publique, écriture admin) +
carte « 💶 Paiement & tarifs » : solution d'encaissement (Stripe,
Cyberpaiement Banque Populaire, PayZen, Systempay, Stancer, PayPal,
autre), **lien de paiement** ou clés API, devise, tarifs. `PAYWALL_ENABLED`
n'est plus figé dans le code : il suit le réglage admin. Ajout de
`admin_set_plan` (SECURITY DEFINER, gatée admins) et de la carte
« ⭐ Activation Pro manuelle » — indispensable avec un lien bancaire, où
la confirmation de paiement est manuelle.

## v6.210 → v6.214 — vitrine, largeurs, connexion à onglets

Élargissement des écrans (le contenu flottait au milieu d'une page vide),
**entrée unique** sur la vitrine (trois boutons menaient au même endroit),
halo lumineux et hibou flottant sur la connexion, carte « Aujourd'hui » du
tableau de bord en grande carte dégradée. La connexion passe en **carte à
onglets** (« Se connecter » / « Première visite ») conformément à la
maquette validée ; `tests/login-layout.test.js` est **réécrit** pour
verrouiller la nouvelle disposition au lieu de l'ancienne.

## v6.215 → v6.218 — barres latérales et vitrine en page d'accueil

Admin puis ortho passent en tableau de bord **à barre latérale**
(`.admin-side`), fond décoratif masqué, pleine largeur. La vitrine devient
la **page d'accueil réelle** de reparole.fr : tant qu'il n'y a pas de
session active et qu'on n'arrive pas d'un bouton de la vitrine (drapeau
`#connexion` / `#nouveau`), c'est elle qui s'affiche.

## v6.219 — service worker : les pages passent en « réseau d'abord »

**Cause racine de déceptions répétées identifiée.** Le service worker
servait les pages en cache-first : après un déploiement, l'ancienne
version continuait de s'afficher, donnant l'impression que les
corrections n'avaient pas été livrées. Les navigations passent en
réseau-d'abord (cache en secours hors-ligne uniquement) et `accueil.html`
entre au pré-cache.

## v6.220 → v6.222 — largeurs assumées, décor, pied de page

Fin des plafonds fixes : les écrans suivent la largeur réelle
(`min(px, vw)`), la liste d'erreurs de l'admin est plafonnée et
défilante, le pied de page de la vitrine est recentré, la connexion
reçoit deux halos qui dérivent lentement, et le tableau de bord patient
passe à 96 % de la largeur.

## v6.223 → v6.224 — admin au plus près de la maquette

Émojis de la barre latérale remplacés par de **vraies icônes dessinées**,
en-tête « Vue d'ensemble » avec la date, et **4 tuiles de chiffres réels**
alimentées par `get_admin_overview()` — compteurs **agrégés uniquement**
(aucun nom, aucun code, aucune ligne individuelle). Purge de l'historique
avec durée au choix (7 / 14 / 30 jours) via `admin_purge_login_events`.
Règle respectée : en l'absence de données, les tuiles affichent « — »,
jamais un chiffre inventé.

## v6.225 → v6.226 — voix, fond « 3D », écrans d'exercice

`pickVoiceForSpeech()` prenait la **première voix venue**, souvent la plus
robotique. Elle classe désormais les voix par qualité perçue (moteurs
neuronaux d'abord, voix compactes et fantaisistes écartées) — gain
immédiat et gratuit. Fond en profondeur côté patient (deux halos en
perspective, volontairement subtils : public aphasique, rien de
clignotant, coupé en `reduced-motion`). Écrans d'exercice modernisés :
fond nettoyé, consigne en relief, réponses en cartes cliquables, barre de
progression en dégradé, anneau de focus sur les champs.

**Rappel :** le vrai gain vocal reste la génération Google Cloud TTS
(`scripts/generate-voice-audio.js`) — le dossier `audio/` est toujours vide.

## v6.227 → v6.228 — diagnostics visibles et liens réparés

Les tuiles de l'admin échouaient **en silence** : l'erreur Supabase est
maintenant affichée sous les tuiles. Côté ortho, « Sécurité du compte » et
« ReParole Pro » vivent dans l'accordéon « Réglages du compte » : cliquer
le lien ouvre désormais l'accordéon avant de défiler — sans quoi le lien
semblait ne rien faire.

## v6.229 → v6.230 — grille sans trous et CSV lisible

Les cartes étaient réparties en **colonnes typographiques**
(`column-count`) : avec des cartes hautes et insécables, une colonne
entière pouvait rester vide. Passage à `grid-template-columns:
repeat(auto-fit, minmax(340px, 1fr))`. Export CSV rendu réellement
ouvrable dans Excel français : **point-virgule**, marqueur d'encodage
(BOM), en-têtes traduits, dates JJ/MM/AAAA HH:MM, catégories d'erreurs en
clair et colonne « Réussite (%) » calculée.

## v6.231 → v6.232 — icônes ortho et bilan accessible

Barre latérale de l'ortho alignée sur celle de l'admin (icônes
dessinées). Surtout : le **bilan existait mais était introuvable** — la
barre d'onglets de la fiche patient s'arrêtait à « Recommandations » et
rien ne menait à la carte « Rapport », tout en bas. Ajout d'un onglet
« 📄 Rapport » réutilisant une clé i18n déjà traduite.

## v6.233 → v6.234 — deux interrupteurs de paiement, carte pliable

Une seule case pilotait tout. Désormais **deux cases indépendantes** :
« Patients : passer au payant » (exercices limités, IA indisponible en
gratuit) et « Orthophonistes : passer au payant » (Pro et IA réservés aux
abonnés). Au passage, `iaAllowed()` cesse d'être figé dans le code
(`IA_BETA_OPEN = true`) et suit le réglage admin ; le test correspondant
est réécrit pour verrouiller le nouveau mécanisme. Ajout du **prix annuel
patient** et carte « Paiement & tarifs » rendue **pliable**.

## v6.235 — patients inactifs (pseudonymisés) et purge

Carte « 💤 Patients inactifs » : comptes sans séance depuis 14 / 30 / 60
jours, **prénom réduit à sa première et sa dernière lettre** (« David » →
« D…d »), calcul fait **côté serveur**. La clé du patient n'est jamais
renvoyée au navigateur (sinon l'admin pourrait se connecter à sa place) :
la suppression se fait donc **en lot**, et la fonction refuse tout seuil
inférieur à 30 jours d'inactivité.

## v6.236 → v6.238 — bannière de mise à jour sur les trois espaces

Second signal de détection (`updatefound`) ajouté à `index.html`, puis
constat : la bannière **n'existait que dans l'application patient** — les
espaces ortho, admin et aidant n'enregistraient même pas le service
worker. Extraction dans un module partagé `js/sw-update.js`, branché sur
les trois espaces.

## v6.239 — espace ortho réorganisé

« Vue cabinet (IA) » et « Veille scientifique » étaient **imbriquées dans**
la carte « Dossiers suivis », déséquilibrant complètement la grille (une
colonne géante, deux minuscules, du blanc en bas). Elles deviennent des
cartes autonomes : la grille les répartit et la liste des patients
remonte en haut, où elle doit être.

## v6.240 — la case « révéler tous les jeux » ne tenait pas

Signalé par le propriétaire : la case était bien enregistrée en base mais
revenait décochée à la réouverture de la fiche. Elle s'appuyait sur
l'instantané renvoyé par `listPatients()`. La valeur est désormais
**relue directement en base** à chaque ouverture (`orthoGetGamesUnlock`),
via une méthode dédiée `syncGamesUnlock()` — `openPatient()` devant rester
compacte, un test limitant l'écart avant `show('ortho-detail')`.

## v6.241 — les 10 cartes de l'admin deviennent pliables

Toutes les cartes passent en `<details>` repliés par défaut : on arrive
sur l'en-tête, la date, les 4 chiffres clés et la liste des titres. La
barre latérale ouvre automatiquement la carte visée.

## v6.242 — des chiffres réels, mais mal nommés

Signalé par le propriétaire : « 2 d'un côté, 27 de l'autre, 9 patients
actifs — c'est vraiment réel ? ». Oui, mais deux mesures portaient des
noms trop proches. « Connexions aujourd'hui » comptait **chaque** ouverture
de session **admin/ortho** (donc surtout celles du propriétaire en train
de tester), quand le graphique « Connexions par jour » compte des
**patients distincts**. La tuile devient « Connexions admin / ortho » avec
sa précision, une note explique la différence sous les tuiles, chaque
tuile dit ce qu'elle compte, et la liste des inactifs affiche la **date de
création** et la mention « jamais utilisé » (plusieurs comptes portaient
le même libellé masqué « M…e »).

## v6.243 — `sql/app-settings.sql` rendu rejouable

PostgreSQL refuse `create or replace` quand le type de retour change
(erreur **42P13**), rencontrée en ajoutant la date de création à
`get_admin_inactive_patients`. Chaque fonction du fichier est désormais
précédée d'un `drop function if exists` : le fichier peut être rejoué
autant de fois que nécessaire.

`CACHE_NAME` incrémenté à chaque version (`reparole-v6-202` →
`reparole-v6-243`).

## v6.244 — les voix cessent d'être effacées à chaque mise à jour

**Le problème.** Les voix cloud sont en ligne depuis le 18 juillet
(10 langues, ~300 mp3 chacune). Or `sw.js` supprimait, à chaque
`activate`, tout cache dont le nom différait de `CACHE_NAME` — lequel est
incrémenté à CHAQUE version, par règle du projet. Conséquence : chaque
déploiement vidait les voix déjà téléchargées. Le patient les
re-téléchargeait intégralement, mot par mot, et réentendait la voix
synthétique du navigateur en attendant (`speak()` bascule au bout de
2,5 s). Autrement dit, la fonctionnalité la plus coûteuse du projet se
dégradait à chaque livraison.

**Le correctif.** Les enregistrements vivent désormais dans un cache
séparé, `AUDIO_CACHE = 'reparole-audio-v1'`, que `activate` épargne
explicitement. C'est sans risque d'obsolescence : un mp3 est nommé
d'après le hash de son texte, donc pour un nom de fichier donné le
contenu ne change jamais ; un texte modifié produit un nouveau nom, donc
un nouveau téléchargement. Seules les réponses 200 sont mises en cache —
un 404 n'est pas mémorisé, pour qu'un enregistrement ajouté plus tard
reste visible.

**Second correctif.** `speak()` tentait un fichier audio même pour les
langues qui n'en ont aucun (dz, ma, tn, kab) : un 404 à chaque mot
prononcé, et la parole retardée du temps de l'aller-retour réseau. La
liste `CLOUD_VOICE_LANGS` (les 10 langues de
`scripts/generate-voice-audio.js`) filtre maintenant en amont. Un test
vérifie que les deux listes ne divergent pas.

**Ménage.** `tests/fiche-clinique-v197.test.js` supprimé : brouillon
abandonné testant une conception non retenue (volets 4-5 calculés
localement), remplacé le jour même par `dossier-clinique-v197.test.js`
(volets 4-5 générés par l'IA à la demande). Il n'était branché dans
aucun `npm test` et échouait sur 3 tests sur 5 si on le lançait — un
faux signal de régression en attente. Deux tests d'hygiène ajoutés dans
`deploy-tooling.test.js` pour que ce cas ne puisse plus se reproduire :
tout fichier `tests/*.test.js` doit être lancé par `npm test`, et
`npm test` ne doit référencer aucun fichier disparu.

**Nouveau fichier** : `tests/audio-cache-v244.test.js` (9 tests).
Aucune clé i18n ajoutée ni supprimée.

`CACHE_NAME` incrémenté (`reparole-v6-243` → `reparole-v6-244`).

## v6.245 — audit complet : l'arabe, l'identité visuelle, et le verrou du journal

Audit de la totalité du dépôt (16 pages, 45 fichiers JS, 3 fichiers SQL,
3 feuilles de style). Rapport détaillé : `docs/AUDIT-v6.245.md`.

**Ce que l'audit a confirmé de sain**, et qui mérite d'être écrit : 708
clés i18n × 14 langues sans aucune manquante ; 21 tables sur 21 avec RLS,
7 tables sensibles accessibles uniquement par RPC ; **les 57 fonctions
`security definer` fixent toutes leur `search_path`** — le vecteur
d'élévation de privilèges classique sur PostgreSQL, sans exception ; zéro
fichier JS mort, zéro `TODO`, zéro `console.log`.

**L'arabe s'affichait de travers.** `dir="rtl"` était bien posé sur
`<html>` pour l'arabe et les trois darijas, mais aucune feuille de style
ne le suivait : 24 propriétés physiques (`text-align:left`,
`margin-left`, `padding-right`) figeaient la mise en page en sens
gauche-à-droite. Toutes remplacées par leurs équivalents logiques
(`start`, `margin-inline-start`, `padding-inline-end`), y compris dans
`js/app.js`, `js/assessment.js`, `js/phonation.js` et
`dashboard-ortho.html`. Comportement strictement identique dans les 10
langues gauche-à-droite.

**L'identité visuelle n'avait pas suivi la refonte v6.206.**
`manifest.json` portait encore le beige `#efead9` et le vert `#2F6B57` :
l'écran de démarrage de l'application installée affichait donc les
couleurs d'une version qui n'existe plus. Manifeste réaligné sur `--bg`
et `--accent-dark`, `theme-color` uniformisée sur les **16** pages (6
n'en avaient aucune). Un test compare désormais le manifeste aux
variables CSS : la dérive ne peut plus repasser.

**Deux détails corrigés au passage.** `icons/icon-512-maskable.png`
était déclarée dans le manifeste mais absente du cache hors-ligne.
`accueil.html` n'avait aucune balise Open Graph : un lien vers
reparole.fr envoyé par mail s'affichait en URL nue — gênant au moment
précis où des prises de contact sont en cours (FNO, France AVC).

**Le journal est complet.** Les entrées v6.202 → v6.243, jusque-là
manquantes, ont été fournies par le propriétaire. Un test verrouille
désormais la règle : la version de `CACHE_NAME` doit apparaître dans
`README.md`, comme elle doit déjà apparaître dans `docs/INDEX.md`.

**Constaté et NON corrigé, volontairement** (détail et chiffres dans le
rapport) : l'application patient pèse 2,5 Mo bruts en 42 fichiers, dont
735 Ko de `js/i18n.js` qui embarque les 14 langues quand le patient n'en
utilise qu'une. Le découpage par langue touche le fichier le plus
verrouillé du projet et mérite sa propre version. De même, quatre pages
n'ont aucune clé i18n (`accueil.html`, `mon-resume.html`,
`reset-password.html`, `report.html`) : 129 chaînes à faire traduire,
soit 1 806 valeurs — chantier de traduction avant d'être un chantier de
code.

**Nouveau fichier** : `tests/rtl-et-identite-v245.test.js` (11 tests).
Aucune clé i18n ajoutée ni supprimée.

`CACHE_NAME` incrémenté (`reparole-v6-244` → `reparole-v6-245`).

## v6.246 — stabilisation : la perte de données silencieuse

Trois défauts trouvés en creusant la résilience plutôt que l'apparence.
Tous les trois étaient **silencieux**, ce qui est le pire cas possible
ici : une personne aphasique ne signalera pas « ma séance n'a pas été
enregistrée ».

**1. Le stockage local était lu sans filet — 31 fois.** En mode local
(sans compte cloud), `localStorage` **est** la base de données du
patient. Or `JSON.parse(localStorage.getItem(...))` était appelé 31 fois
dans `js/storage.js` sans `try`. Une seule valeur corrompue — écriture
interrompue par un quota plein, plantage du navigateur, extension de
nettoyage — levait une exception au chargement : écran cassé, données
toujours présentes mais illisibles, aucun moyen de s'en sortir.

Les 31 lectures passent désormais par `_lsGet(clé, défaut)`, qui ne lève
jamais. Choix important : **une valeur illisible n'est pas écrasée**.
Elle est mise de côté une fois sous `<clé>:corrompu`, donc récupérable,
et la lecture repart sur la valeur par défaut. Perdre l'affichage se
répare ; perdre les données, non. Le helper refuse aussi du JSON valide
mais de mauvaise forme (une chaîne là où une liste est attendue), qui
aurait cassé au premier `.push()`.

**2. La file d'attente hors-ligne pouvait perdre sans le dire.** Elle
n'avait aucun plafond, aucune purge, aucune limite de réessais — et
`_enqueue()` **ignorait la valeur de retour** de l'écriture : quota
atteint, élément jamais écrit, personne au courant. Un élément rejeté
pour une raison permanente était rejoué à chaque reconnexion,
indéfiniment, bloquant la file derrière lui.

Désormais : plafond à 300 éléments (ce sont les **plus anciens** qui
cèdent la place, jamais l'exercice qui vient d'être terminé), abandon
après 5 tentatives, purge au-delà de 30 jours, et surtout **tout ce qui
est abandonné est compté**. Le compte remonte dans la table des erreurs
client, déjà affichée dans l'espace administrateur : aucune interface
nouvelle, aucune clé de traduction, mais une perte silencieuse devient
une perte constatable.

**3. Et pendant ce temps, l'interface promettait le contraire.** Le
message affiché au patient disait littéralement « *rien n'est perdu, ça
se synchronisera automatiquement* » — une promesse que le code ne pouvait
pas tenir. C'est exactement ce que la règle 5 interdit. Le message dit
maintenant ce qui est en attente, et, le cas échéant, ce qui n'a pas pu
partir — en précisant que les exercices, eux, ont bien été faits : c'est
leur trace qui manque.

**Au passage** : `reset-password.html` était la seule page interactive du
parcours patient à ne pas charger `js/error-tracking.js`. Si elle
cassait, personne ne l'apprenait. Corrigé, et un test vérifie les cinq
pages du parcours.

**Garde-fou de poids** : un test échoue si l'application patient dépasse
2 765 Ko (elle en fait 2 540 aujourd'hui). Il ne fait pas maigrir le
site, il empêche la reprise de poids pendant qu'on travaille à
l'alléger — et il devra être abaissé quand `i18n.js` sera découpé.

**Nouveau fichier** : `tests/stabilisation-v246.test.js` (17 tests).
Aucune clé i18n ajoutée ni supprimée.

`CACHE_NAME` incrémenté (`reparole-v6-245` → `reparole-v6-246`).

## v6.247 — le dictionnaire quitte i18n.js : l'app patient perd 667 Ko

**Le chantier différé depuis l'audit.** `js/i18n.js` embarquait les 14
langues — 735 Ko téléchargés, analysés et gardés en mémoire à chaque
ouverture, pour n'en utiliser qu'une. Sur le téléphone d'entrée de gamme
d'une personne âgée après un AVC, c'était le poste de dépense le plus
lourd de l'application.

**Le montage.** Chaque langue vit dans `js/i18n/<code>.js` et
s'enregistre par `I18N.register()`. `i18n.js` (14 Ko) ne garde que la
mécanique : `t()`, `rand()`, `apply()`, et le chargeur `I18N.charger()`
(dédoublonné, avec `I18N.appliquer()` qui charge PUIS bascule — jamais
d'écran à moitié traduit). Le **français reste chargé en dur** dans les
trois pages traduites : c'est le repli inconditionnel de `t()`, l'app
doit pouvoir l'afficher même si tout le reste échoue. La langue mémorisée
est injectée avant le rendu (aucun « flash » de français pour un
arabophone), et un changement de langue qui échoue au réseau bascule
quand même — un écran en français vaut mieux qu'un écran figé, surtout
pour quelqu'un qui cherche déjà ses mots.

**Hors-ligne.** `sw.js` ne pré-cache QUE le français ; les autres
langues se mettent en cache au premier usage (branche « cache d'abord »),
donc disponibles hors-ligne dès la deuxième ouverture. Les pré-charger
toutes aurait annulé le bénéfice du découpage — un test l'interdit.

**Vérification d'équivalence.** L'ancien et le nouveau dictionnaire ont
été comparés valeur par valeur, fonctions comprises : 14 langues × 708
clés, **aucune clé perdue, ajoutée ou modifiée**. Les tests de complétude
existants continuent de vérifier l'ensemble (ils chargent maintenant le
dossier `js/i18n/` en plus du cœur).

**Le chiffre.** L'app patient passe de **2 540 Ko à 1 873 Ko** bruts
(586 → 378 Ko compressés), français inclus ; une langue non française
ajoute ~50-60 Ko, une seule fois. Le budget de poids posé en v6.246 est
**abaissé** de 2 765 à 2 048 Ko, conformément à sa propre consigne.

**Honnêteté sur l'origine et les limites.** Le gros du découpage
(extraction des 14 fichiers, adaptation de ~77 fichiers de tests, pages,
`prefs.js`, `sw.js`) a été trouvé déjà en place dans la copie de travail,
sans journal ni version — vraisemblablement une autre session. Il a été
vérifié (équivalence ci-dessus, suite complète), corrigé
(`documentation.test.js` lisait les blocs supprimés), verrouillé
(`tests/decoupage-langues-v247.test.js`, 11 tests) et achevé (version,
journal, budget). Reste ce qu'aucun test ne remplace : **ouvrir l'app
dans un vrai navigateur**, changer de langue, couper le réseau, et
vérifier que tout suit — le chargement dynamique est précisément le genre
de mécanique que jsdom ne simule pas fidèlement.

**Nouveau fichier** : `tests/decoupage-langues-v247.test.js` (11 tests).
Aucune clé i18n ajoutée ni supprimée (prouvé, pas seulement affirmé).

`CACHE_NAME` incrémenté (`reparole-v6-246` → `reparole-v6-247`).

## v6.248 — retours du premier vrai test navigateur

Le propriétaire a testé reparole.fr en conditions réelles (français,
darija algérienne, italien) — le test qu'aucune suite automatique ne
remplace — et rapporté quatre choses : voix robotiques persistantes,
libellé « Quel est ce mot ? » inadapté, du français dans la page darija,
et des emoji qui font enfantins. L'enquête a trouvé les causes.

**Les voix robotiques : un bug de superposition dans `speak()`.**
Vérifié d'abord que les fichiers existent : 1 205 mp3 attendus (fr, it,
ar, ja) contrôlés un à un en ligne, **zéro manquant**, hachages
identiques des deux côtés. Le vrai coupable : si le mp3 mettait plus de
2,5 s à arriver, le filet lançait la voix du navigateur — puis
`canplaythrough` arrivait et lançait la voix cloud PAR-DESSUS, sans
vérifier que le repli avait déjà parlé. Deux voix superposées, la
robotique en premier. Cas aggravé en test avec Ctrl+Maj+R, qui contourne
le service worker : chaque écoute redevient une « première » écoute
réseau. Corrigé : le repli qui a parlé garde la parole, la voix cloud
arrivée en retard se tait (le fichier est en cache : la PROCHAINE écoute
sera la bonne voix, immédiatement), et le minuteur est désarmé quand la
voix cloud prend la main à temps.

**Le manifeste des voix était empoisonné depuis le découpage v6.247.**
`scripts/extract-voice-content.js` charge l'app comme une page réelle —
donc, depuis le découpage, uniquement le français : pour les 9 autres
langues, `I18N.t()` retombait sur le français et le manifeste capturait
des chaînes mi-françaises (« Citez le plus possible de… palabras que
empiezan por B »). Réparé (le script charge les 14 langues), manifeste
régénéré et vérifié : plus aucune chaîne française hors du bloc `fr`.

**« Quel est ce mot ? » → « Qu'est-ce que c'est ? »** Demandé en
« Quel est cet objet ? », mais la banque contient des chats, des
girafes, des papillons : « objet » aurait été faux une fois sur trois.
Retenu : la formule classique de dénomination, valable pour tout.
Changée dans les 10 langues à voix cloud ; **kab, dz, ma, tn
intouchées** (règle 4 : on n'invente jamais une traduction — leurs
formules actuelles restent valables). ⚠️ Nouveau texte = nouveau
hachage : relancer `node scripts/generate-voice-audio.js` (il saute
l'existant, ~9 fichiers à créer) puis téléverser dans `audio/`.

**Les emoji deviennent un repli, plus le principal.** Nouveau module
`js/exo-images.js` : chaque support d'exercice cherche d'abord une vraie
photo `img/<codepoints>.webp` (puis `.jpg`), et affiche l'emoji dans une
carte sobre à défaut. L'emoji est LA clé partagée des 14 langues (🐱
sert CHAT, CAT, GATO…) : 116 photos couvrent tout. Converti partout, y
compris le jeu de mémoire de la capture d'écran (séquence, boutons,
rappel) et le support d'association. Un fichier absent n'est demandé
qu'une fois par session ; les photos trouvées vont dans le cache
permanent du service worker (leçon v6.244). `img/` suit les règles
d'`audio/` : jamais supprimé au déploiement, exclu des zips —
`HEBERGEMENT.md` et le script de build mis à jour. Liste des fichiers
attendus : `docs/IMAGES.md`. **Aucune photo n'existe encore** : tant que
`img/` est vide, l'affichage reste l'emoji — zéro régression.

**Le français dans la page darija n'est PAS corrigé — c'est un choix
documenté (v6.9)** : la fluence est un exercice vocal, la darija n'a pas
de reconnaissance vocale dans les navigateurs, donc l'app bascule sur
l'exercice français en l'annonçant (le bandeau rouge, en darija, dit
exactement ça). Le changer — masquer ces exercices en dz/ma/tn/kab, ou
les marquer dans le menu — est une décision de conception qui appartient
au propriétaire, pas un correctif à glisser ici.

**Origine, pour l'honnêteté du journal** : comme en v6.247, une partie du
travail (module `exo-images.js`, `docs/IMAGES.md`, conversions de
`app.js`, branche `img/` du service worker, `HEBERGEMENT.md`) a été
trouvée déjà en place dans la copie de travail, sans version ni journal.
Elle a été vérifiée, complétée (jeu de mémoire, prompt d'association,
exclusion du zip de build) et verrouillée ici.

**Nouveau fichier** : `tests/retours-navigateur-v248.test.js` (16 tests).
Clés i18n : aucune ajoutée ni supprimée ; 1 valeur modifiée
(`denom_prompt`) dans 10 langues.

`CACHE_NAME` incrémenté (`reparole-v6-247` → `reparole-v6-248`).

### Volet photos en détail (même version — texte trouvé en place, vérifié)

Demandé par le propriétaire, capture à l'appui : une tortue emoji
flottant dans une page blanche, pour un public d'adultes après un AVC,
infantilise — le contraire du respect que le reste de l'application leur
témoigne.

**Le montage.** `js/exo-images.js` résout chaque support visuel en
cascade : `img/<codepoints>.webp` → `img/<codepoints>.jpg` → l'emoji,
présenté non plus flottant mais dans une **carte sobre**
(`.prompt-media`) — cadre discret, sans animation, qui rappelle une
carte d'imagier clinique. Les cinq sites de rendu de mots (dénomination
écrite et orale, mots croisés, anagramme, écriture) plus le bilan
passent par ce chemin ; les emoji décoratifs de l'interface (🌱, 🌟, 🔒)
ne sont pas concernés.

**L'observation qui rend le chantier faisable.** L'emoji est LA CLÉ
partagée des 14 langues : 🐱 illustre CHAT, CAT, GATO, KOT… Une photo de
chat sert donc toutes les langues. **116 images couvrent la totalité des
banques**, pas 116 × 14. Le nom de fichier est la suite de codepoints
(🐱 → `1f431.webp`, ☀️ → `2600.webp` — sélecteur de variante retiré,
comme Twemoji), stable et sans problème d'encodage.

**Leçons des versions précédentes appliquées d'emblée** : un fichier
absent n'est demandé qu'une fois par session — jamais de rafale de 404
(v6.244) ; les photos trouvées vont dans le **cache permanent** du
service worker, à côté des voix — elles survivent aux mises à jour ;
l'image reste cachée tant qu'elle n'a pas chargé — jamais d'icône
d'image cassée, l'emoji tient la place.

**Le dossier `img/` suit les règles d'`audio/`** : jamais supprimé au
déploiement (HEBERGEMENT.md mis à jour : « tout supprimer sauf `audio/`
ET `img/` »), exclu des zips, rempli progressivement chez l'hébergeur —
une photo ajoutée est en ligne immédiatement, sans nouvelle version.

**`docs/IMAGES.md`** explique le fonctionnement, les règles (fond
neutre, ~600 px, pas de visage identifiable sans consentement, licences
compatibles avec un usage commercial), et liste les **116 fichiers
attendus** — tableau généré depuis les banques réelles par
`scripts/liste-images.js`, avec un test qui refuse la désynchronisation.

**Ce que cette version ne fait pas** : elle ne fournit AUCUNE photo.
Impossible d'en télécharger depuis l'environnement de développement, et
en dessiner de médiocres n'aurait pas mieux valu que les emoji. Le
pipeline est prêt ; les photos sont un travail de sourcing — les photos
personnelles prises par l'aidant restant le meilleur choix clinique.

**Nouveau fichiers** : `js/exo-images.js`, `scripts/liste-images.js`,
`docs/IMAGES.md`, `tests/images-exercices-v248.test.js` (12 tests).
Aucune clé i18n ajoutée ni supprimée.


## v6.249 — vie privée : plus un octet vers Google, et le dossier RGPD

Sur les trois bloquants de mise en production (HDS, RGPD, sauvegardes),
celui-ci fait avancer le deuxième — et corrige au passage un vrai
problème de conformité déjà en production.

**Les polices ne passent plus par Google.** Les 16 pages chargeaient
Fredoka, Atkinson Hyperlegible et Noto Sans depuis
`fonts.googleapis.com` : l'adresse IP de chaque patient partait chez
Google à chaque ouverture, sans consentement — le transfert exact pour
lequel des sites ont été condamnés au titre du RGPD, et d'autant moins
acceptable pour une application de santé. Les 18 fichiers woff2 vivent
désormais dans `fonts/` (376 Ko, sous-ensembles latin + latin-ext —
indispensables au polonais ł/ż et au turc ğ/ş/ı), déclarés par
`css/fonts.css` générée depuis @fontsource avec les mêmes
`unicode-range`. L'arabe et le japonais ne changent pas : ces familles
ne les ont jamais couverts, ils utilisent les polices système, comme
avant. Bonus réel : les polices sont pré-cachées et marchent
**hors-ligne** (avant, non), et le rendu ne dépend plus d'un serveur
tiers.

**`docs/RGPD.md`** : registre des traitements fondé sur le code réel —
les 20 tables du schéma couvertes une à une (un test échoue si une
table du schéma manque au registre), durées de conservation
CONSTATÉES (purge des voix à 30 jours, purge admin des connexions,
purge des inactifs), droits réels (export JSON `exportMyData()`,
suppression `delete_patient_account` — tous deux déjà testés),
sous-traitants et localisations, et surtout **les lacunes assumées** :
HDS non commencé, GitHub Pages sert depuis un CDN américain, pas de
DPO, `confidentialite.html` à mettre à jour. Document de travail à
faire valider par un professionnel — il le dit lui-même.

**Filet complété.** Trois lectures `JSON.parse(raw)` indirectes avaient
échappé au durcissement v6.246 (le test ne cherchait que
`JSON.parse(localStorage`). Converties vers `_lsGet()`, et le test
élargi pour attraper aussi ce motif — en excluant le corps du helper,
seul `JSON.parse` légitime du fichier.

**Nouveau fichiers** : `css/fonts.css`, `fonts/` (18 woff2),
`docs/RGPD.md`, `tests/vie-privee-v249.test.js` (9 tests).
Aucune clé i18n ajoutée ni supprimée.

`CACHE_NAME` incrémenté (`reparole-v6-248` → `reparole-v6-249`).

## v6.250 — RGPD « nickel de partout » : la chaîne complète, vérifiée

Demandé par le propriétaire. Plutôt que de relire les documents, l'audit
a suivi la chaîne dans le code — et trouvé que deux promesses faites au
patient étaient fausses.

**Le droit à l'effacement était incomplet.** `delete_patient_account`
supprimait la fiche patient (et ses 10 tables en cascade) et les
médias — mais laissait DEUX traces : le code patient **en clair** dans
`login_events`, sans limite de durée, et son hachage dans
`patient_connections`. « Supprimer mon compte — suppression définitive
de toutes vos données » qui ne supprime pas tout est un mensonge fait au
patient. La fonction efface désormais les deux, en utilisant strictement
le même hachage que l'insertion (un test vérifie que les deux ne peuvent
pas diverger), et un test parcourt CHAQUE table du schéma : toute table
portant un code patient doit être couverte par cascade ou suppression
explicite — une future table oubliée fera échouer la suite.

**`client_errors` n'avait aucune purge.** Messages, pages et
`user_agent` (donnée personnelle au sens du RGPD) s'accumulaient sans
limite. Nouvelle fonction `admin_purge_client_errors` (7/14/30 jours,
gatée admin, rejouable), bouton dans la carte « erreurs techniques » de
l'espace admin, avec le même double clic de confirmation que les autres
purges — jamais de `confirm()` natif.

**L'export « complet » ne l'était pas.** Les mots favoris et les
métadonnées des enregistrements de voix manquaient. Ajoutés au bundle
JSON (les fichiers audio eux-mêmes sont purgés à 30 jours — leurs
métadonnées, elles, sont exportées).

**Les pages légales décrivaient l'état d'hier.**
`confidentialite.html` affichait encore « [Durée à définir] » alors que
des purges réelles existent : la section liste maintenant les durées
CONSTATÉES (voix 30 jours, purges admin, inactifs, mode navigateur sans
aucune collecte), et la portée exacte des deux boutons de droits.
`politique-cookies.html` décrivait le chargement Google Fonts et son
transfert d'IP — supprimés en v6.249 : la section annonce désormais
l'auto-hébergement. `gestion-cookies.html`, déjà honnête, n'a pas bougé.
Restent deux champs que seul le responsable peut remplir : [email de
contact] et l'identité de l'éditeur dans les mentions légales.

**`docs/RGPD.md`** mis à jour en conséquence : deux lacunes barrées
(faites), les bloquants réels (HDS, sauvegardes, validation
professionnelle) toujours en tête — un test refuse qu'ils disparaissent
du document.

⚠️ **À rejouer sur Supabase** : `sql/schema.sql` (nouvelle
`delete_patient_account`) et `sql/app-settings.sql` (nouvelle purge) —
les deux fichiers restent rejouables tels quels.

**Nouveau fichier** : `tests/rgpd-nickel-v250.test.js` (11 tests).
Aucune clé i18n ajoutée ni supprimée (pages légales : françaises par
décision de projet ; espace admin : français en dur assumé).

`CACHE_NAME` incrémenté (`reparole-v6-249` → `reparole-v6-250`).
