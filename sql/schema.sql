...
-- v6.169 : CORRECTIF. En v6.168, log_session est passée de 5 à 6
-- paramètres (ajout de p_lang). "create or replace" NE remplace PAS
-- l'ancienne fonction dans ce cas : une signature différente crée une
-- SECONDE surcharge, laissant orpheline la version à 5 paramètres. Sur
-- une base déjà en service, les deux coexistent alors — et un appel à
-- 5 arguments (ex. une séance mise en file d'attente par un navigateur
-- antérieur à v6.168) devient ambigu ("function is not unique"). On
-- supprime explicitement l'ancienne signature avant de recréer.
drop function if exists log_session(text, text, int, int, int);
-- v6.197 : durée réelle des séances (dossier clinique en 5 volets).
-- L'historique antérieur n'a pas de durée : l'interface l'estime et le
-- dit. Le DROP explicite évite l'ambiguïté de surcharge (l'ancienne
-- signature à 6 paramètres + la nouvelle à 7 rendraient l'appel nommé
-- « function is not unique ») — l'ancien client déployé reste
-- compatible : p_duration_sec a une valeur par défaut.
alter table sessions add column if not exists duration_sec int;
drop function if exists log_session(text, text, int, int, int, text);
create or replace function log_session(p_code text, p_type text, p_score int, p_total int, p_level int, p_lang text default null, p_duration_sec int default null)
returns void language sql security definer set search_path = public as $$
  insert into sessions (code, type, score, total, level, lang, duration_sec) values (p_code, p_type, p_score, p_total, p_level, p_lang, p_duration_sec);
$$;

-- v6.169 : CORRECTIF (même piège que get_patient documenté plus haut).
-- get_history renvoie "setof sessions" — ce qui FIGE le type ligne de
-- "sessions" au moment de la création. En v6.168, une colonne a été
-- ajoutée à sessions (lang). Rejouer ensuite "create or replace
-- function get_history(...)" sur une base existante échoue alors avec
-- "ERROR: 42P13: cannot change return type of existing function /
-- HINT: Use DROP FUNCTION get_history(text) first." — et comme
-- get_caregiver_data() (l'espace aidant de v6.168) est déclarée APRÈS
-- get_history dans ce fichier, tout ce qui suit ne s'appliquait pas.
-- Le drop explicite règle le problème, exactement comme pour get_patient.
drop function if exists get_history(text);
...
-- v6.169 : activité récente (14 jours calendaires) de TOUS les patients
-- rattachés à l'orthophoniste connecté, en un seul appel — pour la
-- mini-frise d'activité par ligne dans la liste (js/dashboard-ortho.js).
-- Sécurité : l'identité est dérivée de auth.uid() DANS la fonction,
-- jamais d'un paramètre de confiance. Un orthophoniste ne peut donc
-- jamais lire l'activité des patients d'un∙e autre, même en falsifiant
-- un appel. Renvoie { "<code patient>": ["YYYY-MM-DD", ...], ... } —
-- même format de dates que v_active_days de get_caregiver_data (v6.168),
-- pour que le rendu client soit strictement identique à l'espace aidant.
create or replace function get_ortho_activity()
returns jsonb language sql security definer set search_path = public as $$
  select coalesce(jsonb_object_agg(code, days), '{}'::jsonb)
  from (
    select s.code, jsonb_agg(distinct date(s.at)) as days
    from sessions s
    join patient_assignments pa on pa.patient_code = s.code
    where pa.ortho_code = auth.uid()::text
      and s.at > now() - interval '14 days'
    group by s.code
  ) t;
$$;
...
-- v6.173 : les mots peuvent maintenant venir de deux sources — l'aidant
-- (comportement historique) ou l'orthophoniste ("mots ciblés" depuis la
-- fiche patient). Même table, même intégration aux exercices côté
-- patient : seule l'origine est tracée, pour l'afficher dans les deux
-- espaces. ⚠ Cette colonne modifie le rowtype de la table : les
-- fonctions `returns setof caregiver_words` ci-dessous DOIVENT être
-- précédées d'un `drop function` (voir tests/sql-schema-sanity.test.js).
alter table caregiver_words add column if not exists source text not null default 'caregiver'
  check (source in ('caregiver','ortho'));
...
-- dans js/app.js.
-- v6.173 : drop obligatoire — `returns setof caregiver_words` fige le
-- rowtype, et la colonne `source` ajoutée plus haut le modifie (même
-- classe de bug que get_history en v6.169).
drop function if exists get_caregiver_words(text);
...
-- v6.173 : même raison que ci-dessus pour le drop.
drop function if exists get_caregiver_added_words(text);
...
--  ---------------------------------------------------------------------
--  L'aidant pouvait proposer des mots, pas l'orthophoniste — un manque
--  presque incohérent : c'est pourtant l'ortho qui sait quels mots
--  travailler en priorité. Même table, même pipeline d'intégration aux
--  exercices ; seule différence : l'identité vient de auth.uid() (compte
--  Supabase Auth de l'ortho), jamais d'un paramètre de confiance, et
--  l'ortho doit être rattaché·e au patient (patient_assignments).
-- =====================================================================

create or replace function add_ortho_word(p_patient_code text, p_word text, p_emoji text default null)
returns void language plpgsql security definer set search_path = public as $$
begin
  if length(trim(p_word)) = 0 then
    raise exception 'mot vide';
  ...
  if not exists (
    select 1 from patient_assignments pa
    where pa.patient_code = p_patient_code and pa.ortho_code = auth.uid()::text
  ) then
    raise exception 'patient non rattaché à ce compte';
  end if;
  insert into caregiver_words (code, word, emoji, source)
    values (p_patient_code, trim(p_word), p_emoji, 'ortho');
end;
$$;

-- L'orthophoniste peut retirer n'importe quel mot de SES patients
-- (y compris un mot proposé par l'aidant : autorité clinicienne sur le
-- contenu des exercices — principe de conception du projet). Le
-- rattachement est re-vérifié via auth.uid() sur la ligne visée.
create or replace function delete_target_word(p_id bigint)
returns void language plpgsql security definer set search_path = public as $$
begin
  delete from caregiver_words w
  where w.id = p_id
    and exists (
      select 1 from patient_assignments pa
      where pa.patient_code = w.code and pa.ortho_code = auth.uid()::text
    );
end;
$$;
...
--  l'app.
-- =====================================================================
-- v6.250 — DROIT À L'EFFACEMENT COMPLET (audit RGPD)
-- L'ancienne version supprimait patients (les 10 tables en cascade
-- suivaient) et les médias — mais laissait DEUX traces :
--   1. login_events.code : le code patient EN CLAIR, sans limite ;
--   2. patient_connections.code_hash : le hachage du code.
-- « Supprimer mon compte » qui ne supprime pas tout est un mensonge
-- fait au patient. Les deux traces partent désormais avec le compte.
drop function if exists delete_patient_account(text);
...
begin
  ...
  delete from login_events where code = p_code;
  delete from patient_connections
    where code_hash = encode(digest(p_code, 'sha256'), 'hex');
  delete from patients where code = p_code;  -- cascade : sessions, journal,
  -- erreurs, favoris, mots aidant, notes, bilans, médias, assignations,
  -- enregistrements de voix, exercices personnalisés.
end;
$$;
...
-- v6.169 : même précaution que get_history / get_patient. "returns setof
-- notes" fige le type ligne de "notes" ; comme cette table reçoit un
-- ALTER ... ADD COLUMN (visible_to_patient, juste au-dessus), tout futur
-- ajout de colonne rendrait cette recréation impossible sur une base
-- existante sans un DROP explicite. On l'ajoute dès maintenant.
drop function if exists get_patient_visible_notes(text);
...
-- =====================================================================
--  v6.174 — BOUCLE VOCALE ASYNCHRONE (patient → orthophoniste)
--  ---------------------------------------------------------------------
--  Le différenciant : tous les outils du marché remontent des SCORES ;
--  l'orthophonie, c'est de l'AUDIO. Ici, le patient enregistre sa
--  production réelle sur ses mots ciblés (v6.173) ; l'orthophoniste
--  l'écoute entre deux séances et tranche (« acquis » / « à
--  retravailler ») — le jugement humain fait foi, pas un score
--  automatique. Prolongement du cabinet entre les séances.
--
--  VIE PRIVÉE (données de santé sensibles) :
--   - CONSENTEMENT EXPLICITE : patients.voice_consent, faux par défaut.
--     Aucun enregistrement accepté sans lui (vérifié CÔTÉ SQL, pas
--     seulement dans l'interface).
--   - RÉVOCABLE : retirer le consentement SUPPRIME immédiatement tous
--     les enregistrements (métadonnées + fichiers du bucket).
--   - RÉTENTION 30 JOURS : les lectures filtrent à 30 jours, et
--     purge_old_voice_recordings() efface métadonnées + fichiers
--     expirés (à appeler via pg_cron si disponible, sinon elle est
--     appelée opportunément côté client à la connexion ortho).
--   - Les fichiers vivent dans le bucket patient-media sous
--     <code>/voice/… : la purge de suppression de compte existante
--     (delete ... name like p_code || '/%') les couvre déjà.
--   - ⚠ patients reçoit une colonne : get_patient (setof patients) a
--     déjà son drop préalable (v6.44) — même mécanique que `levels`.
-- =====================================================================

alter table patients add column if not exists voice_consent boolean not null default false;
...
create table if not exists voice_recordings (
  ...
  word        text not null,
  url         text not null,
  verdict     text not null default 'pending' check (verdict in ('pending','acquired','retry')),
  created_at  timestamptz default now()
);
create index if not exists voice_recordings_code_idx on voice_recordings(code, created_at);
alter table voice_recordings enable row level security;
-- (aucune policy directe : accès uniquement via les fonctions ci-dessous)

-- Consentement : activé/révoqué par le patient (identifié par son code,
-- comme tout l'accès patient). La révocation efface tout, tout de suite.
create or replace function set_voice_consent(p_code text, p_consent boolean)
returns void language plpgsql security definer set search_path = public as $$
begin
  update patients set voice_consent = p_consent where code = p_code;
  if not p_consent then
    delete from voice_recordings where code = p_code;
    delete from storage.objects where bucket_id = 'patient-media' and name like p_code || '/voice/%';
  end if;
end;
$$;

-- Dépôt d'un enregistrement : REFUSÉ si le consentement n'est pas actif
-- (défense côté base, l'interface seule ne suffit pas).
create or replace function add_voice_recording(p_code text, p_word text, p_url text)
returns void language plpgsql security definer set search_path = public as $$
begin
  if not exists (select 1 from patients where code = p_code and voice_consent) then
    raise exception 'consentement vocal non actif';
  end if;
  if length(trim(p_word)) = 0 then
    raise exception 'mot vide';
  end if;
  insert into voice_recordings (code, word, url) values (p_code, trim(p_word), p_url);
end;
$$;

-- Lecture côté PATIENT (ses propres enregistrements + le verdict de
-- l'ortho — c'est son retour pédagogique). Rétention : 30 jours.
create or replace function get_voice_recordings(p_code text)
returns setof voice_recordings language sql security definer set search_path = public as $$
  select * from voice_recordings
  where code = p_code and created_at > now() - interval '30 days'
  order by created_at desc;
$$;

-- Lecture côté ORTHOPHONISTE : identité par auth.uid() DANS la fonction,
-- rattachement exigé — un ortho n'écoute que SES patients. 30 jours.
create or replace function get_patient_voice_recordings(p_patient_code text)
returns setof voice_recordings language sql security definer set search_path = public as $$
  select v.* from voice_recordings v
  where v.code = p_patient_code
    and v.created_at > now() - interval '30 days'
    and exists (
      select 1 from patient_assignments pa
      where pa.patient_code = v.code and pa.ortho_code = auth.uid()::text
    )
  order by v.created_at desc;
$$;

-- Verdict de l'orthophoniste — le jugement humain qui fait foi.
create or replace function set_voice_verdict(p_id bigint, p_verdict text)
returns void language plpgsql security definer set search_path = public as $$
begin
  if p_verdict not in ('acquired','retry') then
    raise exception 'verdict invalide';
  end if;
  update voice_recordings v set verdict = p_verdict
  where v.id = p_id
    and exists (
      select 1 from patient_assignments pa
      where pa.patient_code = v.code and pa.ortho_code = auth.uid()::text
...
-- Purge de rétention (métadonnées + fichiers > 30 jours). Idéalement
-- planifiée via pg_cron ; sinon appelée opportunément côté client.
create or replace function purge_old_voice_recordings()
returns void language plpgsql security definer set search_path = public as $$
begin
  delete from storage.objects o
  using voice_recordings v
  where v.created_at <= now() - interval '30 days'
    and o.bucket_id = 'patient-media'
    and o.name = v.code || '/voice/' || split_part(v.url, '/voice/', 2);
  delete from voice_recordings where created_at <= now() - interval '30 days';
end;
$$;

-- =====================================================================
--  v6.175 — ASSISTANT DE RÉDACTION IA (brouillons de comptes-rendus)
--  ---------------------------------------------------------------------
--  Table de comptage des appels IA, pour le plafond journalier appliqué
--  par l'edge function generate-report-draft (voir js/ia-edge-function.md).
--  L'edge function écrit ici avec la clé service role ; aucun accès
--  client direct (RLS sans policy). La clé du fournisseur IA vit dans
--  les secrets Supabase, jamais côté client ; les données partent
--  ANONYMISÉES (ni nom ni code patient — voir le prompt système).
-- =====================================================================
create table if not exists ia_usage (
  id          bigint generated always as identity primary key,
  ortho_code  text not null,
  at          timestamptz not null default now()
);
create index if not exists ia_usage_ortho_at_idx on ia_usage(ortho_code, at);
alter table ia_usage enable row level security;

-- =====================================================================
--  v6.183 — ATELIER D'EXERCICES IA (l'IA crée, l'ortho valide, le
--  patient joue)
--  ---------------------------------------------------------------------
--  L'IA génère un exercice sur mesure (QCM au schéma natif du moteur :
--  text/choices/answer) d'après le profil anonymisé du patient. RIEN
--  n'atteint le patient sans le clic « Proposer au patient » de
--  l'orthophoniste : l'enregistrement ci-dessous EST l'acte de
--  validation. Le patient voit alors une carte « Exercices de votre
--  orthophoniste » et joue dans le moteur standard.
-- =====================================================================
...
create index if not exists custom_exercises_code_idx on custom_exercises(code, created_at);
alter table custom_exercises enable row level security;
-- (aucune policy : accès uniquement via les fonctions ci-dessous)

-- L'enregistrement = la VALIDATION par l'ortho (auth.uid + rattachement).
create or replace function add_custom_exercise(p_patient_code text, p_title text, p_payload jsonb)
returns void language plpgsql security definer set search_path = public as $$
begin
  ...
  insert into custom_exercises (code, title, payload) values (p_patient_code, trim(p_title), p_payload);
end;
$$;

create or replace function delete_custom_exercise(p_id bigint)
returns void language plpgsql security definer set search_path = public as $$
begin
  delete from custom_exercises e
  where e.id = p_id
    and exists (
      select 1 from patient_assignments pa
      where pa.patient_code = e.code and pa.ortho_code = auth.uid()::text
    );
end;
$$;

-- Lecture par code (patient ET fiche ortho) — 20 plus récents.
create or replace function get_custom_exercises(p_code text)
returns setof custom_exercises language sql security definer set search_path = public as $$
  select * from custom_exercises where code = p_code order by created_at desc limit 20;
$$;

-- =====================================================================
--  v6.186 — MÉNAGE DE L'HISTORIQUE DES CONNEXIONS
--  Bouton admin « faire le ménage » : supprime les connexions de plus de
--  30 jours. Gaté par l'appartenance à la table admins (auth.uid() lu
--  côté serveur, comme le reste de l'espace admin).
-- =====================================================================
create or replace function purge_login_events()
returns void language plpgsql security definer set search_path = public as $$
begin
  if not exists (select 1 from admins a where a.code = auth.uid()::text) then
...
-- =====================================================================
--  v6.187 — QUÊTE-DÉCOUVERTE : INTERRUPTEUR DE L'ORTHOPHONISTE
--  ---------------------------------------------------------------------
--  Les jeux se révèlent progressivement (quête-célébration côté client,
--  seuils minuscules dérivés de patients.sessions — rien à stocker).
--  Soupape d'autorité clinicienne : l'ortho peut tout révéler pour un
--  patient donné. Même pattern d'ajout de colonne que voice_consent
--  (v6.174) : get_patient fait `select *`, la colonne suit le rowtype.
-- =====================================================================
alter table patients add column if not exists games_all_unlocked boolean not null default false;

create or replace function set_games_unlock(p_code text, p_unlock boolean)
returns void language plpgsql security definer set search_path = public as $$
begin
  if not exists (
    select 1 from patient_assignments pa
    where pa.patient_code = p_code and pa.ortho_code = auth.uid()::text
  ) then
    raise exception 'patient non rattaché à ce compte';
  end if;
  update patients set games_all_unlocked = p_unlock where code = p_code;
end;
$$;
