...
create table if not exists patients (
  ...
  -- v6.32 : quota gratuit journalier (voir sql/migrations/003_daily_quota.sql
  -- pour le détail du bug corrigé — installation existante ? exécuter
  -- cette migration plutôt que de recréer ce fichier en entier).
  daily_sessions_date  date,
  daily_sessions_count int not null default 0
);
...
-- v6.30/v6.32 : p_plan puis p_daily_sessions_date/p_daily_sessions_count
-- ajoutés (voir sql/migrations/002_plan_upsert.sql et
-- sql/migrations/003_daily_quota.sql pour le détail des bugs corrigés —
-- installation existante ? exécuter ces migrations plutôt que de
-- recréer ce fichier en entier).
create or replace function upsert_patient(
  ...
  p_daily_sessions_date date default null,
  p_daily_sessions_count int default null
