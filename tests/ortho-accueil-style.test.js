...
(async () => {
  ...
  await test('css : les connexions sont centrées verticalement (sur .wrap, flex-column + center)', () => {
    assert.ok(/#ortho-login \.wrap[^{]*\{[^}]*flex-direction:column[^}]*justify-content:center/s.test(CSS),
      'la règle de centrage vertical (portée par .wrap) est absente');
  });
