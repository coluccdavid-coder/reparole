...
// v6.61 : l'utilisateur a demandé un historique complet plutôt que
// tronqué (c'était limité à 20 séances / 10 entrées de journal à
// l'affichage, alors que les données complètes étaient déjà
// disponibles côté serveur sans limite).
await test('historique complet : plus de 20 séances -> toutes affichées, aucune tronquée', async ()=>{
  const dom = loadSummaryPage('http://localhost/mon-resume.html?code=p-test0000002');
  const history = [];
  for(let i=0;i<25;i++){
    history.push({ type:'denomination', score:i%5, total:5, level:2, at:new Date(Date.now()-i*86400000).toISOString() });
  }
  seedLocalPatient(dom, 'p-test0000002',
    { code:'p-test0000002', name:'Amina', level:2, sessions:25, correct:60, total:125, streak:1 },
    history
  );
  await dom.window.buildSummary();
  const rows = dom.window.document.querySelectorAll('#report table tbody tr');
  assert.strictEqual(rows.length, 25, `attendu 25 lignes (historique complet), obtenu ${rows.length}`);
});
