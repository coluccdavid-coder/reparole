...
await test('v6.162 : VRAI BUG CORRIGÉ — "Écouter la consigne" ne lit plus le code SVG de l\'horloge à voix haute (signalé par l\'utilisateur : "un tas de code incompréhensible")', async ()=>{
  const dom = loadApp();
  dom.window.eval(`Store.savePatient('SVGBUG', {name:'Test',level:1,sessions:0,correct:0,total:0,streak:1})`);
  dom.window.document.getElementById('name').value = 'Test';
  dom.window.document.getElementById('code').value = 'SVGBUG';
  await dom.window.eval('login()');
  await dom.window.eval("startExercise('heure')");
  const btn = dom.window.document.querySelector('.speak-btn');
  assert.ok(btn, 'bouton "Écouter la consigne" introuvable');
  const spoken = btn.getAttribute('onclick');
  assert.ok(!/<svg|viewBox|stroke-width/.test(spoken), `la consigne ne devrait jamais contenir de balisage SVG : "${spoken}"`);
  assert.ok(spoken.includes('Quelle heure est-il'), `consigne attendue "Quelle heure est-il ?", trouvé : ${spoken}`);
});
...
await test('v6.159 : les 5 exercices restent visibles en anglais (traduction complète désormais disponible)', ()=>{
  const dom = loadApp();
  dom.window.eval("Prefs.setLang('en')");
  TYPES.forEach(type=> assert.strictEqual(isHidden(dom, type), false, type));
});

await test('v6.159 : les 5 exercices sont visibles dans les 9 langues complètes, avec 12/12/12 items chacun', ()=>{
  const dom = loadApp();
  ['en','es','it','pt','de','ar','tr','pl','ja'].forEach(lang=>{
    const bank = dom.window['BANK_'+lang.toUpperCase()];
    TYPES.forEach(type=>{
      assert.ok(bank[type], `BANK_${lang.toUpperCase()}.${type} manquant`);
      [1,2,3].forEach(lvl=>{
        assert.strictEqual(bank[type].items[lvl].length, 12, `${lang}.${type} niveau ${lvl}`);
      });
    });
  });
});

await test('v6.159 : les traductions utilisent une monnaie locale adaptée, pas juste l\'euro traduit mot à mot', ()=>{
  const dom = loadApp();
  assert.ok(dom.window.BANK_EN.monnaie.items[1][0].answer.includes('$'), 'anglais devrait utiliser $');
  assert.ok(dom.window.BANK_JA.monnaie.items[1][0].answer.includes('円'), 'japonais devrait utiliser 円');
});

await test('v6.159 : 1620 items au total vérifiés sans doublon ni réponse orpheline, sur les 9 langues complètes', ()=>{
  const dom = loadApp();
  let checked = 0;
  ['en','es','it','pt','de','ar','tr','pl','ja'].forEach(lang=>{
    const bank = dom.window['BANK_'+lang.toUpperCase()];
    TYPES.forEach(type=>{
      [1,2,3].forEach(lvl=>{
        bank[type].items[lvl].forEach(it=>{
          assert.ok(it.choices.includes(it.answer), `${lang}.${type} niveau ${lvl} : "${it.answer}" absent de ses choix`);
          assert.strictEqual(new Set(it.choices).size, it.choices.length, `${lang}.${type} niveau ${lvl} : doublon`);
          checked++;
        });
      });
    });
  });
  assert.strictEqual(checked, 9*5*3*12); // 1620
});
