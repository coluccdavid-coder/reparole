...
(async () => {
  ...
  await test('edge : les 6 tâches existent, réparties par rôle (ortho vs admin), tâche inconnue refusée', () => {
    assert.ok(/ORTHO_TASKS = \['report_draft', 'suggest_words', 'prep_note', 'rewrite_note'\]/.test(DOC));
    assert.ok(/ADMIN_TASKS = \['triage_suggestions', 'errors_digest'\]/.test(DOC));
  ...
  await test('ortho : mots suggérés — AUCUN ajout automatique, ciblage uniquement au clic (délégation data-ai-word)', () => {
    ...
    assert.ok(m[0].indexOf("orthoPlan !== 'pro'") < m[0].indexOf('iaAssist'), 'contrôle Pro avant l\'appel');
  });
  ...
  await test('ortho : note de préparation -> zone éditable ; reformulation -> reste dans le champ, l\'ajout reste manuel', () => {
    ...
    for(const fn of [prep[0], rw[0]]){
      assert.ok(fn.indexOf("orthoPlan !== 'pro'") < fn.indexOf('iaAssist'), 'contrôle Pro avant l\'appel');
    }
  });
