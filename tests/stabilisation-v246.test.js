...
await test('plus aucun JSON.parse(localStorage…) non protégé dans storage.js', ()=>{
  const restants = [];
  // Le corps de _lsGet lui-même est le SEUL JSON.parse légitime (il est
  // dans un try) : on l'exclut de la chasse.
  const debutHelper = STORAGE.indexOf('function _lsGet(');
  const finHelper = STORAGE.indexOf('function _lsSet(');
  const [ligneDebut, ligneFin] = [STORAGE.slice(0, debutHelper).split('\n').length,
                                   STORAGE.slice(0, finHelper).split('\n').length];
  STORAGE.split('\n').forEach((ligne, i)=>{
    if(i+1 >= ligneDebut && i+1 <= ligneFin) return;
    // v6.249 : attrape aussi le motif indirect « raw = getItem(...) puis
    // JSON.parse(raw) », passé sous le radar de la v6.246 (3 occurrences).
    if((/JSON\.parse\(localStorage/.test(ligne) || /JSON\.parse\(raw\w*\)/.test(ligne)) && !/try\{/.test(ligne) && !/_lsGet/.test(ligne)) restants.push(i+1);
  });
