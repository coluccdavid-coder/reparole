...
await test('langue partielle (kabyle) : repli propre sur le français pour cet espace (pas de texte vide)', ()=>{
  const dom = loadPage('aidant.html');
  dom.window.eval("Prefs.setLang('kab')");
  const title = dom.window.document.querySelector('#caregiver-login h1').textContent;
  assert.strictEqual(title, 'Espace aidant');
});
...
await test('langue partielle (sango) : repli propre sur le français pour cet espace', ()=>{
  const dom = loadPage('dashboard-ortho.html');
