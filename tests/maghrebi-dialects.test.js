...
//  (LANGUAGES, PARTIAL_LANGS), le socle minimal de 22 clés (même
//  périmètre que le kabyle), et que chaque dialecte utilise bien ses
...
console.log('\nSocle minimal (mêmes 22 clés que le kabyle, pas plus)');

await test('les trois dialectes ont bien le même socle de 22 clés que le kabyle (ni plus, ni moins, cohérent avec le statut "partiel")', ()=>{
  const dom = loadPatientApp();
  const kabKeys = Object.keys(dom.window.I18N_STRINGS.kab).sort();
  ['dz','ma','tn'].forEach(code=>{
    const keys = Object.keys(dom.window.I18N_STRINGS[code]).sort();
    assert.deepStrictEqual(keys, kabKeys, `${code} devrait couvrir exactement les mêmes clés que le kabyle`);
  });
});
