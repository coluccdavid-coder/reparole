...
await test('window.BANK_DZ existe avec dénomination + complétion (PAS encore compréhension)', ()=>{
  ...
  assert.ok(!B.comprehension, 'comprehension ne devrait pas encore exister — voir docs/dz-parity-request.md (mettre à jour ce test le jour où elle est ajoutée)');
});
...
await test('dz reste dans PARTIAL_LANGS (contenu partiel : pas de compréhension, pas de voix)', ()=>{
  const dom = loadPatientApp();
