...
await test('code aidant inconnu -> message d\'erreur, tableau de bord non affiché', async ()=>{
  ...
  // v6.61 : vérifie la vraie classe utilisée pour l'affichage
  // (classList "active") plutôt que style.display, qui n'est plus le
  // mécanisme employé depuis le correctif du bug "page blanche".
  assert.strictEqual(dom.window.document.getElementById('caregiver-dashboard').classList.contains('active'), false);
});
...
await test('caregiverLogout() revient à l\'écran de connexion', async ()=>{
  ...
  assert.strictEqual(dom.window.document.getElementById('caregiver-login').classList.contains('active'), true);
  assert.strictEqual(dom.window.document.getElementById('caregiver-dashboard').classList.contains('active'), false);
});
