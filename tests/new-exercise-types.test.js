...
console.log('\nNouvel exercice "Trouver la rime" (point 6 de la demande d\'amélioration)');

await test('BANK.rhyme existe avec 8/8/8 items par niveau', ()=>{
  const dom = loadPatientApp();
  const B = dom.window.BANK;
  assert.ok(B.rhyme, 'BANK.rhyme manquant');
  [1,2,3].forEach(lvl=>{
    assert.strictEqual(B.rhyme.items[lvl].length, 8, `niveau ${lvl}`);
  });
});

await test('rhyme : chaque item a sa bonne réponse dans ses propres choix, sans doublon', ()=>{
  const dom = loadPatientApp();
  const B = dom.window.BANK;
  let checked = 0;
  [1,2,3].forEach(lvl=>{
    B.rhyme.items[lvl].forEach(it=>{
...
await test('la tuile "Trouver la rime" existe sur le tableau de bord', ()=>{
  const dom = loadPatientApp();
  assert.ok(dom.window.document.querySelector('[data-type="rhyme"]'));
});

await test('startExercise(\'rhyme\') fonctionne comme les autres types de choix multiple', async ()=>{
  const dom = loadPatientApp();
  await dom.window.eval("Store.savePatient('T7', {name:'Test',level:1,sessions:0,correct:0,total:0,streak:1})");
  dom.window.document.getElementById('name').value = 'Test';
  dom.window.document.getElementById('code').value = 'T7';
  await dom.window.eval('login()');
  await dom.window.eval("startExercise('rhyme')");
  const c = dom.window.eval('__testGetCurrent()');
  assert.strictEqual(c.type, 'rhyme');
  assert.ok(c.total > 0);
});

await test('showExerciseHelp() fonctionne pour "rhyme" (fr) — vérifié immédiatement cette fois', async ()=>{
  const dom = loadPatientApp();
  await dom.window.eval("Store.savePatient('T8', {name:'Test',level:1,sessions:0,correct:0,total:0,streak:1})");
  dom.window.document.getElementById('name').value = 'Test';
  dom.window.document.getElementById('code').value = 'T8';
  await dom.window.eval('login()');
  await dom.window.eval("startExercise('rhyme')");
  dom.window.eval('showExerciseHelp()');
  const bubble = dom.window.document.querySelector('#companion-exercise .companion-bubble');
  assert.ok(bubble && bubble.textContent.length > 0, 'la bulle d\'aide devrait afficher un texte pour "rhyme"');
});

await test('COMPANION_PHRASES : "rhyme" présent dans les 10 langues complètes + dz/ma/tn (comme association/syntax)', ()=>{
  const dom = loadPatientApp();
  ['fr','en','es','it','pt','de','ar','tr','pl','ja','dz','ma','tn'].forEach(lang=>{
    const ex = dom.window.COMPANION_PHRASES[lang].explain;
    assert.ok(ex && ex.rhyme, `COMPANION_PHRASES.${lang}.explain.rhyme manquant`);
  });
});
