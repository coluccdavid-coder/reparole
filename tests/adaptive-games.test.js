...
(async () => {
  ...
  await test('robustesse : mélange qui évite l\'ordre déjà correct + banque trop maigre = repli silencieux + emojis abstraits exclus', () => {
    ...
    // v6.185 : une image de jeu doit être un objet reconnaissable — la
    // bulle 💬 en piège a dérouté l'utilisateur (capture CHAMPIGNON).
    assert.ok(/ABSTRACT_GAME_EMOJIS = new Set\(\['💬'/.test(APP), 'filtre des emojis abstraits absent');
    assert.ok(/!ABSTRACT_GAME_EMOJIS\.has\(it\.emoji\)/.test(APP), 'le pool doit appliquer le filtre');
  });
