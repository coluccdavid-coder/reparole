...
//  contrairement au kabyle/sango, le japonais a un vrai speechLocale et
...
await test('window.BANK_JA existe avec les 3 types d\'exercice attendus', ()=>{
...
await test('ja présent dans LANGUAGES avec un speechLocale (contrairement au kabyle/sango)', ()=>{
  const dom = loadPatientApp();
...
await test('ja présent dans PARTIAL_LANGS (interface non traduite, même si le contenu est riche)', ()=>{
  const dom = loadPatientApp();
  assert.ok(dom.window.PARTIAL_LANGS.includes('ja'));
});
...
await test('dénomination en japonais : utilise BANK_JA, PAS le mécanisme audio pré-enregistré (isKabDenom doit être faux)', async ()=>{
  ...
  // Contrairement au kabyle/sango, le bouton doit appeler speak() (synthèse
...
await test('exercice vocal non disponible en japonais (ex. répétition) : repli propre sur le français, pas de plantage', async ()=>{
  const dom = loadPatientApp();
  dom.window.eval("Prefs.setLang('ja');");
