...
const { Learner, classifyError, lengthTag, fieldTag, CLINICAL_PROFILES } = require('../js/learner.js');
...
console.log('Profils cliniques (v6.42 : traumatisme crânien, ORL, Parkinson)');
test('les 3 nouveaux profils existent avec un label et des boosts', ()=>{
  ['traumatisme_cranien','orl','parkinson'].forEach(key=>{
    assert.ok(CLINICAL_PROFILES[key], `profil ${key} manquant`);
    assert.ok(CLINICAL_PROFILES[key].label && CLINICAL_PROFILES[key].label.length > 0);
    assert.ok(Object.keys(CLINICAL_PROFILES[key].boost).length > 0);
  });
});
test('traumatisme crânien priorise dénomination/compréhension/fluence', ()=>{
  Learner.load({ byType:{ denomination:{seen:5,ok:3}, comprehension:{seen:5,ok:3}, repetition:{seen:5,ok:3} }, errors:{}, byTag:{} });
  const withProfile = Learner.priority('denomination','traumatisme_cranien');
  const withoutProfile = Learner.priority('denomination', null);
  assert.ok(withProfile > withoutProfile, 'le score doit augmenter avec le profil traumatisme_cranien');
});
test('ORL priorise répétition/dénomination orale/intonation, pas complétion', ()=>{
  Learner.load({ byType:{ repetition:{seen:5,ok:3}, completion:{seen:5,ok:3} }, errors:{}, byTag:{} });
  const repWithProfile = Learner.priority('repetition','orl');
  const repWithout = Learner.priority('repetition', null);
  const complWithProfile = Learner.priority('completion','orl');
  const complWithout = Learner.priority('completion', null);
  assert.ok(repWithProfile > repWithout, 'répétition doit être boostée pour le profil orl');
  assert.strictEqual(complWithProfile, complWithout, 'complétion ne doit pas être affectée par le profil orl');
});
test('Parkinson priorise répétition/intonation, sans jamais toucher aux exercices de déglutition (garde-fou n°2 : aucun exercice de ce type n\'existe dans l\'app)', ()=>{
  const boostedTypes = Object.keys(CLINICAL_PROFILES.parkinson.boost);
  assert.ok(!boostedTypes.includes('deglutition') && !boostedTypes.includes('dysphagie'));
});
