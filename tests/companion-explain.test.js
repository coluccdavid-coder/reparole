...
console.log('\nComportement de Companion.explain()');
{
  ...
  test("repli sur le français si la langue active n'a pas de banque explain", () => {
    ...
    // l'app — toutes les langues partielles restantes (kab/dz/ma/tn)
    // ont maintenant leur propre banque COMPANION_PHRASES. Code de
    // langue synthétique pour continuer à couvrir ce repli.
    dom.window.eval("Prefs.data.lang = 'xx'; Companion.explain('companion-exercise', 'denomination');");
    const bubbleText = dom.window.document.querySelector('#companion-exercise .companion-bubble').textContent;
  ...
  test('utilise la bonne langue quand une banque explain existe (kabyle, v6.119)', () => {
...
console.log("\nAccent chaleureux sur la bulle de célébration (v6.52)");
{
  const dom = loadApp();

  test('mood "delighted" (série de jours) : la bulle porte companion-bubble-celebrate', () => {
    dom.window.eval("Companion.say('streak'); Companion.render('companion-dashboard');");
    const bubble = dom.window.document.querySelector('#companion-dashboard .companion-bubble');
    assert.ok(bubble, 'la bulle doit être affichée');
    assert.ok(bubble.classList.contains('companion-bubble-celebrate'), 'mood "delighted" doit ajouter companion-bubble-celebrate');
  });

  test('mood "happy" (bonne réponse) : pas de companion-bubble-celebrate', () => {
    dom.window.eval("Companion.say('correct'); Companion.render('companion-dashboard');");
    const bubble = dom.window.document.querySelector('#companion-dashboard .companion-bubble');
    assert.ok(bubble, 'la bulle doit être affichée');
    assert.ok(!bubble.classList.contains('companion-bubble-celebrate'), 'seule la série de jours doit avoir cet accent, pas une simple bonne réponse');
  });
}
