...
test('.dashboard-grid existe et contient exactement 11 cartes (v6.95 : + carte conseils orthophoniste)', () => {
  const grid = doc.querySelector('#dashboard .dashboard-grid');
  assert.ok(grid, '.dashboard-grid doit exister dans #dashboard');
  const cards = grid.querySelectorAll(':scope > .card');
  assert.strictEqual(cards.length, 11, `attendu 11 cartes directement dans .dashboard-grid, trouvé ${cards.length}`);
});
