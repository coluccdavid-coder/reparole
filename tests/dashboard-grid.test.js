...
test('.dashboard-grid existe et contient exactement 12 cartes (v6.95 : + conseils orthophoniste ; v6.174 : + boucle vocale)', () => {
  const grid = doc.querySelector('#dashboard .dashboard-grid');
  assert.ok(grid, '.dashboard-grid doit exister dans #dashboard');
  const cards = grid.querySelectorAll(':scope > .card');
  assert.strictEqual(cards.length, 12, `attendu 12 cartes directement dans .dashboard-grid, trouvé ${cards.length}`);
});
