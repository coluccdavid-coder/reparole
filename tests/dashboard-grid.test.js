...
test('.dashboard-grid existe et contient exactement 9 cartes', () => {
  const grid = doc.querySelector('#dashboard .dashboard-grid');
  assert.ok(grid, '.dashboard-grid doit exister dans #dashboard');
  const cards = grid.querySelectorAll(':scope > .card');
  assert.strictEqual(cards.length, 9, `attendu 9 cartes directement dans .dashboard-grid, trouvé ${cards.length}`);
});
