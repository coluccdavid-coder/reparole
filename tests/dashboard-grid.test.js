...
//  ---------------------------------------------------------------------
//  Vérifie que les cartes du tableau de bord sont bien à l'intérieur
...
test('.dashboard-grid existe et contient exactement 13 cartes (v6.95 conseils ; v6.174 boucle vocale ; v6.183 exercices sur mesure)', () => {
  const grid = doc.querySelector('#dashboard .dashboard-grid');
  assert.ok(grid, '.dashboard-grid doit exister dans #dashboard');
  const cards = grid.querySelectorAll(':scope > .card');
  assert.strictEqual(cards.length, 13, `attendu 13 cartes directement dans .dashboard-grid, trouvé ${cards.length}`);
});
