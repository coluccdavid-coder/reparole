...
(async () => {
  ...
  await test('storage : orthoGenerateReportDraft — jeton de session (pas la clé anon) + échec propre si non déployée', () => {
    ...
    assert.ok(/catch\(e\)\{[\s\S]*?return \{ error: 'indisponible' \};/.test(m[0]),
      "v6.186 : un échec réseau (fonction non déployée -> pré-vol CORS) doit se mapper vers 'indisponible', pas un NetworkError brut");
  ...
  await test('js ortho : gating Pro avant tout appel + gestion des erreurs distincte (indisponible vs échec)', () => {
    ...
    const idxPlan = m[0].indexOf('!OrthoApp.iaAllowed()');
