...
test('langue sans COMPANION_PHRASES du tout : repli sur le français, pas de texte vide ni d\'erreur', ()=>{
  // v6.151 : sango (qui servait jusqu'ici d'exemple de langue partielle
  // sans COMPANION_PHRASES) a été retiré de l'app — toutes les langues
  // partielles restantes (kab/dz/ma/tn) ont maintenant leur propre
  // banque. On force donc un code de langue synthétique, jamais
  // configuré nulle part, pour vérifier que le repli sur le français
  // reste robuste si une future langue partielle se retrouve
  // temporairement dans cet état (comme dz/ma/tn l'ont été avant
  // d'avoir leur contenu).
  const dom = loadPatientApp();
  dom.window.eval("Prefs.data.lang = 'xx';"); // code volontairement inexistant, contourne setLang()
  dom.window.eval("startExercise('denomination')");
  dom.window.eval('showExerciseHelp()');
  const bubbleText = dom.window.document.querySelector('#companion-exercise .companion-bubble').textContent;
