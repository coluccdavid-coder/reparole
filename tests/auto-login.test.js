...
await test('une connexion réussie enregistre code ET prénom en sessionStorage (v6.139 : format JSON, plus juste le code)', async ()=>{
  const dom = loadPatientApp();
  ...
  await dom.window.eval('login()');
  const stored = JSON.parse(dom.window.sessionStorage.getItem('reparole:active-session'));
  assert.strictEqual(stored.code, 'T2');
  assert.strictEqual(stored.name, 'Paul');
});
...
await test('sessionStorage a priorité sur localStorage quand les deux existent', async ()=>{
  ...
  `);
  dom.window.sessionStorage.setItem('reparole:active-session', JSON.stringify({code:'T5', name:'Sami'}));
  await dom.window.eval("attemptAutoLogin()");
