...
await test('aucune voix pour la langue active -> message clair au lieu d\'un silence inexpliqué', async ()=>{
  ...
  // v6.150 : speak() essaie d'abord un fichier audio pré-généré
  // (asynchrone) avant de retomber sur la synthèse navigateur —
  // speakBrowserTTS() est la fonction qui porte la logique testée
  // ici (choix de voix, message d'erreur), donc on l'appelle
  // directement plutôt que d'attendre l'échec du chargement audio.
  dom.window.speakBrowserTTS('test');
...
await test('une voix existe pour la langue -> utilisée sur l\'utterance, pas de message d\'erreur', async ()=>{
  ...
  const dom = loadPatientApp(mock);
  await dom.window.loadVoices();
  dom.window.speakBrowserTTS('bonjour'); // v6.150 : voir commentaire ci-dessus
