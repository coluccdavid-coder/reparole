...
console.log('Pied de page commun (mentions légales / confidentialité / cookies / CGV / CGU)');
...
for(const page of pagesWithFooter){
  await test(`${page} : charge js/footer.js et affiche les 6 liens légaux`, async ()=>{
    ...
    assert.ok(hrefs.includes('confidentialite.html'), 'lien vers la politique de confidentialité manquant');
    assert.ok(hrefs.includes('politique-cookies.html'), 'lien vers la politique de cookies manquant');
    assert.ok(hrefs.includes('gestion-cookies.html'), 'lien vers la gestion des cookies manquant');
...
console.log('\nBandeau "version bêta" en haut de la page d\'accueil');

await test('index.html : bandeau bêta présent, visible avant tout autre contenu', ()=>{
  const dom = loadPage('index.html');
  const banner = dom.window.document.getElementById('beta-banner');
  assert.ok(banner, 'bandeau bêta manquant');
  assert.ok(/bêta|beta/i.test(banner.textContent));
  // Doit être l'un des tout premiers éléments de <body>, pas enfoui dans un écran
  const bodyChildren = [...dom.window.document.body.children];
  assert.ok(bodyChildren.indexOf(banner) <= 1, 'le bandeau doit être tout en haut de la page');
});

await test('le bandeau bêta a bien un bouton de fermeture qui le retire du DOM', ()=>{
  const dom = loadPage('index.html');
  const banner = dom.window.document.getElementById('beta-banner');
  const closeBtn = banner.querySelector('button');
  assert.ok(closeBtn);
  // v6.88 : runScripts:'outside-only' n'exécute pas les attributs
  // onclick inline (même limite que partout ailleurs dans ce fichier
  // de test) — on vérifie donc l'action déclarée plutôt que de
  // déclencher un vrai clic, qui ne ferait rien ici.
  assert.ok(closeBtn.getAttribute('onclick').includes('remove()'));
});
