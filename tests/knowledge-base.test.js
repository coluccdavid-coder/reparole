      ...
      createClient(){
        return {
          auth: {
            ...
            // v6.82 : double authentification admin — ce test simule un
            // compte sans MFA activée (aal1 -> aal1, jamais de défi).
            mfa: {
...
await test('mode navigateur (pas de cloud) : mergeApprovedContent ne fait rien', async ()=>{
  ...
  for(const src of scripts){
    ...
    // v6.45.1 : ce test veut le mode navigateur (Store.mode()==='navigateur')
    // quelles que soient les vraies clés Supabase baked-in dans storage.js —
    // on les vide explicitement pour ce chargement, sans dépendre du
    // contenu réel du fichier (qui contient désormais les identifiants
    // du projet Supabase de l'utilisateur).
    if(src === 'js/storage.js'){
  ...
  await dom.window.mergeApprovedContent(); // Store.mode() === 'navigateur' ici (clés forcées vides pour ce test)
