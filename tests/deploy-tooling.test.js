...
(async () => {
  ...
  // ---- 4. Hygiène de la suite de tests (v6.244) ----
  // Un fichier de test présent dans tests/ mais absent de `npm test` ne
  // protège RIEN, et devient un piège : il vieillit sans que personne ne
  // le voie, puis quelqu'un le lance un jour et croit à une régression.
  // C'est exactement ce qui est arrivé à tests/fiche-clinique-v197.test.js,
  // brouillon abandonné d'une conception non retenue, supprimé en v6.244.
  await test('hygiène : tout fichier tests/*.test.js est bien lancé par npm test', () => {
    const script = JSON.parse(fs.readFileSync(path.join(ROOT, 'package.json'), 'utf8')).scripts.test;
    const wired = [...script.matchAll(/tests\/([\w.-]+\.test\.js)/g)].map(m => m[1]);
    const onDisk = fs.readdirSync(path.join(ROOT, 'tests')).filter(f => f.endsWith('.test.js'));
    const orphans = onDisk.filter(f => !wired.includes(f));
    assert.strictEqual(orphans.length, 0,
      `jamais lancé(s) par npm test : ${orphans.join(', ')} — les brancher ou les supprimer`);
  });

  await test('hygiène : npm test ne référence aucun fichier de test disparu', () => {
    const script = JSON.parse(fs.readFileSync(path.join(ROOT, 'package.json'), 'utf8')).scripts.test;
    const wired = [...script.matchAll(/tests\/([\w.-]+\.test\.js)/g)].map(m => m[1]);
    const ghosts = wired.filter(f => !fs.existsSync(path.join(ROOT, 'tests', f)));
    assert.strictEqual(ghosts.length, 0,
      `référencé(s) dans package.json mais absent(s) du disque : ${ghosts.join(', ')}`);
  });
