// =====================================================================
//  TESTS — Droit à l'effacement : suppression de compte (v6.87)
//  ---------------------------------------------------------------------
...
//  (mot à taper + confirm() natif), le nettoyage réel des données en
//  mode navigateur, et le comportement de ReParoleStore.deleteAccount
//  en mode cloud (RPC appelée avec le bon code).
//
...
function loadPatientApp(){
  ...
  dom.window.confirm = () => true; // valeur par défaut, écrasée dans les tests qui testent l'annulation
  return dom;
}
...
await test('la section existe avec les bons éléments', ()=>{
  ...
  assert.ok(dom.window.document.querySelector('[onclick="deleteMyAccount()"]'));
...
await test('mot correct mais annulation du confirm() natif -> rien n\'est supprimé', async ()=>{
  const dom = loadPatientApp();
  dom.window.localStorage.setItem('reparole:T', JSON.stringify({ code:'T', name:'Marie' }));
  dom.window.document.getElementById('account-delete-confirm').value = 'SUPPRIMER';
  dom.window.confirm = () => false;
  await dom.window.deleteMyAccount();
  assert.ok(dom.window.localStorage.getItem('reparole:T') !== null, 'les données ne doivent pas être touchées si on annule');
});

await test('mot correct (insensible à la casse) + confirmation -> suppression réelle en mode navigateur', async ()=>{
  const dom = loadPatientApp();
  dom.window.localStorage.setItem('reparole:T', JSON.stringify({ code:'T', name:'Marie' }));
...
await test('retire aussi le profil de la liste des codes mémorisés (v6.71)', async ()=>{
  const dom = loadPatientApp();
  dom.window.localStorage.setItem('reparole:T', JSON.stringify({ code:'T', name:'Marie' }));
  dom.window.localStorage.setItem('reparole:remembered-profiles', JSON.stringify([{ code:'T', name:'Marie' }]));
  dom.window.document.getElementById('account-delete-confirm').value = 'SUPPRIMER';
  dom.window.confirm = () => true;
  await dom.window.deleteMyAccount();
  const remembered = JSON.parse(dom.window.localStorage.getItem('reparole:remembered-profiles') || '[]');
