...
console.log('Structure gratuit/pro (patient)');
{
  ...
  dom.window.eval("__testSetUser({plan:'free'});");

  test('compte gratuit + français + exercice texte = accessible', () => {
    const r = dom.window.eval("lockReason('denomination')");
    assert.strictEqual(r, null);
  });
  test('compte gratuit + français + exercice vocal avancé = verrouillé (type)', () => {
    const r = dom.window.eval("lockReason('repetition')");
    assert.strictEqual(r, 'type');
  });
  test('compte gratuit + conversation guidée = verrouillé (type)', () => {
    const r = dom.window.eval("lockReason('conversation')");
    assert.strictEqual(r, 'type');
  });
  test('compte gratuit + langue non-française = verrouillé (lang)', () => {
    dom.window.eval("Prefs.setLang('es');");
    const r = dom.window.eval("lockReason('denomination')");
    assert.strictEqual(r, 'lang');
    dom.window.eval("Prefs.setLang('fr');");
  });
  test('compte pro = jamais verrouillé, même vocal + langue étrangère', () => {
  ...
  test('quota journalier : 5 séances gratuites puis verrouillé (quota)', () => {
  ...
  test('quota : jour différent = compteur remis à zéro', () => {
