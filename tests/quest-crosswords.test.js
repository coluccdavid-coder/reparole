...
(async () => {
  ...
  await test('croisés : tuile présente, type gratuit, indice = image (aucune définition textuelle dans le rendu)', () => {
    ...
    // v6.248 : l'indice visuel passe par ExoImages (photo réelle si elle
    // existe, sinon l'emoji dans une carte sobre) — l'intention du test
    // reste identique : un INDICE PAR L'IMAGE, jamais par une définition.
    assert.ok(/ExoImages\.html\(/.test(r), "l'indice doit être l'image du mot (photo ou emoji via ExoImages)");
