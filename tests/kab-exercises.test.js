...
console.log('\nParité complète de l\'interface avec le français (v6.145)');

await test('v6.145 : VRAI BUG CORRIGÉ — kab était tombé à 513/538 (mélange visible de kabyle et de français signalé par l\'utilisateur sur capture d\'écran). Doit rester à 100% désormais.', ()=>{
  const dom = loadPatientApp();
  const fr = dom.window.I18N_STRINGS.fr;
  const missing = Object.keys(fr).filter(k=>!(k in dom.window.I18N_STRINGS.kab));
  assert.strictEqual(missing.length, 0, `${missing.length} clé(s) manquante(s) — ${missing.slice(0,5).join(', ')}`);
});
