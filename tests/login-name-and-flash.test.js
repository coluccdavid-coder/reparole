...
function loadPatientApp(){
  ...
  for(const code of inlineScripts){
    ...
    // n'en chargent qu'une. En test, on les charge toutes.
    if(typeof src !== 'undefined' && src === 'js/i18n.js') fs.readdirSync(path.join(ROOT, 'js/i18n'))
      .forEach(lf => dom.window.eval(fs.readFileSync(path.join(ROOT, 'js/i18n', lf), 'utf8'))); }catch(e){ /* certains dépendent d'API absentes de jsdom (serviceWorker) */ }
  }
...
await test('le bon prénom (même casse) se connecte normalement', async ()=>{
  const dom = loadPatientApp();
  ...
  assert.strictEqual(helloText, 'Paul');
});

await test('le bon prénom, casse/espaces différents, se connecte quand même (comparaison tolérante)', async ()=>{
  const dom = loadPatientApp();
  await dom.window.eval("Store.savePatient('T2b', {name:'Paul',level:2,sessions:1,correct:1,total:1,streak:1})");
  dom.window.document.getElementById('name').value = '  PAUL  ';
  dom.window.document.getElementById('code').value = 'T2b';
  await dom.window.eval('login()');
  const activeScreen = dom.window.document.querySelector('.screen.active');
  assert.strictEqual(activeScreen && activeScreen.id, 'dashboard');
});

await test('v6.141 : VRAI BUG CORRIGÉ — un prénom différent du dossier est maintenant refusé (avant : accepté et écrasait le vrai prénom)', async ()=>{
  const dom = loadPatientApp();
  await dom.window.eval("Store.savePatient('T2c', {name:'Paul',level:2,sessions:1,correct:1,total:1,streak:1})");
  dom.window.document.getElementById('name').value = 'Paulette';
  dom.window.document.getElementById('code').value = 'T2c';
  await dom.window.eval('login()');
  const activeScreen = dom.window.document.querySelector('.screen.active');
  assert.strictEqual(activeScreen && activeScreen.id, 'login', 'ne devrait pas se connecter avec un prénom différent');
  const err = dom.window.document.getElementById('login-error').textContent;
  assert.ok(err.length > 0, 'un message d\'erreur devrait être affiché');
  // et le vrai prénom du dossier ne doit surtout pas avoir été écrasé
  const rec = JSON.parse(dom.window.localStorage.getItem('reparole:T2c'));
  assert.strictEqual(rec.name, 'Paul');
});
...
await test('créer un dossier sans prénom est refusé, message clair affiché', ()=>{
  const dom = loadPatientApp();
  dom.window.document.getElementById('new-name').value = '';
  dom.window.eval('createNewPatient()');
  const err = dom.window.document.getElementById('new-error').textContent;
  assert.ok(err.length > 0, 'un message d\'erreur devrait être affiché');
  const activeScreen = dom.window.document.querySelector('.screen.active');
